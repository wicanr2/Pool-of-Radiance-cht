...
上一版寫 60～70%。往上挪，是因為這一版**把「還缺什麼」核實過一遍**：原本列的
八條功能缺口裡有四條早就做完了（休息被打斷、遠程武器的射程、結局的圖、說明書
附錄的規則表），條目卻還留著——那四條是自評的依據高估了缺口，不是這一版新做的。
真正新接上的是效果串列跟著時間走（進出戰場、走一步一分鐘、紮營一刻五分鐘、
到期先跑收尾）與賊技能的建角端（三張原版表逐格對上三個 dosgolem 樣本）。

區間從十個百分點收到七個，理由同上：知道自己缺什麼，估計才會準。

**壓住上限的仍然是同樣那三項**：戰鬥數值沒有對過原版、主線沒有從開場到結局
連續跑過一次、Windows 與 macOS 的真機啟動沒有回填結果。這三項連續兩版都沒有
進展，它們才是 72% 上面那一段。
...
- **探險者手冊進遊戲了，而且不必自己翻**：說明書上冊的線索報導 58 條、
  ...
  直接打編號就跳到那一條。下冊附錄的七節規則表也在裡面（金錢換算、法術表、
  裝備一覽、昇級所需經驗、牧師對抗不死怪物、各等級的裝備與武器、武器一覽）。
...
- **作弊選單（預設關）**：冒險或戰鬥中按 `F6`，`L` 切「鎖 HP」（隊員的 HP 不會少、倒地拉回來）、
  `O` 切「一擊斃命」（隊員命中就讓目標 HP 歸零），`W` 切「穿牆」（牆與鎖門都當開著），`ESC` 或 `F6` 關閉。原版沒有這個功能：
  開著時畫面標「作弊：…」，打開過一次存檔就永遠記著，冒險畫面與隊伍選單一直標「開過作弊」。
  開過作弊的路線不算原版驗收（[spec 141](docs/spec/141-cheat-menu.md)）。F1 說明頁第 1 頁列著這個鍵。
- **密語提示（預設開）**：原版有 20 處要玩家打字（守衛的口令、亡魂的暗語、矮人符文），
  字是幾十格之前的 NPC 說的或說明書寫的。remake 預設把答案接在問句後面——
  `WHAT DO YOU SAY? (TYPE A SINGLE WORD)（LUX）`。不想要就 `F6` 進作弊選單按 `P` 關掉，
  關掉之後問句與原版逐字相同。**關掉不算作弊**（那是往原版靠，不是加東西），
  所以不會寫「開過作弊」（[spec 141](docs/spec/141-cheat-menu.md)、
  [spec 087](docs/spec/087-ecl-input-opcodes.md)）。
...
戰鬥畫面的版面照原版（[spec 129](docs/spec/129-combat-screen-layout-and-command-bar.md)）：
左邊 7×7 格的戰場、右邊三行資訊（名字／`HITPOINTS`／`AC`）、框外一列指令。
外框與冒險畫面**不是同一套**——下緣高一列，而且中間多一條把戰場與資訊欄分開。
對原版逐格比，**外框 9152／9152 ＝ 100%**。

**最下面那一列是算出來的，不是固定字串。** 原版有六段（`Move `／`View Aim `／
`Use `／`Cast `／`Turn `／`Quick Done`），逐段判斷要不要接上去：身上沒東西沒有
`Use `、沒記法術沒有 `Cast `、不是牧師沒有 `Turn `。中文取說明書下冊第五章的
官方譯名（移動／檢視／瞄準／使用／施法／轉變／自動戰鬥／特殊事項）。
照抄八項會讓玩家看到按不動的鍵。
...
分三層）：壓住自評的主要是**玩家會撞到的功能缺口**——

- **走路時沒有人擲遭遇。** 原版走兩步就會撞上哥布林（dosgolem 實跑，截圖在
  `docs/reference/original-dos/slums/`）；remake 的戰鬥目前全部掛在格子事件底下。
  遭遇本身**排得出來**（貧民窟那一段讀完了，spec 136），缺的是走路時的那一次擲骰。
- **探索畫面的 `CAST` 沒有入口**，施法只在戰鬥中做得到；法術效果的派發表
  67 格只讀了 42 個編號。
- **賊技能升級不會長。** 建角那一端接上了（spec 095），訓練所升級那一端還沒查。
- **能量吸取沒有來源。** 欠帳與還帳都接好了，缺的是呼叫它的怪物特殊攻擊那一批。
- **效果代碼的處理常式只接了一個。** 到期的入口接上了，138 個代碼裡真正實作過
  收尾的只有雲團。
- **隊員的多次攻擊沒接**，一回合一次；怪物那一側已經照原版的編碼走。

其次是**驗證缺口**：戰鬥數值沒有對過原版、主線沒有從開場到結局連續跑過一次、
Windows 與 macOS 的真機啟動還沒回填結果、五十六份規格沒有測試指回它。
