...
上一版寫 55～65%，這一版加的是「UI 譯文補完」與「發行包對原版的抽樣對拍
有數字了」；戰鬥數值與真機驗收兩項沒有進展，所以只往上挪五個百分點。
...
十六張的逐項比例在報告裡，這裡不複製一份：數字會隨版本動，抄出來的那份不會。
...
分三層）：壓住自評的主要是**玩家會撞到的功能缺口**——探索畫面的 `CAST` 與
`VIEW` 沒有入口（施法只在戰鬥中做得到）、法術效果的派發表 67 格只讀了 42 個
編號、地圖上沒有遊蕩的怪物群、賊技能與能量吸取沒有產生端、戰士的多次攻擊與
遠程武器的射程沒接、休息不會被打斷、結局的圖沒接、說明書下冊附錄的三張規則表
沒接進 UI。其次是**驗證缺口**：戰鬥數值沒有對過原版、主線沒有從開場到結局
連續跑過一次、Windows 與 macOS 的真機啟動還沒回填結果。
