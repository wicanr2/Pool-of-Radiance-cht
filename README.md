...
可從開場玩到結局的繁體中文 remake。**從標題到結局已經只用鍵盤跑通過**
（建角／建隊 → Phlan → 四件委任 → 城堡 → 結局；作弊選單開著，原版那一側
同樣作弊跑一次，兩邊逐段對照必經區塊與旗標）；遊戲內的敘述文字全部是繁體中文。
...
- **戰術戰鬥打得完了**：擺陣、先攻、移動預算、命中與傷害都照原版的規則走
  ...
  搆得到的人不只一個時擲骰挑（`0D97h`），追擊目標過不了否決查詢（`1087h`）就劃掉
  重擲、最多二十次、挑不到再放寬一輪（`37B8h`）。否決查詢的四個代碼都照原版（spec 059／112）。
...
- **自訂規則（預設關）**：隊伍選單的 `H` 切換「委任經驗值加倍」——原版的市政廳獎賞在戰後
  結算就會折成經驗值、全隊分；開著時同一份再發一次，冒險畫面右上角會標「自訂規則」。關著時所有
