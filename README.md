...
| 正常主線可破關 | 腳本層無阻塞；**戰鬥數值與敵方 AI 仍與原版有差**，未做全程實跑 |
...
- **法術表解出來了**：`START.EXE` 裡有一張 56 筆的法術名稱表（位移 41052，
  ...
  56 條的中譯全部取自軟體世界的官方說明書。遊戲裡按 `K` 查閱。
  記憶與施展還沒接，畫面上也不假裝接好了。契約見 spec 068。
...
- **戰術戰鬥打得完了**：擺陣、先攻、移動預算、命中與傷害都照原版的規則走
  ...
  怪物，戰後腳本接著跑。**敵方 AI 還不是原版的**：目前是「朝目標走一格」，
  在斜向多牆的盤面上繞不過去，所以戰鬥比原版容易（spec 096 已讀出原版的
  骨架）。
