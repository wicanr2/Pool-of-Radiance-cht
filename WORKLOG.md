...
- 目標：推進 GitHub issue #8，從未抽樣清單挑一個可由既有路徑決定性抵達的畫面。
...
- 發行包：以 Docker-only 流程從當前原始碼乾淨建立本機
  ...
  並修掉雙引號命令中的反引號誤執行。
- 驗證：`tools/appimage-dos-parity.sh` 正常玩家按鍵路徑產生 21 項報表；
  `camp-rest` 整張 79.27%、外框 94.04%、視野 100.00%。issue #8 是滾動清單，
  其餘八個 camp 子層與其他畫面仍保留，未宣稱完成。
