...
- dosgolem 原版收據確認肖像內容是 native `224,8` 起的 88×88；原版另外以
  `8X8D1.DAX` block 202 item 20／21／22 畫框，左緣 `x=216..223`、下緣
  `y=96..103`，上、右兩邊與整頁外框共用。
- renderer 將內容由錯誤的 native x=216 移到 x=224，並用原始 symbol 與當前
  `artPalette` 補左、下框，不修改人物資產。七個框 tile 與 selector 1／1 的
  88×88 內容逐像素對原版；selector 1／1、14／12 以英文與繁中共四組走正式 renderer。
- `v.1.1.6-20260914` AppImage 對拍通過：建角資料頁由 86.02% 升到 92.12%，
  `VIEW` 資料頁由 86.25% 升到 92.35%，兩者框比例均由 98.39% 升到 100%。
- 完整 `tools/go.sh test ./...`（`cmd/pool-game` 208.214 秒）與 `vet ./...` 通過；
  可重生收據在 `docs/audit/portrait-frame-parity.json`，兩張更新後繁中畫面已入版控。
