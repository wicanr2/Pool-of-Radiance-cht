...
// （spec 056）：+0 combatant 索引、+1 成本、+2 攻擊該目標所需的朝向。
...
// FootprintOffset 是佔格偏移表的一組偏移。X 對應 combatant record +0，
// Y 對應 +1。
...
func FootprintOffsets(class uint8) ([]FootprintOffset, error) {
	...
	for slot := 0; slot < FootprintSlots; slot++ {
		...
		offsets = append(offsets, FootprintOffset{X: a, Y: int8(row[slot*2+1])})
	}
	return offsets, nil
}

// FootprintCell 是展開到基準座標之後的一格。原版以「X 的最高位元被設起」表示無效，
// 且無效時不會寫 Y，因此這裡也只用 X 判定。
type FootprintCell struct {
	X uint8
	Y uint8
}

// Valid 重現原版的 `cmp ...,0 / jl`：X 以有號位元組看為負即無效。
// 這同時涵蓋「表已終止」與「加總後越過 127」兩種情形，與原版一致。
func (cell FootprintCell) Valid() bool { return int8(cell.X) >= 0 }
...
func FootprintCells(class uint8, baseX, baseY uint8) [FootprintSlots]FootprintCell {
	var cells [FootprintSlots]FootprintCell
	for slot := range cells {
		cells[slot] = FootprintCell{X: footprintInvalid, Y: footprintInvalid}
	}
	if class == 0 || int(class) > FootprintClassCount {
		return cells
	}
	...
	for slot := 0; slot < FootprintSlots; slot++ {
		...
		cells[slot] = FootprintCell{
			X: baseX + uint8(a),
			Y: baseY + uint8(int8(row[slot*2+1])),
		}
	}
	return cells
}
