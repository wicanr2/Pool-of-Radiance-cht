...
//
// 寫這八格的是 **overlay-23 entry 4**（`031Eh`），建角走到「這個角色是賊嗎」
// 那一步時由 overlay-16 `0CAFh` 呼叫。四種寫入版面在 38 顆 overlay 與
// START.EXE 裡掃過都零命中——那個零只證明「遊戲跑起來之後沒有人改它」，
// 因為 entry 4 寫的是 `es:[di+76h+i]`，位移是迴圈變數算出來的，
// 常數位移掃描結構上看不到（spec 095）。
const (
