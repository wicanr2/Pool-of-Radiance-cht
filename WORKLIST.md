...
- [ ] **市政廳外那一格的第二段，原版已經回到自由移動。** 原版走到 `(3,4)`
      分兩段：第一段等 Return，第二段換成公告那一句而**底下是指令列**
      （`AREA CAST VIEW ENCAMP SEARCH LOOK`）——訊息留在框裡，但事件已經結束、
      玩家可以直接走開。remake 第二段仍停在 `cellEventPending`，底下那一列被
      spec 132 的「ENTER 翻到議會公告」佔著，要再按一次 Return 才走得掉。
      **驗收**：讀那一格的 ECL 結構，讓第二段結束事件、指令列照畫；手冊提示
      改到不會擠掉指令列的位置。對照幀見
      `docs/audit/dos-parity-sample.md`〈市政廳外那一格〉。
