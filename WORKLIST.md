...
- [ ] **休息被打斷之後沒有跑 ECL 的紮營入口。** **打斷本身 2026-09-10 接上了**：那兩個參數是 ECL 變數（`+5A4h`／`+5A6h` 換算回 ECL 位址是 `6DD2h`／`6DD3h`），ECL1／2／3 共 117 個 `SAVE` 寫它們，`camp.go` 的 `restInterruption()` 從 `eventMachine.Memory` 讀，spec 114 已閉合。
      剩下的是打斷之後那一段：原版的順序是**打斷 → 回傳 1 → 呼叫端跑 ECL 的紮營入口**，貧民區的城衛隊（`YOU ARE ROUSTED BY THE CITY WATCH…`／`GO STAY`）在 ECL3／block 0 的 **entry 3**，而 remake 目前只跑 entry 0（每格）與 entry 1（搜尋）。
      **驗收**：打斷之後跑 ECL entry 3；走到貧民區紮營要看得到城衛隊那一問。
...
- [ ] **戰鬥中掛的效果不會寫回隊伍。** 戰鬥用的效果串列在 `tacticalState.Effects`，戰鬥結束時**沒有任何一處把它寫回** `state.Party[i].Effects`，所以打完架效果就消失了。原版的串列掛在角色記錄 `+7Fh`，是持久的——詛咒、疾病那一類本來就要跨戰鬥留著（神殿的九項服務就是拿掉它們的，spec 115）。存檔那一側現在只有三處會**移除**效果（解除魔法、地圖上施法、神殿服務），沒有任何一處加入。
      **卡在**：原版在戰鬥結束時對那條串列做什麼還沒讀——哪些效果留著、哪些跟著戰鬥消失。睡眠與定身這種只在戰術盤上有意義的，跟詛咒不會是同一種處置。
      **驗收**：把戰鬥結束時的處置讀出來寫進 spec，接上之後打完架身上該留的效果留得住，神殿那九項服務拿得掉。
...
- [ ] **平面圖的畫法與原版不同。** `A` 切出來的俯視圖，原版畫的是**灰色實心區塊**，remake 畫的是**青色線條輪廓**。2026-09-10 把這一張納入對拍時量到整張 74.34%、外框 97.58%——十八項裡最低的一項，差距主要就來自這個畫法。
      **卡在**：原版那一張怎麼畫的還沒讀：實心塊代表的是通道還是牆、一格幾個像素、哪些格子算「已探索」都要對回原版。
      **驗收**：把畫法對回原版並寫進 spec，對拍那一項的整張比例要跟著上去。
