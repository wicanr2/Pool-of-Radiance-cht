...
- [ ] 解出第一張地圖的移動遮罩、第一人稱背景／視錐與第一個玩家事件。正常 Begin
  ...
  **同狀態 DOS 畫面已經對拍了**（2026-09-05）：`tools/capture-dos-adventure.sh`
  ...
  沒對上的還有三項：視野的透視尺度（remake 的牆片畫在 `viewLeft + 欄 × 16` 再
  放大兩倍，背景卻填 `StageInset{88×88}`，所以牆片縮在框底一條，原版那一框的
  透視是滿的）、兩側水面與柱子的配色、以及右半版面（原版是隊伍面板加狀態列，
  remake 是除錯文字）。
...
- [ ] **走得到的內容量到了**（2026-09-03）：六個種子各走 6000 步，走到
  ...
  **下一步因此是把地點選單挑得更完整**：跨趟共用只讓每一格多輪到幾個選項，
  樞紐圖上一格有四五個地點時仍然輪不完。要嘛加趟數（時間換覆蓋），
  要嘛讓探索器記住「這一格還有哪幾項沒選過」。
