...
- [x] **二十五槽的完成條件都讀出來了**（2026-09-03，spec 041）。把每個 `FEh`
  ...
  **這一支上線的第一次就抓到一個玩不下去的缺陷**：`36h ADD NPC` 沒有把職業
  ...
  第二種帶法另外撞出一個開放缺陷：探索器在 `GEO8/29` 的井選單答了四千次出不來。
  原版那一段已經讀完（地形碼 4／7／18／19 走第 3 支，`9A81h` 用 `4A10h` 擋，
  選 YES 會把它清掉，照設計走不出無限迴圈），而**回報的座標 (7,7) 地形碼是 1，
  根本不是井格**——線索指向「印出來的位置與 ECL 正在處理的那一格對不起來」，
  不是井的腳本。細節在 spec 041 的〈開放缺陷〉。
