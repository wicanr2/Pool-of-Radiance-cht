...
- [x] **有目的地走完整個世界**（2026-09-03 新增量測）。隨機走路量的是「不會
  ...
  量到的數字：**393 格、2 張圖、4 個 ECL block（0、8、11、21）**。
  兩張圖是**訓練所那一區**（GEO3/0，171 格）與**索寇要塞**（GEO4/21，222 格）
  ——碼頭那一格的文字寫著「唯一的船是去索寇要塞的」，走過去就上船。
  **一趟走不完**：第一趟在起點附近就踩到碼頭，之後困在要塞回不來，
  起始圖 226 格只走了 31 格。所以改成**逐趟走**：把已知的換圖點記下來，
  下一趟先擋掉它，等這一張踩完了再走。八趟收斂，253 → 393 格。
  全遊戲有 29 個有文字的 block，**其餘 25 個走不到的原因是城區之間的移動
  還沒接上**，不是走路壞掉。
  起始圖上有四格會換圖（(4,4)、(6,1)、(6,2)、(9,0)，
  `cmd/pool-initial-cell-sweep` 掃得出來），四格都走得到。
  起始圖還有 55 格是幾何上走得到、實際踩不到的——**還沒查**。
  這一輪順手挖出三個會讓遊戲停住的東西：
