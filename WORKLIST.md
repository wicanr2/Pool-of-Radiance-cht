...
- [ ] **走得到的內容量到了**（2026-09-03）：六個種子各走 6000 步，走到
  ...
  **還缺 9 個**：`[3 4 5 6 7 9 13 15 28]`。

  | 缺的 | 路 |
  |---|---|
  | 28 | 同一條野外鏈，還沒擲到含 (3,32) 的那組位移 |
  | 13 | 東野外圖 27 的 (6,15) |
  | 15 | 走出 GEO8/29 的**南**邊界（表 `@AFCA` 朝向 2 → 封存檔 2 區塊 15）|
  ...
  **但第二段（9 → 6）不是走邊界。** GEO2/9 的兩個北緣格 (4,0) 與 (11,0)
  **一踏上去就換到區塊 2**，所以 `ecl2/9` 入口 0 那條「`C04C == 0` 就
  `NEWECL 6`」永遠輪不到——那條要 `6DD5 != 0`（踏出去的那一步），而踏出去
  之前就被換走了。區塊 6 真正的入口是 **`ecl2/9 A558h` 的 `ENCOUNTER MENU`**：
  `A57E` 的 `ON GOTO` 有一支走到 `A58E`，把隊伍擺到 GEO (4,15) 朝北再
  `A5A0 NEWECL 6`——**是遭遇的結果，不是走出去的**。下一步要查的是那個
  `ON GOTO` 的哪一支、以及那場遭遇怎麼觸發。

  試過但**沒有效果、已經退回**的一招：把「先回頭踩已知的換圖點」與「挑一個
  用得最少的出口」對調順序（前者會用 `planToCells` 直接走上舊的換圖格，繞過
  `avoid`，所以 (15,4) 才會被重複用）。對調之後區塊數一樣是 19、時間 154 秒
  對 183 秒——沒有換到覆蓋，卻動到「拿到船票之後回頭再用碼頭那一格」那條
  刻意的行為，所以不留。
