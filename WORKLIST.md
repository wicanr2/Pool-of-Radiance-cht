...
- [ ] 解出第一張地圖的移動遮罩、第一人稱背景／視錐與第一個玩家事件。
  ...
  **這一格還開著是因為底下累積的下游線還沒收**——市議會、神殿、戰利品、
  訓練那幾條各自有自己的未閉合點，寫在下面。
  正常 Begin
  ...
  **對拍表清空了：兩張都逐格 100%**（2026-09-06，spec 126）。剩下的那
  ...
  原版**用什麼機制**不畫 `0Dh` 還沒讀到：`Put8x8Symbol` 在 `[bp+8]` 非零時
  走 overlay-36 entry 8（`08F5h`），那一支用 `[bp+0Eh] & 1` 開遮罩模式並看
  圖片記錄 `+13h`／`+15h` 的遠指標——遮罩色沒讀出來，`0Dh` 是像素反推的。
...
- [ ] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **六十七支全部接得出來了**（2026-09-05，最後一支是臭雲術）。這一項仍然
  開著的只剩**瞄準那一層的 Manual 格子游標**——`352Ch` 的結構已讀出來
  （spec 127，八個方向鍵、確定／取消、`013D:0061h` 都讀出來了），
  只剩迴圈頭那一個旗標的極性——靜態讀不出來，要實跑或讀 Turbo Pascal 的 RTL。
