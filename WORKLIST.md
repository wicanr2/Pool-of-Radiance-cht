...
- [ ] **走得到的內容量到了**（2026-09-03）：六個種子各走 6000 步，走到
  ...
  **「跨出去再跨回來換一座迷宮」行不通**，這一輪試過了：往東與往西的推移大小
  ...
  所以下一步不是再調走法，是**把三條航線各自的位移鏈量完**，再對
  `workplace/wildreach` 的表問「(10,9) 與 (3,32) 在哪一條航線下走得到」。
  另外 EAST 那一趟實際踩到五個地點，比 (15,14) 那組位移預測的三格多——
  路上還有別的東西在動位移（地點腳本會直接寫 `49C3`／`49C4`），那也要量。
