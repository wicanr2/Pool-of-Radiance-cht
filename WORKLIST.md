...
- [ ] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **雲是四格的 2×2**（雲心、東、東南、南），先前寫的「五格多一格西南」是把
  ...
  | 法術 | 還缺什麼 |
  |---|---|
  | 迷霧術（34，`1AF6h`）／臭雲術（`22h`）| **站在雲裡的人會怎樣**。候選縮到三處了（`workplace/cmpscan` 把 38 支 overlay 裡「和 `1Eh` 做位元組比較」的指令全掃出來，整包只有 8 處，五處可以排除）：**overlay-24 `0B11h`**（`cmp byte [bp-1], 1Eh`，命中之後推 `1Eh` 呼叫 `010A:00A7`；overlay-24 正是套用傷害與效果那一支，最像答案）、overlay-22 `1C93h`、overlay-09 `073Ah`。排除的五處裡最容易誤認的是 overlay-12 `2E07h`——那裡的 `1Eh` 是 `Roll(1,100)` 的**三成機率**（命中就清掉魅惑 `0Bh` 與催眠 `35h`），不是地形碼。每回合誰來呼叫、`1Fh` 那一組物件是什麼仍未讀。詳見 spec 121 |
