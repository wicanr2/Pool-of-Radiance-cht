...
- [ ] **法術效果的派發表 67 格只讀了 42 個編號。** `SpellIsImplemented`
      是前端的閘（法術一覽的 `*`），選得到的都施得出來；但剩下那些原版有、
      remake 直接不給選。spec 073 的 57..67 十一格連來源都還是 DRAFT。
      **驗收**：每多讀一支就把編號加進 `SpellIsImplemented` 並補單元測試，
      spec 073 的狀態從 DRAFT 收成 CONFORMED。
- [ ] **地圖上沒有怪物群。** `encounter.go` 自承 remake 沒有遊蕩遭遇，
      目前的戰鬥全部掛在格子事件底下。**驗收**：照原版的遭遇表與觸發條件接，
      並在戰術盤面上以 `encounterStartDistance` 的距離開場。
...
- [ ] **休息不會被打斷。** `camp.go` 自承 `Stop Resting?  The Party is rudely
      interrupted!` 還沒接。**驗收**：休息中依原版的機率與遭遇表中斷，
      並接回戰鬥流程。
...
人物資料頁（spec 130，`sheet` 整張 85.16% → 86.63%）。
