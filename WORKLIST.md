...
- [ ] **探索器測試的玩家策略層：休息／治療／訓練。** 策略層已有：包紮、催眠、集火、旅店休息並睡到早上、城衛隊問句照安全選項答、交件集中獎金、訓練所、隊伍 pool 買甲。兩條主線探針停在索寇要塞 (8,5) 全滅；house rule seed 136..150 只有 143 打贏諾里斯；三條探索器測試卡在走路迴圈（2026-09-17 補九）。
      **驗收**：貧民窟不因睡不成而死（屋內或旅店挑近的；白金用完明確停下）；探針依 #26 順序多拿等級再量索寇 (8,5)；三條探索器測試回綠或改成記錄停在哪；多 seed 結局表。
      **討論**：[#22](https://github.com/wicanr2/Pool-of-Radiance-cht/issues/22)
- [ ] **主線路線表補委任獎賞、建議等級與順序。** 委任只給錢不給經驗值（`ecl3/8 9EC2h`，exact）。建議順序第 1、2 段與進城辦事的時刻已實跑（seed 143：市政廳 `49C9 >= 14` 鎖門，spec 102），第 3、5 段沒跑；第 4 段的索寇要塞 (8,5) 全滅。
