...
- [ ] **走得到的內容量到了**（2026-09-03）：六個種子各走 6000 步，走到
  ...
  **走得到的變 27 個**，還缺 `[17 28]`：野外 26 的 (12,11)（游牧民營地，
  ...
  **所以機率不是原因。而實測的結果是：野外 26 走 14032 步，一次洞穴都沒有。**
  ...
  `RANDOM` 的值域也查了：共用 engine 的 `0x08` 是 `Intn(bound)` 而
  `bound = maximum + 1`（`eclvm/machine.go`），也就是 **`RANDOM n` 回 0..n
  含端點**。所以 `RANDOM 20 == 20` 成立得了，機率 1/21；配上
  `RANDOM 19 == 0` 的 1/20，洞穴是 **1/420 每步**。
  一萬四千步的期望值是 33 次，實際 0 次——**這個落差目前沒有解釋**，
  而且不是機率、不是閘門、不是值域。

  下一步只剩一條路：在野外 26 走一步時把 `Result.RandomValues` 印出來，
  看 `RANDOM 19` 到底有沒有回過 0。回過就往下追第二擲，沒回過就是
  `RandomStream` 在那張圖上的狀態有問題。
