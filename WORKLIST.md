...
- [ ] **密碼提示只涵蓋一種模式**（2026-09-04）。問密碼時把答案附在問句後面
  已經上線，答案取自原版資料而不是寫死的表：`eclInputAnswer` 從 `INPUT STRING`
  往後掃，找 `03h COMPARE <剛寫進去的位址> <內嵌字面>`。ecl7/23 的密碼門
  （`A4CA COMPARE 6E79h "NOKNOK"`）已生效。

  索寇要塞那三處還沒接，因為原版寫法不同——答案先存進字串變數再比對：

      9E80 INPUT STRING #7 982Ch
      9E8D SAVE #6(內嵌) 9890h      ← SAMOSUD／SHESTNI 依 4A26h 二選一
      9EA6 COMPARE 982Ch 9890h      ← 比的是變數，不是字面

  要接的話，掃描得順便記下 `09h SAVE <字面> → <位址>`，再讓 `COMPARE` 的第二
  個運算元查那張表；`9890h` 被寫兩次，兩個候選都要列出來（玩家當下需要哪一個
  靜態分不出）。
