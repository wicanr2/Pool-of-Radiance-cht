...
- [ ] **戰場部署與走位對回原版（spec 053／057／061）。** 部署照原版換掉並以 dosgolem 四筆同狀態收據（單人隊、五人隊哥布林、獸人的家、衛兵攔截、驚動衛兵）逐格對上（spec 061 CONFORMED）；遭遇距離走法補上（spec 078）；foeTurn 的非原版備案拿掉、照 spec 096 走。剩下：敵方走位「誰先動」要同骰流才對得出同一條路、GEO4 block 21 重跑確認、TestSokalKeep 變慢的成因。
      **驗收**：敵方走位在同骰流下對上原版（或把先攻／挑目標的骰流讀出來重播）；GEO4 block 21 那一場不再隔牆；#32 關閉。
