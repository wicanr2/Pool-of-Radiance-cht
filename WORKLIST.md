...
- [ ] `29h ENCOUNTER MENU`：只用按鍵走出去，會經過渡船抵達索寇要塞
  ...
  **前置條件**：逃跑／逼近判定要隊伍的移動力，而 remake 的角色記錄沒有這個
  欄位。基礎值與兩條下修規則已由 spec 079 讀出來（基礎 12、盔甲看重量分
  12／9／6 且魔法盔甲 +3、負重超過 512／768／1024 分別壓到 9／6／3），
  但原版出貨的七個預設人物 `+11Ch` 推不出來，那個欄位很可能是「當下剩餘」
  而不是「速率」——先別拿它當驗收目標。
