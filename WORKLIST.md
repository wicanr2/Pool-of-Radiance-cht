...
- [ ] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **施展接上了七支**（2026-09-03）：祝福術、治療輕傷、燃燒之手、魔法飛彈、
  電擊術、火球術、閃電束，各自的算法逐條標了 overlay-22 的位址，除了魔法
  飛彈都與說明書逐字相符。戰鬥中按 C 開清單、Enter 施出，**只列得出讀過
  處理常式的法術**，沒讀過的不列也不會安靜地什麼都不做。
  選目標還是 remake 自己的作法（傷害打繞得過去的最近敵人、治療打自己），
  原版的 `View Aim` 還沒讀。
  還缺：其餘四十幾支處理常式（共用機制已解出並寫成 spec 098：擲骰是 `Roll(顆數, 面數)`、
