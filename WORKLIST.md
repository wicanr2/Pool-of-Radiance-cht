...
- [ ] **remake 的法術清單還沒有實拍。** 版面已經照原版量的排好（spec 134，
      逐行由單元測試釘住），但截圖流程建的是戰士，走到那一頁只會得到
      「沒有記憶法術」。**驗收**：截圖腳本改成建施法職業、用 `K` 記一條、
      紮營休息讓它生效，拍到 `pool-remake-chinese-spell-page.png`。
      原版那一側的基準幀已經有了（`workplace/dosgolem-ref-spells/57-m.idx`）。
