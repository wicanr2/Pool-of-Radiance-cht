...
清單其餘部分幾乎都打勾了，而自評只有 60～70%——兩者不矛盾：打勾的是「規則解出來、
接進去、有測試」，沒打勾的是下面三層。**壓住分數的主要是第一層**：那幾條是玩家
玩得到、而原版有、remake 沒有的東西；第二層是「接了但沒拿原版當裁判驗過」；
第三層是版面與資料的差距。
...
> 就是那樣（派發那一格早就接上，條目卻還寫著「派發表六十七格裡只剩這一
> 支」）。所以每一項都掛一個 `verify`：**跑 `-mode verify` 為真代表這一條
> 仍然未完成**，為假就是該回頭改了。多數條目綁在程式碼的自承註解上，
> 註解一旦被拿掉，verify 就會開口。
>
> 沒有機器可判訊號的那幾條標成 `manual`，verify 一律回「仍未完成」並標出來
> ——**沉默不等於通過**，那幾條要人自己去看。

**哪些畫面還沒對拍**是另一份清單，在
[`docs/audit/dos-parity-sample.md`](docs/audit/dos-parity-sample.md)
的〈還沒進抽樣的〉——對拍的權威只有那一份，這裡不複製。

歷次核實抓到的過期自承：2026-09-08 三處（`spells.go` 說「施展還沒接」與
「remake 還沒有紮營畫面」、`cast.go` 說「還沒有瞄準那一層」）；2026-09-10
一處（臭雲術的條目說「缺盤面上的雲團物件」，而那整套早就接上了——當天改寫成
真正的缺口「雲不會散」，同日補完後移出清單）。

<!-- worklist:begin 這一段由 `cmd/pool-worklist -mode render -write WORKLIST.md` 產生，不要手改 -->

### 一、玩家會撞到的功能缺口

- [ ] **走路時沒有人擲遭遇。** `encounter.go` 自承 remake 沒有遊蕩遭遇，目前的戰鬥全部掛在格子事件底下。**這一條有數字**：起始地圖 GEO3／0 的 1024 個「座標×朝向」樣本各跑一次格子生命週期，沒有任何一格走到 `24h COMBAT`（`docs/audit/pool-initial-cell-sweep.json`）。\n\n**2026-09-11 查清楚了一半**：貧民窟的遭遇**排得出來**——由那一區的入口 3（紮營被打斷）跑，隊伍強度決定數量與分幾群，怪物編號從腳本自己的五張三格表查（spec 136，`slum_encounter_test.go` 實跑排出三群）。所以缺的不是「遭遇表沒接」，是**走路時沒有人去擲那一次**：入口 0 從頭到尾不碰 `24h COMBAT`。
      
      **原版走路確實會遇到怪（2026-09-11 實跑）**：dosgolem 從 `(0,4)` 往西走進貧民窟，第二步就撞上 `YOU HAVE SURPRISED A PARTY OF GOBLINS.`，畫面在 `docs/reference/original-dos/slums/`。而且 `GOBLINS.` 正是入口 3 那三支 `ON GOTO` 的第二支——**走路的遭遇跑的是同一段**，入口 3 是這一區的遭遇入口，不是紮營專用。所以 remake 只差引擎那一側的「走一步擲一次」。
      **卡在**：擲骰在引擎那一側，還沒找到。**已經排除四條**（spec 136 的表）：ECL 入口 0、overlay-03 `312Ah`（那是紮營畫面那條路，`00ACh` 是 overlay-15 不是 overlay-20）、休息的 `6DD2h`／`6DD3h`、以及 `6DC7h`／`6DC8h`（戰後結果碼）。下一步掃移動那一側：overlay-07 更新座標之後有沒有擲骰再呼叫入口 3。
      **驗收**：把走路那一次擲骰的條件與參數讀出來寫進 spec 136，接上之後**走幾步就要遇得到**——拿 remake 走同一條路線（進貧民窟往前走），要在合理步數內排出遭遇並停在遭遇選單，然後才是戰鬥。
- [ ] **賊技能沒有產生端。** 記錄 `+77h` 起八格（spec 095）在建角與升級都沒有產生端。**非賊角色的 0 是正確答案不是佔位**（spec 095 自己註明），缺的是賊角色那一份。\n\n**2026-09-11 收窄到一個加項**：原版建角**真的會算**——dosgolem 建了兩個一級賊（矮人與人類），值分別是 `35 45 40 15 10 10 75 0` 與 `35 35 25 15 10 10 85 0`。基礎表與種族修正表都在 `START.EXE` 的 `B424h` 起（DS `3C74h`），**種族那七列與 AD&D 逐格相同、索引就是 `種族碼 − 1`**。\n\n扣掉基礎與種族之後，兩個樣本的差額**一模一樣**：`+5 +10 +5 0 0 0 0 0`。兩者的 DEX 不同（11 與 13）、種族不同，所以那不是敏捷也不是種族；另外還有一道**下限夾到 0**（矮人的讀語言 0 − 5 在實測裡是 0）。
      **卡在**：那個 `+5 +10 +5` 的出處還沒找到，而且實測值本身不在 `START.EXE` 裡（整列與前三格都掃過，零命中）——是算出來的。下一步是找**寫 `+77h` 那八格的程式碼**：先前「四種寫入版面零命中」的掃描面對建角那條路可能有洞（`+77h` 是 disp8，八格連續也可能是 `rep movsb`），要換形狀重掃並先做正對照。再建幾個樣本也能收窄——同種族不同 DEX 的兩個賊可以直接判定敏捷有沒有參與。
      **驗收**：把公式（基礎表 ＋ 種族修正 ＋ 那個還沒解出來的加項 ＋ 夾限）寫進 spec 095 並移出 DRAFT，再讓建角與升級算得出來，存檔欄位不再是常數 0。**驗收要用原版樣本對**：至少三個不同種族的一級賊逐格相同。
- [ ] **能量吸取沒有來源（缺的是特殊攻擊那一批）。** 欠帳那對欄位（記錄 `+74h`／`+75h`）與還帳（恢復術走 `gamepack.Restore`）**都接好了**，spec 097 是 CONFORMED、`21C4h..2286h` 逐行讀完了。缺的是**誰來扣**：原版由怪物的特殊攻擊呼叫 `21C4h`，而 remake 還沒有特殊攻擊那一批。所以 `DrainedLevels` 一律是 0——**空的是正確答案不是佔位**（沒欠帳時原版的 `2C16h` 也是直接返回），程式裡的註解已經這樣寫。
      **卡在**：特殊攻擊的分派還沒讀。線索在 spec 065：「第一形態空的那六隻怪物，牠們的第一形態是特殊攻擊」——所以怪物記錄的兩個攻擊槽裡，空骰子的那一格代表走特殊攻擊，但走去哪一支、怎麼挑，都還沒定位。
      **驗收**：把特殊攻擊的分派讀出來寫進 spec 並標 READY，接上之後吸取扣得動等級，復原走神殿那條路。
- [ ] **效果代碼的處理常式只接了一個。** 到期那一刻的入口**已經接上了**：原版 `01B8h` 呼叫的 `0100h:002Ah` 就是 **overlay-24 entry 2**（`0100h` 是 overlay-24 的 stub 段，`(offset − 20h) ÷ 5` 換回進入點，spec 109／112）——`+4` 非 0 的節點先用**模式 1** 叫一次自己代碼的處理常式，再摘掉。remake 這一側是 `(*app).expiredEffectTeardown`，戰場那一側是 `tacticalState.effectTeardown`，兩條路同一個入口。
      
      **缺的是代碼本身。** 原版填了 138 格（代碼 1..139，136 個落在 overlay-12、2 個在 overlay-13，spec 112），remake 有具名常數的只有十個，真正實作過收尾的只有 `28h`（雲團）。所以現在地圖上到期就只是摘掉節點——**這對已實作的代碼是對的**，對還沒實作的代碼則是「還沒接」與「不必接」分不出來。
      **卡在**：138 個代碼不是一次做完的工作，要按玩家碰得到的順序挑。挑之前缺一份盤點：哪些代碼玩家在這一版走得到、哪些已經由別的機制涵蓋（例如神殿那九項服務直接改`Effects`，不走處理常式）。
      **驗收**：先產出代碼覆蓋盤點（原版 138 格 vs remake 實作），再按玩家可見度接。每接一個代碼要有走玩家路徑的測試：掛上、時間過去、狀態真的回復。

### 二、驗證缺口：接了，但沒拿原版當裁判驗過

- [ ] **戰鬥數值對原版。** 敵方 AI 已換成原版的接近規則，命中、傷害、豁免與存活**沒有在同一場戰鬥裡對過原版**。
      **驗收**：同一組人物、同一場遭遇、固定骰子種子，逐回合比 THAC0、傷害骰結果與雙方 HP 變化；不一致的逐條記進 spec，不留「大致相符」。
...
- [ ] **五十八份規格沒有任何測試指回它。** `docs/audit/doc-index.json` 的 `specs_without_tests` 有 58 份。實作那一欄剛剛清完（每一份要嘛有檔案的註解指回它，要嘛在檔頭寫明實作不是 Go），測試這一欄還沒。\n\n**這不等於沒測**：測試可能存在，只是註解裡沒寫 `spec NNN`，那條線就接不起來。所以逐份要分三種處置——補註解（測試已經在測它）、補測試（真的沒測）、或在檔頭寫明為什麼不需要（純 RE 文件、實作在別的 repo）。
...
第一人稱框與對話框不再互相切（spec 132，`intro` 整張 78.50% → 79.65%）、
**挑法術那一頁**（spec 134，實拍逐列與原版相同：四條繩索橫條、標題、清單、
底下三行、指令列在框外；截圖流程改建人類牧師才走得到那一頁）、
**文字框的停頓點**（spec 082，`11h`／`12h` 都只是往框裡寫字，要玩家按一下的
是腳本自己放的**單選項** `HORIZONTAL MENU`；那一個選項原版不畫，改畫
overlay-03 `118Ah` 的 `PRESS <ENTER>/<RETURN> TO CONTINUE`。走路的入口 0 與
SearchLocation 共用同一個判準）、
**紮營**（spec 135，視野換成營火 `PIC<區號>.DAX` 區塊 29 的兩張動畫、時鐘行
接上「紮營中」、整棵選單樹：`CAMP`／`REST`／`ALTER`／`ORDER`／`SPEED`／`PICS`／
