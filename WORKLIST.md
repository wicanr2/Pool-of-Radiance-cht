...
- [ ] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **記憶已經接上**（2026-09-03）：可記憶數在記錄 `+0B1h + 組 × 3 + 等級`，
  ...
  他們的記憶陣列也全部落在上限之內。畫面掛在法術一覽上（1-6 挑人、
  M 記憶、F 忘掉）——原版的入口在紮營選單，remake 還沒有紮營畫面。
  還缺：**施展**（共用機制已解出並寫成 spec 098：擲骰是 `Roll(顆數, 面數)`、
  ...
  `(等級+1) ÷ 2`；第 1 級照碼算出來是 0 發 0 傷害。
  兩個出口已經關掉：`08BCh` 沒有下限（傷害 0 就整段跳過），引數順序也沒讀反
  （Fireball 是 `Roll(等級, 6)`，與說明書的「1-6 點×施術者等級」逐字相符）。
  **決定性的實驗**：原版拿第 1 級法師施魔法飛彈看傷害是不是 0。
  在那之前不接施法）、紮營畫面、
  法師的法術書（哪些法術記得起來，現在不限制）、
  記憶陣列第 7 位旗標的語意（overlay-15 `0000h` 只處理設了第 7 位的格子，
  但預設人物一個都沒設而陣列是滿的——兩件事同時成立，所以
  「第 7 位＝已記憶」講不通）、參數表另外十個欄位、參數表 `+6` 的 1..3、
