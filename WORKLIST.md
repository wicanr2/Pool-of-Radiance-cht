...
- [x] **二十五槽的完成條件都讀出來了**（2026-09-03，spec 041）。把每個 `FEh`
  ...
  **這一支上線的第一次就抓到一個玩不下去的缺陷**：`36h ADD NPC` 沒有把職業
  ...
  剩下的槽還沒被探索器走到。試過把成果帶到下一趟：帶 `FEh` 更差（3→1 條），
  帶「交差完」的狀態確實把世界打開（地圖 13→22 張、block 13→19 個）**但委任
  還是三條**——剩下的條件要密碼、湊物品或挑特定選單，不是走得到就解得開。
  第二種帶法另外撞出一個開放缺陷：探索器在 `GEO8/29` 的選單答了四千次出不來。
  原版那一段已經讀完，量到的值（`4A10h = 0`、`PC = $AF71`）**排除了井那一條**
  ——VM 是從另一個呼叫端 `AE83h` 進來的，而那個選單從來沒被答成「是」。
  順帶修掉一個真的 bug：`consumeInitialTransitionResources` 看到
  `WaitingForMenu` 就整個返回，**把同一個 result 裡發生在選單之前的資源事件
  丟掉**（`AEAFh` 正是「先 `LOAD FILES #32` 換地圖再問問題」）。修完迴圈仍在，
  所以那不是唯一成因。下一步該量什麼寫在 spec 041 的〈開放缺陷〉。
