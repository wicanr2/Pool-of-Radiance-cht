...
- [x] 豁免判定：全遊戲只有 overlay-24 entry 7（`0D61h`）一支。自然 1 必敗、
  ...
  spec 075。
- [ ] 法術的記憶與施展。四個容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  spec 072 格數，含建角寫下的第 1 級一格）。效果串列的掛上／拿掉也已閉合
  （spec 069），派發表、參數表與豁免判定已閉合（spec 073／074／075）。
  還缺：記憶陣列第 7 位旗標的語意（overlay-22 取名時濾掉它，但沒有任何
  overlay 直接存取 `+1Fh`，兩輪位元組搜尋與一輪反組譯文字搜尋都沒中）、
  參數表另外十個欄位的語意、參數表 `+6` 的 1..3 各代表哪一種處置，以及
  編號 57..67 這 11 個無名效果從哪裡進來。
- [ ] 昇級流程本身：門檻有了，HP（`+32h`／`+11Bh`）與 THAC0 的重算還沒讀完。
...
- [ ] **ECL opcode 的待辦剩 16 條、253 個呼叫點**，盤點在
  ...
  **還缺**：挑人的 UI（overlay-25 entry 42）、NPC 記錄從哪個封存檔載入
  ...
  待辦從 16 條 253 處降到 **0**。
- [ ] **人多的戰鬥會卡死，成因是暫定的移動規則不是原版的**。
  六個種子的隨機走查裡有兩個卡住；把卡住的那一場量出來：
  **41 隻敵人對 6 人隊伍，第 915 回合，兩邊都是滿血（360 對 775），
  相鄰對數 0**，敵方回合的回報是 `FOE 27 CLOSED 0 STEPS ON 5`。
  也就是說沒有人碰得到任何人，而不是打得慢。
  這**不是漏接收尾規則**：原版的回合收尾同樣沒有「兩邊都還在」的出口
  （overlay-08 `0955h` 只在「我方還在、敵方清光」時問 `Continue Battle:`，
  spec 062 已閉合）。成因是 remake 的移動是「朝目標走一格，走不進去就試
  其餘七個方向」——盤面一擠，雙方互相擋住就再也靠不近。
  **修法是讀 overlay-09 的敵方 AI**（下一條），不是自己補一個出口。
  骨架已由 spec 096 讀出來：每回合是一條「依序試各種行為，誰先動就停」的鏈，
  每隻怪物身上帶一個戰術模式（runtime `+15h`，1..6，四分之一的回合重擲）。
  關鍵差別在**接近的做法**——原版（overlay-31 entry 6）是「先列出目標周圍
  可站的格子再挑」，remake 現在是「朝目標的方向走一格」。人一多時會不會
  擋死，分水嶺就在這裡。

  已排除的成因（量過，不要再查）：
  - **不是收尾規則漏接**——原版同樣沒有「兩邊都還在」的出口（spec 062）。
  - **不是佈陣把雙方擺到牆的兩側就算了**：開場最近的一對相距 10 格，
    直線追蹤 `complete=false`，但那一格八個方向有七個走得進去。
  - **不是戰場產生錯了**：盤面上那些斜向的牆是**原版的斜投影**
    （spec 060，地城格 X 也吃 dy 的位移），不是列距算錯造成的剪切。
    寫入與讀取都用 `TacticalRowStride = 32h`，兩邊一致。
  - **不是方向常數混用**：`WallDirection*` 是 0／2／6，`geometry.Grid.Wall`
    收的正是 0／2／4／6，與 spec 076 那個 0..3 的坑不同。

  量到的現場：41 隻敵人對 6 人隊伍，第 915 回合，兩邊滿血、相鄰對數 0。
  也就是**在一張斜向、牆很多的盤面上，兩邊都用「朝目標走一格」的規則時
  會互相擋死**。
