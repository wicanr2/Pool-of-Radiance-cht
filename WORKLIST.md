...
- [ ] **休息要挑地方：街上睡不成，駕駛需要旅店（`4A07`）或清出來的屋子。** 入口 2 接上（#24）之後 `6DD2h`／`6DD3h` 有真值：城區 1／101、貧民窟街上 24／24、屋內 0／0。回一點生命力要連續睡滿 24 小時，所以街上實際睡不了，而貧民窟被打斷是直接排一場隨機遭遇（`ecl2/20 9A49h`）。原版只有屋內或 `4A07 != 0`（hypothesis：旅店）睡得成。
      **卡在**：駕駛怎麼挑地方睡歸 #22；`TestQuickCombatEarnsExperience` 的治具也掛在這一條。
      **驗收**：`4A07` 的 writer 讀出來並定名；駕駛能走到睡得成的地方把隊伍補滿；spec 114 最後那一段的 hypothesis 改成定論。
