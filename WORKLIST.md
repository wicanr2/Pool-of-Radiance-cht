...
- [ ] **作弊選單（鎖 HP、一擊斃命）與諾里斯那一場的探針。** 使用者 2026-09-17 決定：F1 說明頁列出作弊選單的按鍵，可開關鎖 HP 與一擊斃命；house rule 探針在諾里斯那一場打開、打完關掉，之後照正常強度跑到卡住。照 spec 140 前例預設關、畫面標示、存檔記 CheatsUsed，不算 #5 的收據。
      **驗收**：spec 141 READY；作弊開關與 CheatsUsed 存讀檔；關著時既有測試與對拍不變；一擊斃命蓋住盤點出的隊伍傷害路徑；seed 136..150 的諾里斯與卡住位置表寫進 playtest 與 #43。
      **討論**：[#43](https://github.com/wicanr2/Pool-of-Radiance-cht/issues/43)

### 二、驗證缺口：接了，但沒拿原版當裁判驗過

- [ ] **主線從開場到結局連續跑過一次。** 路線層（spec 137）已用明確的狀態機從標題按到結局。自然建角的兩條探針（原版規則、house rule seed 143）都走到索寇要塞 (8,5) 那一場 50 隻全滅（2026-09-17，`docs/playtest/mainline-end-to-end.md` 補九）。
      **卡在**：索寇要塞 (8,5) 避不掉（#37），要的是等級與戰術（#22）。
...
- [ ] **探索器測試的玩家策略層：休息／治療／訓練。** 策略層已有：包紮、催眠、集火、貧民窟就近進屋休息（只挑不會開打的房間）或旅店、睡到早上、城衛隊問句照安全選項答、交件集中獎金、訓練所、隊伍 pool 買甲；睡不成而有人倒地就在原地報原因，索寇裡睡不成帶著現狀往下打。house rule seed 136..475：諾里斯約 15% 勝率，19 個走到索寇 (8,5) 全數全滅（A 二級、其餘一級）；三條探索器測試卡在走路迴圈（2026-09-17 補十一）。
      **驗收**：貧民窟不因睡不成而死（屋內或旅店挑近的；付不出房錢明確停下）；探針依 #26 順序多拿等級再量索寇 (8,5)；三條探索器測試回綠或改成記錄停在哪；多 seed 結局表。
      **討論**：[#22](https://github.com/wicanr2/Pool-of-Radiance-cht/issues/22)
- [ ] **主線路線表補委任獎賞、建議等級與順序。** 使用者選路線 (a)：貧民窟 25 場交件 → 職員列出波多廣場 → 拍賣喬裝、WAIT FOR WINNER 結案 → 升級 → 諾里斯 → 索寇。鎖血診斷（#40，playtest 補十二）實跑過貧民窟 25 場、交件、職員列出波多廣場、拍賣結案不開戰；之後的交件與港務長被 #41 擋住。索寇 (8,5) 正常強度 19 個 seed 全滅（補十一）。
      **驗收**：spec 137〈建議順序〉照 (a) 改寫並以 #40 的實跑數字取代 hypothesis；拍賣結案有實跑收據；正常強度下貧民窟 24／33／34 的勝率另記。
...
- [x] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **恢復術也接上了**（2026-09-05）：欠帳那對欄位（記錄 `+74h`／`+75h`）
  ...
  施法時呼叫既有的 `gamepack.Restore` 還一級。**2026-09-13 已接上來源**：
  `MONnSPC` 的 `55h`／`56h` 在怪物命中後吸取一級／兩級，恢復術也會把職業
  等級與 HP 一起還回。接得出來的法術變成 66 格。
