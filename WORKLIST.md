...
- [x] 上冊轉錄的結構清點（2026-09-04 機械複驗）：`## p.N` 涵蓋 p.1–54 無斷號，
  ...
  頁層級標題，不是 `#### 7.`，內容沒有漏。
- [ ] 兩冊第二輪覆核：逐條回對掃描原頁，確認沒有轉錄漏字。
  **三處標記的疑義已經解掉**（2026-09-04，回對 `workplace/manual-scan` 的原頁）：
  - 上冊 p.23（`Pic0016.jpg` 右半）：那個字是 **搯**（扌＋舀），不是掏——
    右半是「爫 蓋 臼」，沒有掏的 勹 包住 缶。兩者同義，原書印的是異體字。
  - 下冊 p.16（`Pic0043.jpg` 左半）：`THAC0 19`、`HP 9` 都對，維持原判讀。
  - 下冊 p.22（`Pic0046.jpg` 左半）：`THAC0 16` 對，維持原判讀。
  **還缺**：逐行回對整整兩冊。68 張掃描裡只有 25 張有 OCR
  （`docs/audit/manual-scan-manifest.json`），所以「整頁漏掉」這一類還沒有
  機械化的檢查；目前擋著的是結構計數（線索報導 1–58、傳言 1–23、
  公告字號 18 則、附錄七節）。
...
- [ ] **探索器到不了野外**。主線那條路本身通了（`harbour_walk_test.go` 從走進
  ...
  **2026-09-04 回答了「隊伍為什麼還離得開城區」**：出口選擇確實關了
  （`leaving := hops < exploreMaxTransitionHops && !settling`，鎖住時
  `chooseAreaExit` 與 `chooseTransitionCell` 都進不去）。離開是**走路踩到的**
  ——探索器在城區逐格踩沒踩過的格子，踩到換圖格就換圖了，而換圖不是在按下
  前進那一 tick 發生，是格子事件裡的 `LOAD FILES` 做掉的。

  鎖住時把當前圖的換圖點加進 `avoid` 那一段擋不住：`avoid` 收的是
  `transitionUses` 裡**已經踩過**的點，第一次遇到的換圖格一定踩得到。

  可行的修法是不要靠「踩過才知道」——`cmd/pool-world-graph` 已經量得出每一張
  GEO 邊界上「往外沒有牆」的格子，鎖住時把那些格從目標清單裡濾掉。
  這是治具的覆蓋策略，不是玩家可見功能；世界巡迴（`TestWorldTour…`）已經
  走得到野外（GEO6/25），所以那些區域本身有被跑過。
