...
- [ ] **戰場部署與走位對回原版（spec 053／057／061）。** 部署已照原版換掉（spec 061 的 1A99h／1609h、spec 078 的遭遇距離走法），dosgolem 只拍到單人隊一筆收據；六人隊的陣型、敵方三回合走位、GEO4 block 21 那一場的對照還沒做。
      **驗收**：dosgolem 六人隊在獸人家（或走得到的固定事件）的 5E85h／6039h 收據與 remake 同狀態一致；敵方走三回合的位置對照；spec 061 狀態行改 CONFORMED。
