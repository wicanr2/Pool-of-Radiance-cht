...
清單其餘部分幾乎都打勾了，而自評只有 65～72%——兩者不矛盾：打勾的是「規則解出來、
...
真正的缺口「雲不會散」，同日補完後移出清單）；2026-09-11 五處（`camp.go` 檔頭
說「**還沒接**：休息被打斷」，而打斷、ECL 入口與那兩個參數的來源都已經接上；
README 的缺口清單留著四條做完的——休息被打斷、遠程武器的射程、結局的圖、
說明書附錄的規則表）。

**這一批有共同的形狀**：都是**檔頭或總覽段落**的自承，而缺口本身是在**別的
檔案、別的函式**裡補完的。補完的人改的是那一支，沒有人回頭看總覽。所以核實
要從總覽往程式走，不是反過來。
...
- [ ] **賊技能升級不會長。** 記錄 `+77h` 起八格（spec 095）的**建角那一端已經接上**：算式是 overlay-23 entry 4（`031Eh`）的 `i = 1..8` 迴圈——`記錄[+76h+i] = 基礎[等級×8+i] + 種族[種族碼×8+i]`，`i ≤ 5` 再加 `敏捷[DEX×5+i]`，種族修正為負且絕對值大於基礎就夾到 0；建角那一刻記錄裡的 DEX 還是 0（能力值要到 overlay-16 `1206h` 之後才寫進去），所以敏捷套的是第 0 列 `+5 +10 +5 0 0`。三個原版樣本逐格吻合。
      
      **沒接的是升級／訓練那一端**：賊在訓練所升到二級，八格不會跟著長。原版那時候記錄裡的敏捷是有值的，如果訓練也走 entry 4，算出來的技能會含真正的敏捷修正——與建角當下不一致。**先查清楚再實作，不要拿建角的行為直接推廣。**
      **驗收**：查出訓練所升級時誰重算那八格、用的敏捷是不是記錄裡的真值，寫進 spec 095 並接上實作。**驗收要用原版樣本對**：拿一個敏捷明顯偏離 13 的賊（例如 DEX 18，敏捷那列是 `+10 +15 +5 +10 +10`）在 dosgolem 裡練到二級，比對升級前後的八格。
