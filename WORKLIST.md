...
- [ ] **主線路線表補委任獎賞、建議等級與順序。** 使用者選路線 (a)：貧民窟 25 場交件 → 職員列出波多廣場 → 拍賣喬裝、WAIT FOR WINNER 結案 → 升級 → 諾里斯 → 索寇（HP 鎖定診斷通關在 #40）。波多廣場的拍賣要先接委任（exact 靜態）；索寇 (8,5) 正常強度 19 個 seed 全滅（補十一）。
...
- [ ] **HP 鎖定診斷通關：路線 (a) 從標題按鍵跑到結局。** 使用者 2026-09-17 決定以 HP 鎖定強制通關，驗證路線 (a)、旗標、換圖與結局鏈。依 CLAUDE.md §3／§6 這是診斷收據，不取代 #5 的正常強度收據；HP 鎖定只在測試治具、每次寫回都計數，不注入旗標座標金錢、不 forced-win、不改 internal/combat。
