...
- [ ] **賊技能升級不會長。** 記錄 `+77h` 起八格（spec 095）的**建角那一端已經接上**：算式是 overlay-23 entry 4（`031Eh`）的 `i = 1..8` 迴圈——`記錄[+76h+i] = 基礎[等級×8+i] + 種族[種族碼×8+i]`，`i ≤ 5` 再加 `敏捷[DEX×5+i]`，種族修正為負且絕對值大於基礎就夾到 0；建角那一刻記錄裡的 DEX 還是 0（能力值要到 overlay-16 `1206h` 之後才寫進去），所以敏捷套的是第 0 列 `+5 +10 +5 0 0`。三個原版樣本逐格吻合。
      ...
      **討論**：[#2](https://github.com/wicanr2/Pool-of-Radiance-cht/issues/2)
- [ ] **能量吸取沒有來源（缺的是特殊攻擊那一批）。** 欠帳那對欄位（記錄 `+74h`／`+75h`）與還帳（恢復術走 `gamepack.Restore`）**都接好了**，spec 097 是 CONFORMED、`21C4h..2286h` 逐行讀完了。缺的是**誰來扣**：原版由怪物的特殊攻擊呼叫 `21C4h`，而 remake 還沒有特殊攻擊那一批。所以 `DrainedLevels` 一律是 0——**空的是正確答案不是佔位**（沒欠帳時原版的 `2C16h` 也是直接返回），程式裡的註解已經這樣寫。
      ...
      **討論**：[#3](https://github.com/wicanr2/Pool-of-Radiance-cht/issues/3)
- [ ] **效果代碼的處理常式只接了一個。** 到期那一刻的入口**已經接上了**：原版 `01B8h` 呼叫的 `0100h:002Ah` 就是 **overlay-24 entry 2**（`0100h` 是 overlay-24 的 stub 段，`(offset − 20h) ÷ 5` 換回進入點，spec 109／112）——`+4` 非 0 的節點先用**模式 1** 叫一次自己代碼的處理常式，再摘掉。remake 這一側是 `(*app).expiredEffectTeardown`，戰場那一側是 `tacticalState.effectTeardown`，兩條路同一個入口。
      ...
      **討論**：[#12](https://github.com/wicanr2/Pool-of-Radiance-cht/issues/12)
- [ ] **每一局的隨機流都一樣。** ECL VM 的 `RANDOM` seed 在三個建立點都硬寫成 1（`internal/gamepack/ecl_catalog.go`、`intro.go` 兩處），遊戲本體也是。所以**同一條路線每一局遇到的怪完全相同**——走路遭遇的測試連跑五次，第 0 步擲中、三群的編號與數量一個位元都不差（spec 136）。
      
      原版的 seed 來自 DOS 的計時器 tick，每一局不同。固定 seed 對拍與測試需要（spec 050 的「DOS 同 seed 戰鬥對拍」），正常遊玩不該有。
      **驗收**：正常遊玩用時間 seed、對拍與測試沿用可指定的固定 seed；同一段路線連玩兩次要遇到不同的怪，而既有測試不受影響。
      **討論**：[#15](https://github.com/wicanr2/Pool-of-Radiance-cht/issues/15)
...
- [ ] **戰鬥數值對原版。** 敵方 AI 已換成原版的接近規則，命中、傷害、豁免與存活**沒有在同一場戰鬥裡對過原版**。
      ...
      **討論**：[#4](https://github.com/wicanr2/Pool-of-Radiance-cht/issues/4)
