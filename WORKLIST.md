...
- [ ] **`DS:6DD5h`：擋住整個世界的那一個變數**（spec 100）。貧民窟
  ...
  **還擋著兩件事**：九個逐鍵重現的測試（固定種子的路線要重新確立）；
  以及貧民窟的隨機遭遇——`ecl2` block 20 `B1B0h` 起的 `LOAD MONSTER` 三個
  運算元都是記憶體參照（`6E80h`／`9808h`／`6E7Bh`／`6E7Ah`），那幾個值在
  remake 這邊是 0，於是排出「怪物 0、數量 0」然後失敗即關閉。
