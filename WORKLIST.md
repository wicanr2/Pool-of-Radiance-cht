...
- [ ] **`12h PRINTCLEAR` 在 remake 還是停頓點，原版只在 `00h EXIT` 停。**
      症狀是走到市政廳外要按兩次 Return 才看到第二段，原版一次。
      照原版改過一次（把 `12h` 也變成不停頓），結果交任務、船運那幾條長腳本
      整段跑掉，六個玩家路徑測試當場紅——所以原版一定有辦法在同一格裡分辨
      「這一頁要不要等」，而那個判準還沒讀出來。
      **驗收**：讀出那個判準，讓分頁的次數與原版一致。
      對照幀與已經對上的部分見 `docs/audit/dos-parity-sample.md`〈市政廳外那
      一格〉。
