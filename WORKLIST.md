...
- [ ] **打不到的敵人會讓戰鬥停在那裡**。實測（八千步隨機走查，三個種子裡
  ...
  另外兩個種子各走完八千步沒有任何硬失敗。
  **原版的回合收尾也沒有「兩邊都還在」的出口**：overlay-08 `0955h` 只在
  「我方還在、敵方清光」時問 `Continue Battle:`，其餘由存活數決定
  （spec 062 已閉合，`DS:4955h` 恆為 0）。所以這不是漏接一條收尾規則，
  可能是 remake 的暫定佈陣或地形產生讓雙方擺到互相到不了的位置。
  下一步是拿卡住的那一場對回原版的佈陣，而不是自己補一個出口。
