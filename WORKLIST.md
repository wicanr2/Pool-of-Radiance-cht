...
- [ ] 法術的記憶與施展。四個容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  （spec 069），派發表已閉合（spec 073）。還缺：記憶陣列第 7 位旗標的語意
  （overlay-22 取名時濾掉它，但沒有任何 overlay 直接存取 `+1Fh`，兩輪位元組
  搜尋與一輪反組譯文字搜尋都沒中）、53 支處理常式各自做什麼（說明書的說明已
  逐條接上遊戲，見 spec 068，但那是說明書寫的行為，不是反組譯出來的規則），
  以及編號 57..67 這 11 個無名效果從哪裡進來。
