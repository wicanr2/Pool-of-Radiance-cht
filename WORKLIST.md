...
- [ ] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **雲是四格的 2×2**（雲心、東、東南、南），先前寫的「五格多一格西南」是把
  ...
  | 法術 | 還缺什麼 |
  |---|---|
  | 迷霧術（34，`1AF6h`）／臭雲術（`22h`）| **站在雲裡的人會怎樣**。收雲那一支完全不碰任何人的記錄，所以傷害／豁免應該是由地形碼 `1Eh` 在別處觸發的，那一段還沒讀；每回合誰來呼叫、`1Fh` 那一組物件是什麼也還沒讀。盤面物件有了，但它對人還沒有作用 |
...
- [ ] **走得到的內容量到了**（2026-09-03）：六個種子各走 6000 步，走到
  ...
  **還缺 3 個**：`[13 17 28]`——都是野外換口袋的老問題（13 在東野外 (6,15)、
  ...
  **走得到的變 27 個**（後來 17 也走到了，是 28 個），還缺 `[17 28]`：野外 26 的 (12,11)（游牧民營地，
  ...
  **走得到的變 28 個**，只剩 `[28]`：西野外 25 的前哨站 (3,32)。
  ...
  位移 (15,13)），隨機區域的問句也認得了。卡的是**一場打不完的戰鬥**，
  而那是真正的缺陷，不是治具問題：

  ```
  卡住：block 25 在 [8 30]｜tactical=true preview=true combatActive=true
  文字 "Encounter: WILD BOAR ×5"
  戰術：回合 22526 行動者 5 提示 false 我方 4 敵方 5 狀態 "BLOCKED"
  ```

  **回合數衝到兩萬兩千還在跑。** 雙方都結束得了回合（`foeTurn` 最後一定
  `endTurn`），但誰也打不到誰：野豬的五個方向都 `MovementBlocked`、繞路備案
  也找不到路，隊伍這邊探索器的駕駛走滿 12 步就結束回合。於是**這一場永遠
  不會結束**。換種子沒用（11／3／7 都在同一格 (8,30) 停住），繞不繞開地點
  也沒用（繞開時 (12,31) 的四個鄰格全是地點，隊伍兩步就動不了）。

  這件事巡迴早就會報「戰術地圖卡住」的硬失敗，但先前只知道現象；
  現在有具體的一場可以重現（野外 25、野豬 ×5、狀態 BLOCKED）。
  **修它要動戰術地圖的收場判定**（雙方都無法接近時怎麼算），
  那是獨立的一塊，不該塞在覆蓋率這一條底下。區塊 28 就卡在它後面。
