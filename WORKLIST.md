...
- [ ] 法術的記憶與施展。四個容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  （spec 069）。還缺：記憶陣列第 7 位旗標的語意（overlay-22 取名時濾掉它，
  但沒有任何 overlay 直接存取 `+1Fh`，兩輪位元組搜尋與一輪反組譯文字搜尋都
  沒中）、施展路徑本身（已定位到 overlay-22 的五個掛效果呼叫點，
  見 spec 069「誰在掛效果」；參數語意未讀），以及 56 條法術各自的效果語意
  （說明書的說明已逐條接上遊戲，見 spec 068，但那是說明書寫的行為，
  不是反組譯出來的規則）。
