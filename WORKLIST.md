...
- [ ] **賊技能沒有產生端。** 記錄 `+77h` 起八格（spec 095）在建角與升級都沒有產生端。**非賊角色的 0 是正確答案不是佔位**，缺的是賊角色那一份。\n\n**2026-09-11 把算式挖出來了**：建角在 overlay-16 `0CA7h` 判「賊等級 > 0」之後呼叫 **overlay-23 entry 4**（`031Eh`），那是一個 `i = 1..8` 的迴圈——`記錄[+76h+i] = 基礎[等級×8+i] + 種族[種族碼×8+i]`，`i ≤ 5` 再加 `敏捷[DEX×5+i]`；種族修正為負且絕對值大於基礎就夾到 0。三張表都在 `START.EXE`（DS `3C6Bh`／`3CB3h`／`3CC6h`，最後一張**每列五格**），逐格與 AD&D 相同。
      **卡在**：照那支算出來的值與實測差 `+5 +10 +5 0 0 0 0 0`——三個樣本（矮人 DEX 11、人類 DEX 13 兩個）的差額一模一樣，所以不是敏捷也不是種族。**沒確認的那一步**：overlay-16 推給 entry 4 的遠指標是 `[bp-4]`，而同一支函式在別處用的角色記錄是 `[bp-27h]`，兩者是不是同一個還沒查。**不要拿差額去猜第四張表**：`+5 +10 +5` 恰好等於 AD&D 的 DEX 17 那一列，而樣本的 DEX 是 11 與 13——形狀相符但前提不成立。
