...
- [ ] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **力量那一組三支真的會改到角色了**：`1158h` 的「只往上調」寫成
  ...
  還缺五支：迷霧術（34）、死靈術（36）、解除魔法（41／46 共用一支）、
  恢復術（56，規則已由 spec 097 讀出、只差接施法那一側）。
...
- [ ] **原版的敵方回合讀到一半**（spec 096）。已經讀出來的：每回合的行為鏈、
  ...
  **entry 3 是用身上的魔法物品**（走記錄 `+C8h` 的物品串列、查物品型別表）。
  **戰術模式的意思也解出來了**：它不是「哪一支行為」，是「要往哪幾個方向
  試」——`方向 = (基準方向 + DS:[02ACh + 模式 × 5 + 步]) mod 8`，**步從 1
  數到 5**，每個模式五個偏移，用完就 `(模式 mod 6) + 1` 換下一個。
  六列兩兩成鏡像（先左／先右、另一組鏡像、一路往左掃／一路往右掃），
  那個對稱就是「步是 1-based」最強的佐證。**這正是 remake 缺的那條
  規則**（現在是對目標格做 BFS 挑最短路）。接上去之前還要確定基準方向
  由誰算。還缺 entry 2（施法）的內容。
