...
- [x] **有目的地走完整個世界**（2026-09-03 新增量測）。隨機走路量的是「不會
  ...
  **還沒補的**：戰鬥的生命值不會寫回隊伍，所以打完架隊伍永遠是滿血。
  原版打完之後的生命值處理還沒讀。
- [ ] **`DS:6DD5h`：擋住整個世界的那一個變數**（spec 100）。貧民窟
  ...
  **既有測試的帶法試過一輪**：不重挑種子，而是把它們留在起始區逐格走
  （量的是戰鬥、記憶法術、裝備，不是地理），並補上新走得到的畫面的處理
  （隊伍管理按 B、商店按 ESC、選單有 Exit 就走過去）。四個修好，
  **剩三個卡在同一個新問題**：逐格走會先碰到另一場架，而那一場久久分不出
  勝負——完全不還手的隊伍打到第 3629 回合，敵方一直「ATTACK MISSED」，
  把生命值從 40 降到 6 也一樣。那不是測試的問題，是**那一場敵人打不死人**。
  所以順序是：**先讓隨便走到的架都收得了尾，再接 `DS:6DD5h`**。
  兩支函式都留在程式碼裡（`setMapExitFlag`／`applyMapExitCommit`），
  接的時候把 `setMapExitFlag` 的呼叫放回 `moveInitialDungeonForward` 即可。
- [ ] **隨便走到的架要收得了尾**：接「走出這一區」的前置。目前量到的例子是
  完全不還手的隊伍打到第 3629 回合還沒輸，敵方一直沒打中。與先前修過的
  「敵方靠不過來」同一族——原版的敵方回合（overlay-09 entry 1）還沒讀。
...
- [ ] **清掉索寇要塞**：目前唯一擋住「走得到其餘 25 個 block」的東西，
  而且**不是缺功能，是缺進度**（spec 099）。
  碼頭的船就是旅行樞紐：`SOKAL / EAST / WEST / BAY / NONE` 是一條
  `2Bh HORIZONTAL MENU`（ecl3 block 0 `9CF0h`），選完依 `DS:4AC4h` 切
  archive 再 `NEWECL`。那張選單前面有一道閘門——`DS:4AA7h` 小於 254 就整段
  跳過，玩家只看得到「唯一的船是去索寇要塞的」。而**寫這個旗標的只有索寇
