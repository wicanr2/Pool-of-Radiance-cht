...
- [x] **有目的地走完整個世界**（2026-09-03 新增量測）。隨機走路量的是「不會
  ...
  量到的數字：**253 格、2 張圖（GEO3/0 與 GEO4/21）、3 個 ECL block**，
  走到沒有新格子可踩為止。全遊戲有 29 個有文字的 block，**其餘 26 個走不到
  的原因是城區之間的移動還沒接上**，不是走路壞掉——起始圖上只有四格會換圖
  （(4,4)、(6,1)、(6,2)、(9,0)，`cmd/pool-initial-cell-sweep` 掃得出來），
  四格都走得到，而它們只通到一個目的地。
  這一輪順手挖出三個會讓遊戲停住的東西：
