...
- [ ] **賊技能沒有產生端。** 記錄 `+77h` 起八格（spec 095）在建角與升級都沒有產生端。**非賊角色的 0 是正確答案不是佔位**（spec 095 自己註明），缺的是賊角色那一份。
      **卡在**：spec 095 的 DRAFT：**原版沒有任何 overlay 寫這八格**，所以值從哪來還沒解出來。
      下一步不是反組譯而是實測——用 dosgolem 跑原版建一個賊，把存檔的 `+77h` 起八格讀出來比對規則書的表；值若跟著等級與種族動，產生端就在建角那條路上。
...
- [ ] **效果在地圖上到期時沒有跑收尾。** 效果串列已經會隨時間遞減了（走一步一分、休息一刻五分，`advanceGameTime` → `AdvanceEffects`，spec 069）。**缺的是到期那一刻該做什麼**：原版在 overlay-20 `01B8h` 呼叫 `0100:002A`，傳（角色遠指標、效果碼、節點遠指標），那一支還沒讀。remake 目前只把節點摘掉。
      
      戰場上有一份對應的收尾（`tacticalState.effectTeardown`，目前只處理代碼 `28h` 的雲），地圖上這一條路完全沒有。所以像變形、隱形這一類「結束時要把某個狀態改回去」的效果，時間到了會留下改過的狀態。
      **卡在**：`0100:002A` 是哪一支還沒定位——那是 `0100h` 那個 far call 表的 entry，要先對回 overlay 與 offset 才知道它做什麼。在那之前不要照猜的行為補，`effectTeardown` 那個 switch 現在只有一個代碼有來源。
      **驗收**：讀出 `0100:002A` 的內容寫進 spec 069，地圖上的到期走同一套收尾；至少一個代碼要有走玩家路徑的測試（掛上、時間過去、狀態真的回復）。
...
- [ ] **九支工具沒有測試。** `cmd/` 底下有九支工具一個測試都沒有（清單在 `docs/audit/doc-index.json` 的 `tools_without_tests`）。這不是形式問題——本輪 `pool-disp-scan` 的測試**一寫就抓到假陽性**：`0x90`（nop）的高兩位也是 `10`，被當成 modrm，於是掃描會憑空生出記憶體存取。產生證據的工具算錯，錯的是下游所有引用它的規格，而且錯得很安靜。 **而且這一條是 worklist 機制自己的單點失效**：`json_len`／`json_gap` 讀的是別的工具算出來的 JSON，上游算錯時 verify 會照樣印好消息。2026-09-10 用一個小場景驗過——盤點工具拿「目錄裡多於一個 .py」當「有測試」的代理指標，兩支沒測試的工具被判成有，清單變空，verify 宣告「可能已完成」；條目、verify、JSON 格式全都沒問題，錯的是上游。
      **驗收**：每一支工具至少釘住它自己的判讀規則（換算表、解析格式、分類邏輯），並且測試要有正反對照——訊號在時報命中、訊號拿掉時真的開口。`doc-index.json` 的 `tools_without_tests` 歸零。
- [ ] **三十份規格沒有任何檔案指回它。** `docs/audit/doc-index.json` 的 `specs_without_implementation` 有三十份。對應關係的主鍵是程式碼註解裡的 `spec NNN`，沒有反向引用就查不到「這份規格由誰實作」，只能靠關鍵字猜——而猜出來的對應寫進索引比空著更糟。**這不等於沒實作**：本輪查證的八份純 CONFORMED 裡，三份自己就指名了實作檔（001、005、064，已補），六份的實作在共用 engine（已另外分類）。剩下的要逐份查證。
      **卡在**：有些規格可能真的還沒實作，那要的是實作不是註解。逐份查證之前分不出來。
      **驗收**：每一份要嘛有檔案的註解指回它，要嘛在規格自己的檔頭寫明為什麼沒有（還沒實作／在別的 repo／純 RE 文件）。不准為了讓數字下降而加沒查證過的註解。
- [ ] **對拍跑兩次結果不一樣。** 2026-09-10 連跑三次同一個 AppImage 對同一組基準：**兩次走位失敗**（卡在市政廳那一段的 `await adventure-cell-menu`，停在 `adventure-move`），成功那兩次的截圖**逐像素比對有六張不同**。差異分兩組，各自跨三張同類畫面：狀態列右側那一格（邏輯座標 x=526..597、y=119..127，時鐘）差 284 個像素；人物資料頁主體（x=69..565、y=51..295，擲出來的數值與肖像）差 1896 個像素。所以 `dos-parity-sample.json` 的數字每次跑都會浮動十個像素上下——**小幅變動不代表回歸，也不代表沒有回歸**，那一欄現在分不出來。
      **卡在**：要固定的是兩件事：建角的擲值與肖像（現在跟著時間走）、以及畫面上的時鐘。前者可能要讓 AppImage 吃一個 seed 參數，後者要嘛不入鏡要嘛固定起始時刻——兩個都會動到發行包的介面，不是只改腳本。
...
- [ ] **紮營畫面的視野框差 676 格。** 對拍的視野欄（只比第一人稱那 88×88，把框外的中文排除掉）四張裡，紮營是唯一沒接近滿分的：**7068/7744＝91.27%**，另外三張是 100%／100%／99.42%。\n\n兩邊畫的是同一件事——視野框換成營火圖（原版 `63-e.png`，remake `remake-camp.png`）——所以差的不是版面，是那張圖怎麼畫出來的。整張那一欄看不到這件事：紮營 78.96%、市政廳 78.60%，差 0.36 個百分點。
      **卡在**：沒查過是色盤、縮放還是圖塊來源。三者都會產生「看起來很像但逐格不同」，而且會互相掩蓋——先分出是哪一類再動手。
      **驗收**：查出 676 格差在哪一類，修掉或寫下為什麼修不掉；視野欄要重跑一次對拍確認。
