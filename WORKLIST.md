...
- [ ] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **雲是四格的 2×2**（雲心、東、東南、南），先前寫的「五格多一格西南」是把
  ...
  | 法術 | 還缺什麼 |
  |---|---|
  | 迷霧術（34，`1AF6h`）／臭雲術（`22h`）| **雲對人的作用走的是效果代碼，不是收雲那一支。** 把 38 支 overlay 裡「和 `1Eh` 做位元組比較」的指令全掃出來（`workplace/cmpscan`），整包 8 處、五處可排除（overlay-32 四處是蓋／還原地形本身；overlay-12 `2E07h` 的 `1Eh` 是 `Roll(1,100)` 的**三成機率**，不是地形碼）。剩下兩處的形狀一樣，都是「這一格是雲 → 查身上有沒有某個效果」，而 `010Ah:00A7h` 就是 spec 069 那支效果串列搜尋：**overlay-24 `0B11h`** 查效果 `1Eh`、**overlay-09 `073Ah`** 查效果 `20h`（前面剛讀完格位類別表，所以在移動／接近的迴圈裡）。`1Eh` 在 spec 112 的分派表裡落在群組 11 與 15，**那兩組的呼叫端還沒讀**——下一步就是它們，加上 `013D:007F`。詳見 spec 121 |
