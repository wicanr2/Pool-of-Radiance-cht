...
- [ ] `29h ENCOUNTER MENU`：只用按鍵走出去，會經過渡船抵達索寇要塞
  ...
  adapter passthrough」。4000 步的隨機走查一次戰鬥都沒觸發。
  已讀出來的部分見 spec 078：運算元陣列的版面（型別 `6DCBh+i`、低位
  `6E4Bh+i`、高位 `6E0Bh+i`）、四個選項的字面
  （`~COMBAT ~WAIT ~FLEE ~PARLAY` 與 `~ADVANCE` 兩版）、運算元 1／2／3 是
  名稱索引與數量上限、運算元 4 是「結果寫進哪個 ECL 變數」、5..9 是用選擇
  當索引的五格結果表、13 與 14 是拿隊伍最慢／最快移動力去比的門檻，
  四個選項各自寫回哪個結果碼（0..3）也列成表了。remake 不必解讀那些碼的
  意義，寫對就好，分支由 ECL 自己做。**還缺**怪物數怎麼擲、運算元 10..12、
  以及決定第四項是 PARLAY 還是 ADVANCE 的那個條件。讀完之前維持硬失敗，
  不要先宣告成 passthrough——跳過一場遭遇與正確處理它在報表上分不出來。
  **前置條件**：逃跑／逼近判定要隊伍的移動力。整條規則已由 spec 079 閉合
  （基礎 12 → 盔甲依重量 12／9／6 且魔法盔甲 +3 → 負重減力量寬容後分段
  下壓，只往下不往上），七名預設人物的 `+11Ch` 逐一算得出來。
  **還缺**：remake 的角色記錄還沒有 `+72h`、`+102h` 與算出來的移動力，
  要讓 `29h` 能判定逃跑得先把這三個欄位放進存檔 schema。
