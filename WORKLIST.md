...
- [ ] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **另外二十五支是純泛型的**（2026-09-03）：只推「編號 ＋ 四個零」給
  ...
  25 格（護邪、防護、隱形、解鎖、致盲、詛咒…）。加上逐支讀的三十三支處理常式
  （36 個編號），六十七格裡接得出來的有 **61 格**
  ...
  **實作錯誤**：變大術寫進 `DS:47A6h` 的 `12h` 是十進位的 18（要設成的力量），
  ...
  與參數表 `+8`／`+9` 相同。
  還缺六支：迷霧術（34）、力量術（35，骰子看目標的職業，施法介面還沒把目標
  傳進來）、死靈術（36）、解除魔法（41／46 共用一支）、恢復術（56，規則已由
  spec 097 讀出、只差接施法那一側）。
