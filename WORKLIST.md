...
- [ ] **平面圖的畫法與原版不同。** `A` 切出來的俯視圖，原版畫的是**灰色實心區塊**，remake 畫的是**青色線條輪廓**。2026-09-10 把這一張納入對拍時量到整張 74.34%、外框 97.58%——十八項裡最低的一項，差距主要就來自這個畫法。 **2026-09-10 量出差距**：對拍新增的視野欄（扣掉框外的中文）顯示那一框只有 20/7744＝0.26% 相同——整張的 74.33% 是被框外的版面撐起來的。原版 88×88 裡深灰（可走）75.04%、淺灰（牆，兩像素厚）24.47%，整框填滿；remake 是黑底 83.97% 加 15.97% 的線，而且線色 `(170,255,255)` 不在 EGA 十六色裡。逐像素的表與格子週期記在 spec 119 的〈原版不是畫線，是填色〉。
      **卡在**：原版那一張怎麼畫的已經量出來了（spec 119），剩下的是格子與像素的對應：88 像素要放幾格、週期 8 的邊界怎麼對到 GEO 的 16×16、哪些格子算「已探索」。
