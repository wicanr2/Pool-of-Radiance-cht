...
- [ ] **戰士的多次攻擊還沒接。** 原本三件，兩件在 2026-09-10 接完了：**射程**（`tacticalState.AttackRange` 按格記，建 roster 時從武器型別的 `+0Ch` 減一填，`foeTurn` 拿它當「搆不搆得到」的預算）與**怪物的敏捷**（記錄 `+13h`，先攻修正讀它）。剩下戰士的多次攻擊（記錄 `+A1h`）：`tactical.go` 打完一次就 `endTurn`，接上之後要改成「打完所有攻擊次數才結束」。
      **卡在**：spec 051 的 DRAFT——「玩家角色的攻擊次數來源」還沒讀出來，照 SDD 不能猜著寫。
      另外**拿武器的怪物射程仍然不對**：原版走怪物記錄的 `+0CCh`，而 remake 的怪物只載 285-byte 記錄、沒有物品鏈，所以一律算成近戰一格（原版在 `+0CCh` 為 0 時也是 1，所以近戰怪物兩邊一致）。
      **驗收**：帶一條逐回合的單元測試，且與戰鬥數值對拍那一項同源。
...
- [ ] **抽樣擴到還沒對拍的畫面。** 現況表有十八項；原版也有、remake 也做了、但還沒進抽樣的清單在 [`docs/audit/dos-parity-sample.md`](docs/audit/dos-parity-sample.md) 的〈還沒進抽樣的〉——紮營那一整棵樹、商店、神殿、挑法術頁、裝備、檢視人物、地圖上施法、平面圖。
