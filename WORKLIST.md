...
- [ ] **紮營的 `SAVE`／`ALTER` 兩項還沒接。** 畫面已經照原版改好（spec 135），
      這兩項照原版列在指令列上，按下去給一句「還沒從原版讀出來」。
      另外**營火兩張怎麼交替還沒讀**，remake 只畫第 0 張。
      **驗收**：讀出 overlay-15 那兩項的分派與動畫時序，接上去。
...
**紮營畫面**（spec 135，視野換成營火 `PIC<區號>.DAX` 區塊 29、時鐘行接上
「紮營中」、兩層指令列；指令列的高亮改成「大寫的那個字母」，`daYs` 的鍵
才標得對）。
