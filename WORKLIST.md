...
- [ ] 解出第一張地圖的移動遮罩、第一人稱背景／視錐與第一個玩家事件。正常 Begin
  ...
  即 AD&D 的精靈武器加值。仍未閉合：記錄 `+0AAh` 的 producer。
...
- [ ] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **恢復術也接上了**（2026-09-05）：欠帳那對欄位（記錄 `+74h`／`+75h`）
  ...
  | 法術 | 原版做什麼 | 缺什麼 |
  |---|---|---|
  | 迷霧術（34，`1AF6h`）| 在盤面生一個五格的雲團物件，掛在 `DS:6CAFh` 的串列上，地形 `1Eh` 會與別人的雲合併。效果碼 `28h` 的處理常式在 overlay-12 `0CDEh`（2026-09-05 定位），節點 `+3` 的高四位是雲的序號，收尾那一路印 "The air clears a little..."；戰鬥結束由 overlay-08 `0000h` 把整條串列放掉 | 留在盤面上、每回合結算、彼此會合併的物件（`0DCE` 之後對站在雲裡的人做什麼、每回合誰呼叫、合併規則都還沒讀）|
...
- [ ] **走得到的內容量到了**（2026-09-03）：六個種子各走 6000 步，走到
  ...
  **2026-09-05 稍晚：世界巡迴改走三種主線狀態，量到 16 個。**
  ...
  地圖 19 張。門檻釘在 14（不頂到實測值，見那一條的註解）。

  **還缺 13 個**：`[1 2 3 4 5 6 7 9 10 13 15 19 28]`。其中 **9 個是 spec 055
  認定的自載區域腳本**（1、2、4、6、9、10、13、15、28 ——卡德納紡織廠、
  瓦海登墳場、曼多爾圖書館等），那幾個是「玩家該走得到的地方」；
