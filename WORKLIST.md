...
- [ ] **九支工具沒有測試。** `cmd/` 底下有九支工具一個測試都沒有（清單在 `docs/audit/doc-index.json` 的 `tools_without_tests`）。這不是形式問題——本輪 `pool-disp-scan` 的測試**一寫就抓到假陽性**：`0x90`（nop）的高兩位也是 `10`，被當成 modrm，於是掃描會憑空生出記憶體存取。產生證據的工具算錯，錯的是下游所有引用它的規格，而且錯得很安靜。
