...
- [ ] **戰場部署與走位對回原版（spec 053／057／061）。** 部署已照原版換掉並以 dosgolem 三筆同狀態收據（單人隊、五人隊哥布林、五人隊獸人的家二十五格）逐格對上（spec 061 CONFORMED）；遭遇距離走法補上（spec 078）。剩下：敵方走位同狀態對照要同骰流（原版挑目標擲骰，spec 096）、GEO4 block 21 重跑確認不再隔牆、TestSokalKeep 變慢的成因。
