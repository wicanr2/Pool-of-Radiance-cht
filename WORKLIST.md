...
- [ ] **主線路線表補委任獎賞、建議等級與順序。** 委任只給錢不給經驗值（exact）。索寇之前撐等級的兩條路都沒走通：波多廣場的拍賣要先接委任，而職員要貧民窟、索寇、書至少交過一條才列出它（spec 137，exact 靜態）；斯托揚諾河那條陸路 19 個自然隊伍零個走到金字塔（探索器不朝小船走）。索寇 (8,5) 19 個 seed 全滅（補十一）。
      **驗收**：建議順序第 3 段（斯托揚諾河、波多廣場隨機場）由探針實跑並記等級、XP、時刻、最大一場與結果；第 4 段索寇 (8,5) 用之後的等級再量；spec 137 拿掉「hypothesis，待探針實跑」。
