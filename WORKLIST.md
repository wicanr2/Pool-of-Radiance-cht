...
- [ ] **賊技能沒有產生端。** 記錄 `+77h` 起八格（spec 095）在建角與升級都沒有產生端。**非賊角色的 0 是正確答案不是佔位**（spec 095 自己註明），缺的是賊角色那一份。\n\n**2026-09-11 找到一半**：原版建角**真的會算**這八格——dosgolem 建一個矮人賊，寫出來的 `.CHA` 是 `35 45 40 15 10 10 75 0`。基礎表與兩張修正表也找到了，在 `START.EXE` 的 `B424h` 起（DS `3C74h`）：九級基礎、七列種族修正、六列敏捷修正。**缺的只剩公式**：三張表加起來湊不出實測值，差 `+5 +10 +5 0 0 0 0`，而讀語言那格的 −5 被夾到 0。
      **卡在**：公式還沒對上。種族修正表的列序、敏捷用哪一格當索引、夾限的上下界，**三件事只要猜錯一件就會在這個樣本上自洽而在別的角色上錯掉**。下一步是找讀那兩張表的程式碼：建角在哪顆 overlay（spec 003），就在那裡找對 `3C74h`／`3CBCh` 的讀取。也可以再建幾個不同種族／敏捷的賊當交叉樣本。
      **驗收**：把產生端寫進 spec 095 並移出 DRAFT，再讓建角與升級算得出來，存檔欄位不再是常數 0。
