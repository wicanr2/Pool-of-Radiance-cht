...
- [ ] **探索器測試的玩家策略層：休息／治療／訓練。** 策略層已有：包紮、催眠、集火、貧民窟就近進屋休息（只挑不會開打的房間）或旅店、睡到早上、城衛隊問句照安全選項答、交件集中獎金、訓練所、隊伍 pool 買甲；睡不成而有人倒地就在原地報原因。house rule seed 136..150：九個走到古托井、諾里斯 7 場贏 1 場、索寇 (8,5) 沒有 seed 走到；三條探索器測試卡在走路迴圈（2026-09-17 補十）。
...
- [ ] **主線路線表補委任獎賞、建議等級與順序。** 委任只給錢不給經驗值（exact）。第 3 段在索寇之前有陸路，但波多廣場的拍賣要先接委任，而職員要貧民窟、索寇、書至少交過一條才列出它（spec 137，exact 靜態）；斯托揚諾河沒實跑。第 4 段索寇 (8,5) 全滅。
