...
- [ ] **走得到的內容量到了**（2026-09-03）：六個種子各走 6000 步，走到
  ...
  **只剩 2 個**：`[13 28]`——都是野外換口袋的老問題（13 在東野外 (6,15)、
  28 在西野外 (3,32)），與城堡這條路無關。
  ...
  **這兩個又量細了一輪（`workplace/wildoffsets`）。** 把 256 種位移全跑一遍：
  ...
  - 重擲位移的唯一機制是**區域圖**（`ecl8/27 A20Dh`「YOU HAVE FOUND A SMALL,
    ...
    也就是全部在同一個進不去的口袋裡**。
  - 口袋裡另一個地點 (11,8) 是廢墟城堡（區塊 16、GEO8/30），進得去但
    **出不來**：`ecl8/16` 的入口 0 也是「地形 × 朝向」的表
    （`@9B00`／`@9B0C` 十二組），逐格踩挑不出來；用「逐格 × 四朝向」掃過
    十分鐘還沒掃完（那張圖的戰鬥很貴）。
  - 跨圖來回不換位移：實測 27 是 (6,4)、26 是 (10,4)、25 是 (15,14)，
    出去再回來會抵消（先前已記）。

  所以下一步很明確：**先讓廢墟城堡出得來**（同一個「地形 × 朝向」的教訓，
  見 spec 101 新增的那一節），量它出來之後的位移落在哪裡；那是目前唯一還沒
  試過的換位移路徑。
