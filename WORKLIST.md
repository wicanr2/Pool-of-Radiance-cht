...
- [ ] 昇級流程：**規則已解出並實作成程式庫（spec 097）**，還缺訓練所的畫面
  與把經驗值真的發下去。已閉合的部分：經驗值在記錄 `+0ACh`／`+0AEh`（32 bit，
  ...
  還缺：種族／能力值的等級上限那一段、訓練所自己的職業遮罩 `+550h` 從哪來、
  昇上法師等級之後的學新法術流程。
- [ ] **經驗值目前沒有發也沒有存**：`save.Character` 沒有經驗值欄位，
  隊伍永遠是第 1 級。要先加欄位（存檔 schema 要升）並在戰鬥結束時發下去，
  昇級才有東西可比。
