...
- [ ] **戰鬥的效果串列和隊伍的是兩份。** **問題的形狀 2026-09-11 查清楚了，和原本寫的不一樣。** 原版沒有「戰鬥用的串列」——overlay-24 entry 10（`0E54h`）掛節點是 `GetMem(9)` 接在**角色記錄 `+7Fh`** 串列的尾端（spec 112），所以戰鬥中掛的效果天然就在持久記錄上，怪物的則隨怪物記錄消失。remake 卻有兩份：戰術盤面的 `tacticalState.Effects []gamepack.EffectList`（完整的 9-byte 節點）與存檔的 `poolsave.Character.Effects []uint8`（只有效果碼）。
      
      **而且缺口是雙向的，原本只寫了一半。** `state.Effects = make(...)` 建的是全新的空串列，沒有從 `Party[i].Effects` 帶入——所以**帶著詛咒進戰鬥，那個詛咒在盤面上不存在、不生效**；戰鬥結束時也沒有任何一處寫回去。盤面槽對到隊伍第幾人的對應（`partySlot`）已經有了。
      **卡在**：存檔那份只存效果碼，建不回 9-byte 節點。**「永久」的表示法找到一半**（spec 069）：持續欄是 `0` 的節點不會被有限持續的新效果取代（overlay-24 `16D3h`），那正是詛咒要的行為。**但持續會不會每回合倒數還沒確定**——掃遞減點在 38 顆 overlay 零命中，而那個零不算數：正對照顯示 `dec WORD PTR` 這個形式整包都沒有，Borland 產的是「載入暫存器、遞減、寫回」，pattern 從一開始就抓不到。要換形狀重掃並先做正對照。在那之前 `+1` 同時相容於「剩餘回合數」與「強度／施法者等級」兩種讀法，不要挑一個來實作。
      **驗收**：把原版表示永久效果的方式讀出來寫進 spec 069，接上之後：帶著詛咒進戰鬥那個效果在盤面上生效、打完架身上該留的留得住、神殿那九項服務拿得掉。兩個方向各要一條走玩家路徑的測試。
