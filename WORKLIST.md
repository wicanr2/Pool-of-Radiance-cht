...
- [x] **第一人稱那一框已經改成對 dosgolem 比**（2026-09-07 當天收掉）。
      ...
      **交叉核對那一項留著沒刪**——兩個獨立 oracle 現在互相同意，
      ...
      它會先開口。
- [ ] **人物資料頁補齊原版的欄位。** 原版那一頁（dosgolem `16-Return`）有
      `LEVEL`／`EXP`、`AC`／`THAC0`／`ENCUMBRANCE`、`HP`／`DAMAGE`／`MOVEMENT`、
      `STATUS OKAY`，右上角還有肖像；remake 只有性別／種族／職業、年齡、
      六屬性、金幣與 HP。數值本身在別的畫面算得出來（裝備頁有 AC／THAC0／
      移動），是**這一頁沒顯示**。
      **驗收**：那八個欄位與肖像都上得了那一頁，且與裝備頁同源；
      `docs/audit/dos-parity-sample.json` 的 `sheet` 整張比例上升。
