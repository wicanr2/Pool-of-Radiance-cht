...
- [ ] **探索器測試的玩家策略層：休息／治療／訓練。** 策略層已有：包紮、催眠、集火、貧民窟就近進屋休息（只挑不會開打的房間）或旅店、睡到早上、城衛隊問句照安全選項答、交件集中獎金、訓練所、隊伍 pool 買甲；受傷會回頭補血（同位置＋同血量最多試三次，睡不成的地方不無限重試）。市政廳的四萬圈空轉已修：腳本踏上職員格 `SAVE 1 @4A01` 之後 `9BA8h` 就跳過逐槽掃描，`cityHallPlan` 的交差那一支因此要先看這個條件。現在每一趟走 109～161 步就結束，位置集中在 (5,5)／(4,6) 市政廳的 guarded door、(10,5) 神殿守衛、(10,8) 扒手——一級隊伍打不贏。house rule seed 136..475：諾里斯約 15% 勝率，19 個走到索寇 (8,5) 全數全滅（A 二級、其餘一級）。
      **驗收**：三條探索器測試與三條探針測試回綠，或把覆蓋斷言改成「記錄停在哪」並寫明它證明什麼（走得到哪、停在哪一步）；避開會開打的格子與先升級兩條路擇一並寫下理由；多 seed 結局表。
