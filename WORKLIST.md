...
這一節每一條都對 code 核實過，不是照抄上一輪的清單。核實時抓到三處已經過期的
自承（`spells.go` 說「施展還沒接」與「remake 還沒有紮營畫面」、`cast.go` 說
「還沒有瞄準那一層」），已改寫成現況。

### 一、玩家會撞到的功能缺口

- [ ] **臭雲術（`22h`）還沒接。** 派發表六十七格裡**只剩這一支**——其餘
      四十二支逐支讀過、二十五支走純泛型那條路，數字由
      `TestImplementedSpellCount` 釘住（42／25／67）。臭雲術不是「對目標下
      效果」，是在盤面上生一個雲團物件（19-byte 節點掛在 `DS:6CAFh` 的串列上，
      形狀是雲心加東、東南、南四格），remake 沒有這種「留在盤面上、每回合
      結算」的東西。**而且 overlay-12 `0CDEh` 從 `0DCEh` 起「雲對站在裡面的人
      做什麼」那一段還沒讀完**，所以照 SDD 現在還不能動手。
      **驗收**：先把那一段讀完寫進 spec 098 並標 READY，再接雲團物件；
      `TestImplementedSpellCount` 的三個數字跟著改。
- [ ] **地圖上沒有怪物群。** `encounter.go` 自承 remake 沒有遊蕩遭遇，
      目前的戰鬥全部掛在格子事件底下。**這一條現在有數字**：起始地圖
      GEO3／0 的 1024 個「座標×朝向」樣本各跑一次格子生命週期，
      沒有任何一格走到 `24h COMBAT`（`docs/audit/pool-initial-cell-sweep.json`，
      boundary 全部是 `exit` 或 `event`）。所以在斯倫特貧民區怎麼走都撞不到
      架——截圖腳本走四百步拍不到實戰畫面，是這條缺口造成的，不是腳本走錯。
      **驗收**：照原版的遭遇表與觸發條件接，並在戰術盤面上以
      `encounterStartDistance` 的距離開場；接上之後截圖腳本要拍得到
      `pool-remake-chinese-combat.png`。
- [ ] **賊技能沒有產生端。** 記錄 `+79h` 的找／解陷阱（spec 095）在
      `checkparty.go` 與 `internal/save` 都自承是 0。**驗收**：建角與升級
      時算得出來，存檔欄位不再是常數 0。
- [ ] **戰鬥還缺三件。** 戰士的多次攻擊（記錄 `+A1h`，spec 051）、長柄與
      投射武器的射程、記錄 `+13h` 那一項都還沒接進 `tactical.go`。
      **驗收**：三件各自帶一條逐回合的單元測試，且與戰鬥數值對拍那一項同源。
- [ ] **休息不會被打斷。** `camp.go` 自承 `Stop Resting?  The Party is rudely
      interrupted!` 還沒接。**這一條現在有實跑證據**：原版在貧民區排兩小時，
      第五分鐘就被城衛隊趕起來（`YOU ARE ROUSTED BY THE CITY WATCH…`／
      `GO STAY`），那一段是 ECL3／block 0 的 **entry 3**，不是 overlay-20 自己
      印的 `rudely interrupted`。順序是「打斷 → 回傳 1 → 呼叫端跑 ECL 的紮營
      入口」。**這也推翻了 spec 114 原本的推論**——先前說 `RestInterruption`
      用 0／0「不是近似而是還沒有哪一區設起來」，但貧民區明明會打斷，所以
      `+5A4h`／`+5A6h` 一定有第三個 writer。
      **驗收**：找到那個 writer，接上打斷判定，並讓打斷之後跑 ECL entry 3
      （remake 目前只跑 entry 0 與 1）；走到貧民區紮營要看得到城衛隊。
- [ ] **能量吸取沒有來源。** `internal/save/state.go` 的欠等級一律 0，
      `spell_cast.go` 說 overlay-12 `21C4h` 還沒接。**驗收**：吸取扣得動等級，
      復原走神殿那條路。
...
- [ ] **`TestPlayingTheWorldCompletesCommissionsOnItsOwn` 的門檻要重新定。**
      探索器現在走到 21 張地圖、19 個 ECL block、0 次硬失敗（先前是 15 張、
      16 個、15 次被靜靜吃掉的活鎖），但**委任從五條變四條**——少的是卡德納
      那一條（槽 18）。機制沒壞：`ecl4/2 A99C ON GOTO` 的
      `TAKE IT UNOPENED` 寫 `4AC8h=1`，`ecl3/0 AB17` 比對它才走到
      `AB8A SAVE 254 → 4AB8h`，而 `TestEveryCommissionHandsInAtCityHall`
      逐槽驗過交差鏈且是綠的。差在路線：先前探索器被活鎖困在城區反覆繞，
      反而更容易湊上「拿箱子」與「走到 (0,4) 的 gateway」這兩步。
      **驗收**：門檻改成能反映「玩家走得到多少內容」的組合（委任＋地圖＋
      block＋硬失敗數），或讓治具重新走得到那條鏈；不要單獨放寬委任數。
...
- [x] **第一人稱那一框已經改成對 dosgolem 比**（2026-09-07 當天收掉）。
      ...
      **交叉核對那一項留著沒刪**——兩個獨立 oracle 現在互相同意，
      ...
      它會先開口。
- [ ] **肖像編輯器補上人物資料。** 原版是在整張人物資料頁上換頭與身體
      （`24-Return`），remake 是一張只有三行字的獨立畫面。
      **驗收**：換頭／換身體時人物資料仍在畫面上。
- [ ] **戰鬥造形設計補上 OLD／NEW 對照。** 原版同時畫舊造形與新造形各一組
      `READY`／`ACTION`（`25-k`），玩家看得到自己改了什麼；remake 只畫一組。
      **驗收**：四格都在，且 OLD 那一組是進編輯器時的值。
- [ ] **遊戲內攻略擴到其餘地圖。** `F3` 的機制與資料格式做好了
      （[`docs/guide/README.md`](docs/guide/README.md)），目前只有 `3/00`
      費蘭的文明區建了 48 個點。**驗收**：每一張新地圖的點都由原始 GEO／ECL
      推出來、每一個點帶 `source`，且 `internal/guide` 的三個測試全綠——
      **不可以抄第三方攻略的座標**，那種錯的症狀是「測試綠、玩家走不到」。
- [ ] **抽樣擴到商店、神殿與結局。** dosgolem 目前走到第一場遭遇的戰鬥畫面；商店、
      神殿、結局都還沒走到。**驗收**：`tools/dosgolem-reference.sh` 的鍵序
      走到那三處各拍一張，`docs/audit/dos-parity-sample.json` 各多一項。
