...
- [x] 法術的參數表與效果訊息：67 個編號的行為差異全部來自 `DS:3196h` 的
  16-byte 記錄，共用施法常式 `08BCh` 用 `編號 × 16` 取它。`+0` 是 FFh 的正好
  是五個碰觸法術、`+6` 為 0 的不擲豁免（Magic Missile 與 Sleep 都在裡面）、
  `+8` 是效果碼且為零的正好是十三個不留狀態的法術。41 段效果訊息各自屬於哪
  一支常式也解出來了，每一段都驗過本體真的引用。契約見 spec 074。
- [ ] 法術的記憶與施展。四個容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  spec 072 格數，含建角寫下的第 1 級一格）。效果串列的掛上／拿掉也已閉合
  （spec 069），派發表與參數表已閉合（spec 073／074）。還缺：記憶陣列第 7 位
  旗標的語意（overlay-22 取名時濾掉它，但沒有任何 overlay 直接存取 `+1Fh`，
  兩輪位元組搜尋與一輪反組譯文字搜尋都沒中）、參數表另外十個欄位的語意、
  豁免類別 1..3 各是什麼（`0100h:0043h` 未讀），以及編號 57..67 這 11 個無名
  效果從哪裡進來。
