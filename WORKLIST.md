...
- [ ] **解病術這一類只作用在施法者身上。** `finishCast` 的 `RemoveEffects`
      分支沒有吃 `target`，而瞄準層（`beginCastTargeting`）已經在治療與復活
      那幾支用起來了。**驗收**：對隊友施得動，且對非隊員目標不會誤傷；
      加一條「對隊友解病」的單元測試。
...
- [ ] **能量吸取沒有來源。** `internal/save/state.go` 的欠等級一律 0，
      ...
      復原走神殿那條路。
- [ ] **結局的 `FINAL5.DAX` 圖沒接**（`program.go`）。破關那一場打得贏、
      過場文字也翻好了，只有圖是空的。**驗收**：結局頁面出得了圖。
...
人物資料頁的欄位、肖像編輯器、戰鬥造形設計的 OLD／NEW、遊戲內攻略擴到其餘地圖，
以及把抽樣擴到戰鬥畫面——五條連驗收條件列在〈發行〉一節底下，不在這裡複製一份。
