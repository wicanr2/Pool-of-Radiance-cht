...
- [ ] **平面圖的格數與原版不同。** **配色與填法 2026-09-10 對齊了**：深灰鋪可走區、淺灰畫牆、色盤從介面的 `(170,255,255)`（不在 EGA 十六色裡）換成 `artPalette`，對拍的視野欄因此從 **0.26% 拉到 67.59%**、整張從 74.33% 到 80.52%（spec 119 有逐像素的表）。剩下的差距是**格數**：原版一格 8 個 native 像素，88 的框只裝得下 11 格，而 remake 畫的是 16 格各 11 個邏輯像素。
      **卡在**：原版那 11 格對到 GEO 的哪 11 格還沒讀出來。唯一的樣本是隊伍箭頭落在格 (0,4)、而隊伍當時就在 (0,4)，看起來像從 (0,0) 起算的視窗——**一個樣本推不出規則**，隊伍走到地圖另一角時那個視窗會不會捲動要另外拍。
      **驗收**：把原版的格數與視窗規則讀出來寫進 spec 119，接上之後對拍的 `adventure-map` 視野欄要明顯高於 67.59%。
