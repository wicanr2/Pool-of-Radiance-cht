...
- [ ] **探索畫面的 `CAST` 與 `VIEW` 沒有分派。** 那一列六個鍵
      （`command_bar.go`）只接了 `A`／`S`／`L`，`E` 另外接在 `main.go`；
      `openCastMenu` 的唯一呼叫端在 `tactical.go`，所以**施法只在戰鬥中做得到**
      ——探索時補不了血、解不了毒。`VIEW`（人物資料頁）同樣沒有入口。
      **驗收**：兩個鍵在探索畫面按得動，`CAST` 走的是與戰鬥同一套
      `SpellIsImplemented` 閘；戰鬥外沒有目標格時的目標選擇照原版走隊伍清單。
...
戰術盤面的版面、人物資料頁的欄位、肖像編輯器、戰鬥造形設計的 OLD／NEW、
遊戲內攻略擴到其餘地圖，以及把抽樣擴到戰鬥畫面——六條連驗收條件列在〈發行〉
一節底下，不在這裡複製一份。
...
- [ ] **肖像編輯器補上人物資料。** 原版是在整張人物資料頁上換頭與身體
      ...
      **驗收**：換頭／換身體時人物資料仍在畫面上。
- [ ] **戰術盤面的版面對齊原版的戰鬥畫面。** 現在那一頁是 remake 自己的：
      標題「戰術戰場」、四行 `地城／格／阻擋／隊伍／敵方`、`回合／行動者／
      先攻／步數`，以及底下那一列 `H I M Q P O N G 移動 ENTER 結束回合 D 延後
      C 施法`，全部出自 `20-locale` 的 `ui.tactical*`（remake 自擬），
      **不是原版字串的譯文**。原版（說明書下冊 p.37）右側是選中對象的資訊欄
      （`"OGRE" G.R.X ／ HITPOINTS 29 ／ AC -2 ／ SILVER BROAD SWORD`），
      最下一列是 `MOVE VIEW AIM USE CAST TURN QUICK DONE` 八項指令。
      **驗收**：右側資訊欄與八項指令列都在，字串取自原版（進
      `internal/gametext`，不是 `ui.*`）；`docs/audit/dos-parity-sample.json`
      多一項戰鬥畫面的抽樣。這一條與「抽樣擴到戰鬥畫面」互為前後：
      沒有原版那一側的畫面，對不了。
