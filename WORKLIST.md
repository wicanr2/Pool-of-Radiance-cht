...
- [ ] **紮營的 `ALTER → ICON` 還沒接。** 整棵選單樹已經照原版做好（spec 135），
      只有這一項按下去給一句「還沒接」——remake 的戰鬥造形編輯器綁在建角
      流程上（`a.flow`），要對已經在隊伍裡的人開它，得先有「把角色的造形
      選擇載回 flow」那條反向路徑。
      **驗收**：`CAMP → ALTER → ICON` 能改目前角色的戰鬥造形。
      同一條上的兩個小缺口：`ALTER → SPEED` 的值存了也顯示了，但 remake 沒有
      逐字顯示所以還沒有作用；`ALTER → DROP` 的第三句
      ` Breathes A sigh of relief` 在哪個條件下印還沒讀到。
...
接上「紮營中」、整棵選單樹七層：`CAMP`／`REST`／`ALTER`／`ORDER`／`SPEED`／
`PICS` 與兩個問句；指令列的高亮改成「大寫的那個字母」，`daYs` 的鍵才標得對）。
