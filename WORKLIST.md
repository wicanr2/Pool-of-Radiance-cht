...
- [ ] 先完成《軟體世界》說明書中的 Journal／日誌全文翻譯，再進行遊戲接線。
  驗收：固定來源檔、頁碼與 SHA-256；逐頁轉錄並保留原文索引；建立集中術語表，統一
  Phlan 地名、人名、怪物、法術、物品與 AD&D 規則譯名；每一則 Journal 都有完整繁中、
  頁碼／條目 ID、疑義註記與覆蓋率報表，未翻譯數必須為 0。AI 翻譯完成後以原文段落數、
  標題、選項與交叉引用做機器一致性檢查；本階段先完成翻譯 corpus，不把尚未接進 UI
  誤寫成遊戲內中文化完成。
