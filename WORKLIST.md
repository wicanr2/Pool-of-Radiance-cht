...
- [ ] **走得到的內容量到了**（2026-09-03）：六個種子各走 6000 步，走到
  ...
  **所以機率不是原因。實測反而挖到另一件事：野外 26 的每步事件整個沒跑。**
  在野外 26 上實際移動 **14032 次**（不是按鍵數，是位置真的變了），
  結果是「**零個選單**」——不只沒有洞穴，連戰鬥遭遇都一次都沒有。
  同一支治具在野外 27 上，五百步內就跳兩次洞穴、遭遇不斷。

  這是下一輪的目標，而且它比 17／28 本身重要：**野外 26 走起來是空的**，
  表示那張圖的每步腳本沒有跑到，玩家在上面走不會遇到任何事。
  查法：在野外 26 走一步時把 `ecl7/26` 入口 1（`9AE0h`）的執行軌跡印出來，
  看它停在 `9AE0 COMPARE @4A9E 255`、`9AEB COMPARE @4AB3 254` 還是根本沒被呼叫。
