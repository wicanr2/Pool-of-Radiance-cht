...
- [ ] **HP 鎖定診斷通關：路線 (a) 從標題按鍵跑到結局。** 使用者 2026-09-17 決定以 HP 鎖定強制通關，驗證路線 (a)、旗標、換圖與結局鏈。依 CLAUDE.md §3／§6 這是診斷收據，不取代 #5 的正常強度收據。`TestMainlineProbeHPLockedRouteA`（seed 142）走到諾里斯，交件與港務長被 #41 擋住（playtest 補十二）。
      **卡在**：remake 載入 ECL 區塊時不清 `4A00..4A1F`（#41）。
