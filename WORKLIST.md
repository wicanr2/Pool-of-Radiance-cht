...
- [ ] **自訂規則（預設關）：委任獎賞折算經驗值。** 原版委任只給錢（spec 137 獎賞表），使用者要求可切換的 house rule：AD&D 一版 1 gp ＝ 1 XP，市政廳獎賞那一場空戰鬥的戰後結算發放。
      **驗收**：選項存在、預設關；開著時交件經驗值依金幣等值增加，關著時 0，各一條測試；探針多一條 house rule 開的收據。
      **討論**：[#28](https://github.com/wicanr2/Pool-of-Radiance-cht/issues/28)
