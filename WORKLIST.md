...
- [ ] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **流程在 overlay-15**（2026-09-03 定位到）：原版存取記憶陣列一律用
  ...
  還缺：overlay-15 的流程本身、記憶陣列第 7 位旗標的語意（overlay-15
  `0000h` 只處理設了第 7 位的格子，但預設人物一個都沒設而陣列是滿的——
  兩件事同時成立，所以「第 7 位＝已記憶」講不通）、參數表另外十個欄位、
  參數表 `+6` 的 1..3、編號 57..67 這 11 個無名效果從哪裡進來。
  **傷害不在參數表裡**：Magic Missile 與 Fireball 的 `+0Bh..+0Fh` 分不出
  傷害骰，那些在各自的處理常式裡，要逐支讀。
