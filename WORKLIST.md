...
- [ ] 解出第一張地圖的移動遮罩、第一人稱背景／視錐與第一個玩家事件。正常 Begin
  ...
  **`8X8D1.DAX` 區塊 203** 載一次（區塊 203 剛好 45 個 item，與 `01h..2Dh`
  ...
  對拍表上剩兩項：外框美術（原版是紅色繩索花紋），以及剩下的那百分之二三
  （`(0,4)` 拱底兩側 remake 多了兩塊粉紅、`(14,1)` 遠牆頂多一條洋紅）。
...
- [ ] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **恢復術也接上了**（2026-09-05）：欠帳那對欄位（記錄 `+74h`／`+75h`）
  ...
  不是佔位。接得出來的法術變成 66 格。
  還缺的兩支各自卡在一個 remake 還沒有的子系統，不是「再接一支就好」
  ...
  **雲是四格的 2×2**（雲心、東、東南、南），先前寫的「五格多一格西南」是把
  ...
  | 法術 | 還缺什麼 |
  |---|---|
  | 迷霧術（34，`1AF6h`）／臭雲術（`22h`）| **雲對人的作用走的是效果代碼，不是收雲那一支。** 把 38 支 overlay 裡「和 `1Eh` 做位元組比較」的指令全掃出來（`workplace/cmpscan`），整包 8 處、五處可排除（overlay-32 四處是蓋／還原地形本身；overlay-12 `2E07h` 的 `1Eh` 是 `Roll(1,100)` 的**三成機率**，不是地形碼）。剩下兩處的形狀一樣，都是「這一格是雲 → 查身上有沒有某個效果」，而 `010Ah:00A7h` 就是 spec 069 那支效果串列搜尋：**overlay-24 `0B11h`** 查效果 `1Eh`、**overlay-09 `073Ah`** 查效果 `20h`（前面剛讀完格位類別表，所以在移動／接近的迴圈裡）。`1Eh` 在 spec 112 的分派表裡落在群組 11 與 15，**兩組的呼叫端與 `013D:007F` 都讀完了**：群組 11 是**命中判定**（overlay-13 `1595h`、overlay-22 `09B2h`，後者 spec 074 已從法術那頭寫過），群組 15 是**輪到這個人行動**（overlay-08 `0289h` 回合開始、`065Eh` 建行動選單）；`013D:007F` 是 overlay-32 entry 19 `0CB9h`，把佔的四格合成一個地形碼，**雲一票否決**（任一格是雲就回雲）。**代碼 `1Eh` 的處理常式也讀出來了**：overlay-12 entry 29 `0A76h`，印「<名字> **is coughing**」、把 `+108h` 的 `+1`／`+2`（這一回合的行動權）清掉、AC 每次變差 2 點下限 AC 10。同組的 `15h` 是 `is silenced`，逐行同形。**還缺的只剩「誰把 `1Eh` 掛到人身上」**——掛效果的 `0100h:0052h` 全部 38 個呼叫點掃過，沒有一個推常數 `1Eh`，所以代碼是變數來的。詳見 spec 121 |
