...
- [ ] 解出第一張地圖的移動遮罩、第一人稱背景／視錐與第一個玩家事件。正常 Begin
  ...
  **bounded／wrapped 縮小了但還沒定案**（2026-09-06，spec 125）：
  找到寫 `@6DD5` 的人是 overlay-14 `06AEh`——走一步前清成 0，
  那一步會走出 16×16 就立成 1，同時**把座標夾在邊界**
  （`[4937h]+5AAh` 依 class 1 換算就是 `6DD5h`，spec 106）。
  **這也解掉了 spec 100 的 OPEN**（誰寫、寫什麼值）。
  另外確認「牆的判定確實是繞的」：`0131h:0039h` 自己把座標繞回 0..15 才查牆，
  那個慣用法在 overlay-30 出現六次、別處零次。**查詢繞、位置夾是兩件事。**
  還沒定案的原因：`06AEh` 只在越界時寫座標，沒越界的那一步是誰前進的還沒讀到；
  而且實測把 remake 改成夾之後野外那三張圖的貼圖會壞（靠的就是繞回去）。
  **在呼叫端讀出來之前不動行為。**
...
- [ ] 法術的記憶與施展。容器都就位了（spec 069 效果串列、spec 070 記憶陣列、
  ...
  **六十七支全部接得出來了**（2026-09-05，最後一支是臭雲術）。這一項仍然
  開著的是**瞄準與範圍那一層**，不是法術本身：`352Ch` 的格子游標（Manual）、
  火球術的 `DS:4933h → +1CCh` 分支、以及火球術以外的範圍法術的收人預算
  ——那三個沒讀出來之前，範圍法術先收整邊。
  ...
  **`DS:4933h → +1CCh` 那個分支也讀了**：它是 ECL 位址 `49E6h`（class 0 的
  ...
  另外還缺 `352Ch` 的格子游標（Manual），以及那 9 個呼叫點裡另外六個的用途。
