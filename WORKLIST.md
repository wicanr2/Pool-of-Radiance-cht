...
- [ ] **走得到的內容量到了**（2026-09-03）：六個種子各走 6000 步，走到
  ...
  **再往下追一步，路線整條找出來了。** 全封存檔掃 `21h LOAD FILES` 與
  `20h NEWECL` 的運算元：

  - 只有 `ecl6/25` 自己會 `LOAD FILES 19h`（載 GEO6/25），所以隊伍要站上那張圖
    一定得先在區塊 25 裡。
  - 切進區塊 25 的來源只有一個走得到的：**`ecl7/26` 的 `99E2h NEWECL 25`**。
  - 進去之後，`ecl6/25` 自己有 `NEWECL 01`／`13h`／`1Ch` ——正好就是缺的
    **1、19、28** 三個。

  **`ecl7/26` 是野外旅行的分派器**，那一段長這樣：

  ```
  9914  COMPARE [4A9E] 255 → IF = → EXIT
  991C  COMPARE [49C4] 2   → IF <= → GOSUB 9964 → ON GOTO
  9954  COMPARE [49C3] 2   → IF =  → GOSUB 99B8
  99B8    ON GOTO …
  99D6    SAVE 14 → [49C3] ；SAVE 6 → [6E12] ；99E2 NEWECL 25
  ```

  `DS:49C3h`／`49C4h` 就是**野外座標**（spec 105）。所以那三個區塊是躲在
  「走到月之海沿岸的某一個座標」後面的。

  **這才是卡住的地方**：探索器在野外沒有規劃器——spec 105 那一段寫得很清楚，
  野外位置不在 GEO 格子上，所以現在是「輪流轉向再往前走」的隨機走法，
  湊不到指定座標。下一步是給野外一個會瞄座標的走法，不是再調選單輪替
  （選單輪替那一步已經量過，對覆蓋沒有影響）。
  ...
  剩下的 6 個（3、4、5、6、7、15）要更前面的區塊先通：15 在 ecl4/2 之後，
  3／4／5／6／7 是同一個晚期叢集（7 是最終戰）。
