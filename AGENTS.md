...
- **主線通關收據的口徑（使用者 2026-09-17 決定，#5）**：用產品的作弊選單
  （spec 141：鎖 HP、一擊斃命，預設關、開過會記在存檔）從標題以正常按鍵跑到結局，
  算主線通關收據。原版那一側同樣可以作弊通關：dosgolem 是我們自己的工具，可以擴充
  到在原版裡鎖 HP，兩邊逐段對照必經區塊與旗標。**開作弊通關也算對拍**（使用者 2026-09-26，#57），
  前提是遊戲機制與 AI 在過程中都有作用：收據要數得出敵方挑人、走位、出手、命中，
  隊伍照樣擲命中（`TestMainlineProbeCheatMenuToEnding` 的機制閘門）。不開作弊、以原版
  規則強度通關改為可選的量測（#22）：那幾條探針遇到全滅是記錄，不是紅燈。
  上一條「不依賴 forced-win」仍適用於**作弊選單以外**的捷徑：不得用傳送、
  注入旗標、座標或資源來通關。**穿牆**（使用者 2026-09-17 追加）兩邊都可以暫時用：
  remake 是作弊選單的 `W`，原版是駕駛在被擋住的那一步清掉記憶體裡的牆 nibble、踏過去再寫回，
  每一次記進收據；逐段對照得起來就算數（spec 141〈穿牆〉）。
