...
- **A（#54，做完）**：任意鍵掃全部鍵；治具補三處「回標題就這一趟結束」（stojanow 的 `record`、
  野外洞穴與遊牧營地的 `walkTo`／`block`）。順帶修一個被擋住的舊缺陷：`memoriseSpells` 的
  `memberKeys` 只有六個鍵，第七個人（NPC）一進隊就越界 panic。
- **B（#22，前進一大段、未關）**：探索器接上 `restUntilHealed`，每一趟從「79 步就全滅」變成
  「四萬多步」；接著查出那是市政廳 ECL3/8 的 (4,5)↔(5,5) 空轉四萬多圈，也修掉了。
  產品是對的（`4A06` 確實有清），空轉的是治具：腳本第一次踏上職員格會 `SAVE 1 @4A01`，
  之後 `9BA8h` 看到 `4A01 > 0` 就跳過逐槽掃描，待交的槽永遠不會被 acknowledge。
  兩道休息 guard 與這一道的理由寫在程式與 playtest 補十九。
  **每一趟現在走 109～161 步就結束**，而結束位置集中在 (5,5)／(4,6)（市政廳的 guarded door）、
  (10,5)（神殿守衛）、(10,8)（扒手）——剩下的不是休息，是「一級隊伍要不要去踩會開打的內容」。
- **C（#21，有結論、沒改碼）**：原版 (5,7) vs remake (6,7)。直接改判準會讓主線探針在城堡走偏，
  已退回並把「要先逐列對過九次換區塊的落點」寫進 issue。
- **D（#39，拿到正對照與第一筆夜間收據、未關）**：白天進得去、晚上人沒動也沒有問句（`6E7D` 11 → 8）。
  十一個門面還沒逐一量。推時鐘的可行做法寫進 spec 102。
- **停止線**：D 照停止線換過三種推時鐘的做法；C 照「改了會走壞就退回」收手。
