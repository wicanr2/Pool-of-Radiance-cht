...
狀態：CONFORMED（overlay-25 entry 32 的完整資料流、篩選與輸出語意；鄰近格位
產生者定位到 overlay-31 `0912h` 與其三候選結構）；
...
- `docs/audit/ida-overlay25-nearby-opponents.json`，IDA Pro 9.4、overlay-local file
  ...
  overlay-25 SHA-256 `9fede24b…50c0e`。
- `docs/audit/ida-overlay25-opposing-side-selector.json`：被呼叫的 `sub_23F5`，
  由 mover 取得對立陣營值。
- `docs/audit/ida-overlay31-nearby-cell-builder.json`：overlay-31 的 `0579h..0912h`
  與 `0912h..0BDFh` 兩個函式，overlay-31 SHA-256 `64f1f7b8…151df`。
- `START.EXE` 的 MZ header（`docs/re/dos-toolchain-baseline.md`）：59 header
  paragraphs，用於換算 overlay stub 段的 file offset。

## 篩選端：overlay-25 entry 32（exact）

1. 進入後以三個 far helper 由參數取得 mover 與距離，再 far call 產生**鄰近格位表**
   （被呼叫者見下一節），參數是 `ds:6674h` 與 `ds:6676h`。
...
### 那個 far call 的目標不在 overlay-25 內

`ida-overlay25-nearby-opponents.json` 在 `2468h`（十進位 9384）的指令，原始 bytes
是 **`9A 3E 00 38 01`**。`9Ah` 是 far call，其後依序是 offset 與 segment，
目標為 **`0138h:003Eh`**。IDA 顯示的 `sub_13BE` 只是把它線性化
（`0138h × 16 + 3Eh = 13BEh`）後的自動命名，**與該檔其餘位址所用的
「overlay-local file offset, base 0」是兩個不同的位址基準**，不可並列。

實測佐證：overlay-25 的 file offset `13BEh` 落在 `cmp ax, 13h`（`3D 13 00`，
起於 `13BDh`）中間，不是指令邊界；以該 offset 為 seed 匯出，得到首條
`adc ax, [bx+si]`、尾為 `retf 4` 的錯位解碼。檔案本身無誤（SHA 與原匯出相同）。

### `0138h` 是共用的 overlay entry stub 段

掃過既有 IDA 匯出，五個不同 overlay 只呼叫同一段的兩個 offset：
...
segment 是硬編常數、五處共用兩個 offset——這是 Borland overlay 的 entry stub 表。
stub 內容取自 `START.EXE`：header 59 paragraphs，故段 `0138h` 的 file base 為
...
`CD 3F` 即 INT 3Fh，overlay manager 的載入中斷。方法見
`~/.claude/knowledge-base/retro/borland-tpov-overlay-re.md`。

### 目標 overlay 由實測定出

掃過全部 overlay，**只有 `overlay-31.bin` 在 `0579h` 與 `0912h` 兩處同時是
`55 89 E5`（`push bp; mov bp,sp`）函式序言**。IDA 對該檔解出的函式邊界
`0579h..0912h` 與 `0912h..0BDFh` 相鄰不重疊，且 `0BDFh` 正好是同一 stub 表另一筆
（file `1750h`）的目標——三項互相支持。

因此 spec 053 所稱「overlay-25 entry 32 呼叫 `sub_13BE`」，真正的被呼叫者是
**overlay-31 `0912h`**。
...
1. 先以迴圈 `var_3 = 1..3` 產生**三個候選**：每次以 `arg_4` 與迴圈索引呼叫另一個
   ...
   `arg_C`／`arg_A`，存進兩個堆疊陣列；回傳 0 時該格寫 `0FFh` 作為無效標記。
   **列舉是三格，不是八方向掃描。**
2. 清空 `ds:6678h`（結果筆數），再以 `ds:5E88h` 取得 combatant 總數，逐一走訪。
3. combatant 屬性表基底為 **`ds:5E85h`，每筆 4 bytes**；`5E85h` 與 `5E88h` 是同一筆
   內相距 3 的兩個欄位，索引以 `index×4` 計算。第 0 筆的欄位同時被當成總數使用。
4. 逐 combatant 再次呼叫 `13Dh:2Ah`，帶入該 combatant 的 `+3` 欄位與內層索引，
   偏移加上該 combatant 的座標欄位後展開——對應大型怪佔多格的情形。

## 尚未閉合

- **`13Dh:2Ah`**：偏移表的實際內容。它同樣是 overlay stub，要照本規格的方法再解一層
  （`013Dh` 的 file base 為 `3B0h + 13Dh×16 = 1780h`）。解出它才知道三個候選格的
  幾何關係，以及大型怪的 footprint 展開規則。
- `arg_4`／`arg_A`／`arg_C` 的來源與語意（距離參數與基準座標的對應）。
- `0138h:0034h` → overlay-31 `0579h`：被 overlay-13 的 move-probe 與
  move-budget-step、overlay-24 effect-apply 共用的另一個服務，尚未解讀。
- `6674h` 表每筆前兩 byte 的語意（合理推測是格座標，但未證）。
- 迴圈從 1 起算，因此 `6678h`（＝`6676h+2`，即筆 0 的第三欄）與筆 0 的前兩欄是否為
  保留槽或另有用途。`5E85h` 表出現同樣的「表首兼放 count」佈局，使這一點多了一個
  獨立例證，但仍屬 `strong inference`。
...
3. 索引 1-based 的原始語意要保留在資料層；轉成 Go 的 0-based 只能發生在
   邊界轉換處，且要有測試釘住。
4. 在 `13Dh:2Ah` 閉合前，不得自行推定三個候選格的幾何關係，也不得用
   「八方向相鄰」之類的現代假設替代。
5. 引用位址時必須標明基準（overlay file offset 或 segment:offset）；IDA 對 far call
   目標的線性化命名不是 file offset，不可混用。
