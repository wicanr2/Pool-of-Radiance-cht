...
**戰術地圖指標與結果緩衝**：`DS:6674h` 是指向戰術地圖的遠指標
（offset 在 `6674h`、segment 在 `6676h`），overlay-25 entry 32 把它整個推給
`0912h`，`0912h` 再轉給 `sub_419` 當地圖用（spec 057）。緊接在它後面的
`DS:6678h` 是結果筆數，元素自 `DS:6679h` 起，每筆 3 bytes：
`+0` combatant 索引、`+1` 成本、`+2` 攻擊該目標所需的朝向。
程式碼中的折疊常數是 `6676h`，正好落在遠指標的 segment 欄位上——
兩者是相鄰的不同資料，不是同一個結構。
