...
這一輪照這個方向走過一遍，四個修好了，剩下三個卡在**同一個新問題**上：
逐格走會先碰到另一場架，而**那一場久久分不出勝負**——
`TestPassiveCombatTerminates` 讓完全不還手的隊伍打到第 3629 回合，
敵方一直是「ATTACK 4 MISSED」。把隊伍生命值從 40 降到 6 也一樣。
所以那不是測試的問題，是**那一場敵人打不死人**——與先前修過的「敵方靠不
過來」是同一族的洞（spec 096 的敵方回合還不是原版的）。

**結論**：`DS:6DD5h` 要接上去，得先讓「隨便走到的那一場架都能收得了尾」。
在那之前接上去只是把一個已知的洞換成三個紅燈。程式碼裡兩支函式
（`setMapExitFlag`／`applyMapExitCommit`）都留著、都有註解，接的時候只要把
`setMapExitFlag` 的呼叫放回 `moveInitialDungeonForward` 就成立。
