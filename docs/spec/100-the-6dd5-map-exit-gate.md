...
狀態：CONFORMED（八個 archive 共 18 個讀取點、細讀過的三處各自通往哪裡、
「沒有任何 ECL 寫它」，
...
## 旗標活多久（2026-09-18 補，#44）

它只活**一圈主迴圈**。overlay-03 的主迴圈進來就把它清掉：`3620h` 開頭算完地形暫存
（`6A0Bh`／`6A0Cx` → `6A0Fh`）之後，`363Ch` 是 `31 C0`（`xor ax,ax`）接
`26 89 85 AA 05`（`mov es:[di+5AAh], ax`）——`5AAh` 就是 `6DD5h`。所以一圈的順序是
**清 0 → 讀玩家指令（overlay-14 走一步時可能立成 1）→ 跑那一格的入口**。位元組 exact。

由此得到 remake 要照做的規則：**腳本停下來等玩家的時候，旗標就該被清掉**——
等待就是把控制權交回主迴圈，下一圈頂端一定會清。分得出來的兩種路徑：

| 路徑 | 中間有沒有等玩家 | 原版落點 |
|---|---|---|
| 索寇要塞的回程船（`ecl4/21 9973h` 先 `GOSUB AE0Eh` 等按鍵，再 `SAVE 3 @6E12`、`NEWECL 0`）| 有 | 城區碼頭 ECL3/0 (15,1)，**不會**再換一次 |
| 野外碼頭搭船（`ecl8/27 9E37h` 選單，再 `SAVE 3 @6E12`、`NEWECL 0`）| 有 | 同上 |
| 城堡外圈那一串（block 9 → 6 → 3）| 沒有 | 旗標跨過 `NEWECL`，下一個區塊的入口接著換下去 |

**清的時機是「等玩家」，不是「換區塊」。** 換區塊就清會把城堡那一串拆掉
（實測：探針停在 `ECL5/6`，走不進城堡中庭）；不清則兩處搭船都會落地後立刻被
城區入口 0 的 `993Ah` 再送去貧民窟一次（#44 的症狀：玩家出現在貧民窟東北角）。

remake 的實作在 `continueInitialSearch`：續跑之前寫 `6DD5 = 0`。
...
writer 是 overlay-14 `06AEh`（見檔頭）。這一節留的是**當時為什麼找不到**——
兩個掃描面各有一個洞，成因不同，都值得記住。

- **沒有任何 ECL 區塊寫它。** `cmd/pool-ecl-memory-audit -addresses 6DD5`
  掃八個 DAX 的全部區塊：**18 條 `COMPARE`，零條 `SAVE`**。
  這個掃描面比本節原本用的「29 個有文字的 block」寬，零 `SAVE` 的結論在寬的
  面上依然成立——而且當時列為「解不開」的 ecl7/17 也解出了一條（`0x9DE7`）。

  那 18 條攤開來是：ECL1 block 18／24、ECL2 block 9／15／20、ECL3 block 0／14、
  ECL4 block 2／10／21、ECL5 block 3／4／5／6、ECL6 block 1／28、ECL7 block 17、
  ECL8 block 29——**八個 archive 一個不缺**，而且其中 15 條落在
  `0x9918..0x9A00` 這個窄帶裡，和下一節那段貧民窟的碼（`995Dh` 起）同帶。
  「每一張地圖都在同一個位置問同一件事」這個形狀，本身就是語意那一節的第四條
  證據。
- **執行檔裡沒有任何一處直接定址它。** `START.EXE` 與 38 份 overlay 全掃
  ...
  **這個推論是對的**：`06AEh` 寫的是 `[4937h] + 5AAh`——基底在暫存器、位移是
  `5AAh`，位元組序列裡根本不會出現 `D5 6D`。掃 ECL 位址的字面值，結構上就掃
  不到「基底＋位移」形式的引擎寫入。
...
4. **分佈**：八個 ECL archive 每一個都讀它，共 18 處，其中 15 處落在
   `0x9918..0x9A00` 同一個窄帶（清單見上一節）。**每張地圖都在自己的同一個
   位置問同一件事**——這是「地圖級的共用閘」才有的形狀，不是某幾個地點的
   特例旗標。
...
**貧民窟的遭遇已經讀完了**（spec 136）：`ecl2` block 20 `B1B0h` 起那幾條
`LOAD MONSTER` 的值由同一個 block 的 `9B68h`..`9BF7h` 填——隊伍強度 `÷3 ×2`
決定數量，`RANDOM 2` 挑三種怪其中一種，怪物編號從腳本自己位元組裡的五張
三格表查出來。remake 跑同一段排出三群（怪物 5／63／4），接著停在
`29h ENCOUNTER MENU`。

那一段由**入口 3**（紮營被打斷）跑，不是走路。走路時誰擲遭遇仍然沒讀——
入口 0 掃過整張圖沒有一格走到 `24h COMBAT`。
