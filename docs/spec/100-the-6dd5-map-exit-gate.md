...
狀態：CONFORMED（兩個使用點、它們各自通往哪裡、「沒有任何 ECL 寫它」，
以及**誰寫它、寫什麼值**——2026-09-06 由 spec 125 補上）。
日期：2026-09-06（原 2026-09-03）。

> **誰寫它**：overlay-14 `06AEh`。走一步之前先把它清成 0，那一步若會走出
> 16×16 就立成 1（`[4937h] + 5AAh` 依 spec 106 的 class 1 換算就是 `6DD5h`）。
> 細節與「查詢面繞回、位置面夾住」的分工見 [spec 125](125-map-boundary-and-the-exit-flag.md)。
