...
加上逐支讀出來的三十支處理常式（共 33 個編號，Fireball 與 Hold Person
各有兩個編號共用一支），六十七格裡**接得出來的有 58 格**。
...
| 編號 | 法術 | 位址 | 形狀 |
|---:|---|---|---|
| `0Dh` | Reduce | `135Eh` | `DS:6B88h` 為 0 直接返回；`0100h:0043h(目標, 4, 0)` 回非零（豁免成功）就返回；`0100h:006Bh(目標, 0Ch)` 為零（身上沒有效果 `0Ch`）也返回；三關都過才印 `has been reduced` |
| `3Bh` | （無名）| `2E9Ah` | `0100h:007Ah(目標, 15h, 0, &臨時位元組)` 回非零才印 `is stronger`；接著一律 `0100h:0052h(1, 那個位元組, 0x7C7(目前法術, 26h), 目標)` |
| `3Ch` | （無名）| `2F02h` | `Roll(1, 6) + 20` 與 4 一起交給 `287Ch`，再 `2919h(0, 4, 20, 3)`；訊息是 `is paralyzed` |

**三支卡在不同的東西上**：Reduce 卡在效果碼 `0Ch` 的語意；`3Bh` 卡在
`0100h:007Ah` 與 `0052h` 兩支常式；`3Ch` 卡在 `287Ch`／`2919h`。
**擋錯人比不擋更難查**，所以寧可留白也不猜。
