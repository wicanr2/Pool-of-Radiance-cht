...
加上逐支讀出來的二十七支，六十七格裡**接得出來的有 52 格**。
...
| 編號 | 法術 | 位址 | 形狀 |
|---:|---|---|---|
| `0Ah` | Charm Person | `11C5h` | **先擋**：目標的 `+9Fh` > 1 或 `+6Ch` > 1 就印一句話返回；過得了才走 `08BCh` |
...
| `17h`／`31h` | Hold Person／Hold Monster | `1650h` | 兩支共用。豁免修正看**選了幾個目標**：1 個時 Hold Person（`DS:6779h` = `17h`）是 `FEh`（−2）、另一支是 `FDh`（−3）；2 個是 `FFh`（−1）；3 或 4 個是 0。接著逐一對每個目標丟豁免 `0100h:0043h(目標, 參數表 +9, 修正)`，**目標的 `+9Fh` > 1 或 `+6Ch` > 1 就強制當作豁免成功**，最後把參數表 `+0Ah` 的效果碼交給 `0100h:0084h` |

**四支都沒接，卡在同一種東西上**：Reduce 卡在效果碼 `0Ch` 與豁免類別 4 的
語意；`3Bh` 卡在 `0100h:007Ah` 與 `0052h` 兩支常式；`3Ch` 卡在 `287Ch`／`2919h`。
Charm Person 卡在 `+9Fh`／`+6Ch` 兩個欄位。**擋錯人比不擋更難查**，所以
寧可留白也不猜。

`+9Fh`／`+6Ch` 那一對出現在兩個地方——Charm Person（`11C5h`）與
Hold Person／Hold Monster（`1650h`）——而且兩處的用法一致：值大於 1 就
當作那個目標免疫。兩支都是「只對人形生物有效」的法術，所以那兩個欄位很可能
是怪物的種類或體型，但**還沒有直接證據**，先記下形狀。
