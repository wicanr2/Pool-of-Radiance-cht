...
加上逐支讀出來的三十二支處理常式（共 35 個編號，Fireball 與 Hold Person
各有兩個編號共用一支），六十七格裡**接得出來的有 60 格**。
...
| 編號 | 法術 | 位址 | 形狀 |
|---:|---|---|---|
| `3Ch` | （無名，沒有訊息）| `2F02h` | `2F08h` 推 `DS:6CADh`／`DS:6CAEh`、`Roll(1, 6) + 20` 與 4 給 `287Ch`，再 `2919h(0, 4, 20, 3)` |

卡在 `287Ch`／`2919h` 兩支本地常式；`287Ch` 也是閃電束走的那一條
（目標模式 8 ＝ 直線）。`2F02h` **自己沒有訊息**——`2F41h` 的
`is paralyzed` 是下一支（編號 `3Dh`，`2F4Eh`）的。
