...
加上逐支讀出來的十三支，六十七格裡**接得出來的有 38 格**。

## 讀過但還沒接的幾支（形狀已知，下一輪直接做）

| 編號 | 法術 | 位址 | 形狀 |
|---:|---|---|---|
| `2Ah` | Prayer | `249Dh` | 把 `(施法者的 +10Eh << 4) + 施法者等級` 推在**等級覆寫**那一格，其餘三個推 0 |
| `27h` | Cure Disease | `2300h` | 整支只是 `call 225Bh`，共用的「解掉某個效果」常式 |
| `37h` | Slow | `2BC7h` | 先推效果碼 `27h`，再走 `2724h`，訊息 `is Slowed` |
| `39h` | （無名）| `2DB7h` | 先問 `0100h:006Bh(目標, 2Ah)`，回 0 才走泛型那條 |
| `1Ch` | Spiritual Hammer | `19A8h` | 四個覆寫參數，第二格是 1 |

Prayer 那個 `<< 4` 值得記一筆：**等級覆寫那一格被拿來塞兩個東西**——
