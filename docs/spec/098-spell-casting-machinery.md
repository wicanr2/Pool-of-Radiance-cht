...
`352Ch` 那一支（`Next Prev Manual` 的互動介面、`Range = `、`Target `、
`Already been targeted`、`Attack Ally:` 都是它的字串）還沒讀。
remake 做的是同一件事的最小版本：N／P 或方向鍵換人、Enter 確定，
