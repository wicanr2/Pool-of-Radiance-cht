...
41 段效果訊息與它們屬於哪一支常式）；DRAFT（其餘十個欄位、`+6` 的 1..3
各代表哪一種處置、共用施法常式後半段的另外兩個呼叫）。日期：2026-09-03。
...
| `097Ch` | `mov al, [di+319Dh]` → `0100h:0043h` | `+7` 是豁免類別 |
...
`+6` 決定要不要擲，0 就不擲。分佈是 0 共 52 個、1 共 9 個、2 共 4 個、3 共 2
...
1、2、3 分別代表哪一種處置（完全無效、減半、其他）還沒讀。

`+7` 是**豁免類別**，索引角色記錄 `+6Dh` 起那五個目標值（spec 075）。
