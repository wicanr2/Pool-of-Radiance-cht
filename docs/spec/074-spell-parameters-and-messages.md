...
READY（`+0Bh`／`+0Ch`／`+0Dh`：營地法術、施法時間、AI 優先度，讀法在 spec 096〈entry 4〉）；
DRAFT（`+0Bh` 的 1 與 2 差在哪、共用施法常式後半段的另一個呼叫）。
日期：2026-09-06。
...
#### 九個呼叫點的預算各自從哪來（2026-09-06 補完）

引數形狀是 `f(X, Y, 預算, 方向, 體型, 地圖遠指標)`——由 overlay-22 `2687h`
與 overlay-09 `0287h` 兩支完全展開的呼叫定出來；其他幾支的 X／Y 或體型是
先呼叫 `013D:0070h`／`0075h` 從記錄取的。

| overlay | 位址 | 預算 | 備註 |
|---|---|---|---|
| 09 | `0287h` | **參數表 `+0Fh`** | 範圍法術收人 |
| 13 | `1F6Bh` | **參數表 `+0Fh`** | 同上，另一條路 |
| 13 | `2195h` | `(參數表[+6] & 0Fh) >> 4` ——**恆為 0** | 先 and 低四位再右移四位，結果一定是 0。照碼記著 |
| 13 | `2254h` | `參數表[+6] & 7` | `+6` 的低三位。Sleep 1、火球 3、沉默 7、臭雲 1、解除魔法 1 |
| 13 | `32FCh` | 常數 `7Fh` | 幾乎不限，等於全盤搜尋 |
| 22 | `2687h` | 常數 `2` | 火球術自己那一支，被 `[4933h]+1CCh` 罩著（見上面）|
| 24 | `023Ch` | 區域變數 `[bp-0E3h]` | 呼叫端算好的 |
| 25 | `24A8h` | 參數 `[bp+6]` | 呼叫端給的 |
| 25 | `25E8h` | 常數 `0FFh` | 不限 |

**`2195h` 那個恆為 0 值得記著**：`and al, 0Fh` 之後 `shr ax, 4`，兩步一起
必定是 0。預算 0 傳給 `0419h` 的上限是 `0 × 2 + 1 = 1`，也就是只收得到中心
那一格。看起來像原版把 `0F0h` 寫成 `0Fh`，但**照碼記著，不改**。

**`2254h` 的 `+6 & 7` 與 `+0Fh` 在五支範圍法術裡有四支相同**
（Sleep 1、臭雲 1、解除魔法 1、火球 3），只有沉默 15 呎不同（7 對 1）。
兩者是不同的呼叫端、不同的欄位，**不要因為數字像就把它們當成同一個**。
...
| `+7` | ov22 `0AC9h`、`0C40h` |
| `+0Bh` | ov13 `2451h`、ov19 `1BF5h`（為 0 是營地法術：戰鬥中印 "Camp Only Spell"，spec 096）|
| `+0Ch` | ov13 `24ABh`（÷ 3 是施法時間，spec 096）|
| `+0Dh` | ov09 `0300h`（AI 挑法術的優先度 0..7，spec 096）|
