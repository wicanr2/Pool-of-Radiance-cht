...
狀態：CONFORMED（表的位置與版面、`+0`／`+1`／`+2`／`+4`／`+5`／`+8`／`+9`／
`+0Ah` 八個欄位的用途、41 段效果訊息與它們屬於哪一支常式）；
DRAFT（其餘八個欄位、`+8` 的 1..3 各代表哪一種處置、共用施法常式後半段的
另外兩個呼叫）。日期：2026-09-03。
...
`+3`、`+6`、`+7`、`+0Bh`..`+0Fh` 都有人讀，讀的地方在 overlay-09、
