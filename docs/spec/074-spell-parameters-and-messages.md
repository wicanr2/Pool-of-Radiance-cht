...
狀態：CONFORMED（表的位置與版面、`+0`..`+9`、`+0Ah`、`+0Eh`、`+0Fh` 十三個
欄位的用途、41 段效果訊息與它們屬於哪一支常式、`+8` 三種處置）；
DRAFT（`+0Bh`..`+0Dh` 三個欄位、共用施法常式後半段的另一個呼叫）。
日期：2026-09-06。
...
| `+0Fh` | ov09 `0272h`、ov13 `1F56h` | **範圍法術收人的預算**，見下一節 |

### `+0Fh` 是範圍法術收人的預算（2026-09-06）

把三十八支 overlay 掃一遍找 `lcall 0138h:003Eh`（overlay-31 entry 6，
spec 056 的鄰近查詢），整包 **9 個**呼叫點。其中兩個推的是參數表：

```
overlay-09 026Ch  di = 法術編號 × 16
         0272h  al = [di+31A3h]        ; 3194h 的 +0Fh
         0278h  push ax                ; ← 預算
         0279h  push 0FFh              ; 方向不限
         027Bh  push 1                 ; 體型
         0287h  lcall 0138:003E
overlay-13 1F56h  同一個形狀，法術編號取自 [bp+0Eh]
```

那個預算原封不動變成 `0419h`（**TraceMovement**）的上限，所以「在範圍內」
＝ **那個預算內走得到**，不是半徑比大小。

六十七格裡 `+0Fh` 非零的只有七格，而且**正好都是範圍法術**——分組本身就是
證據：

| 編號 | 法術 | `+6` 的目標模式 | `+0Fh` |
|---:|---|---:|---:|
| 21 | Sleep | 9 | 1 |
| 25 | Silence, 15' Radius | Fh | 1 |
| 34 | Stinking Cloud | 9 | 1 |
| 41 | Dispel Magic | 9 | 1 |
| 46 | Dispel Magic | 9 | 1 |
| 47 | Fireball | Bh | **3** |
| 64 | （火球的另一個編號）| Bh | **3** |
...
#### 火球術有兩個預算，`2675h` 那個 2 有條件

overlay-22 自己那一支（`2687h`）推的是**常數 2**，與參數表的 3 不同。
它被前面一道判斷罩著：

```
2661h  les di, ds:4933h
2665h  cmp word es:[di+1CCh], 0
266Bh  jne 26E0h                 ; 非零就整段跳過，不自己收人
266Dh  ...0138:003E(6CADh, 6CAEh, 預算 2, 0FFh, 1, 地圖)
       結果 DS:6678h 是筆數、DS:6676h 起每筆 3 bytes
       → 逐筆換成戰鬥者遠指標存進 DS:6B85h，DS:6B88h = 筆數
26E0h  接著畫特效並結算
```

**`[4933h] + 1CCh` 是 ECL 位址 `49E6h`**（強推論）：`4933h` 是 class 0 的基底，
換算是 `[4933h] + 6E00h + addr × 2`，而 `(6E00h + 49E6h × 2) mod 10000h = 01CCh`
（spec 106）。全域初始化（overlay-07 `02D2h`）把它設成 1，而那一支**接著**才
從 ECL `4A00h` 開始清那 0x800 bytes——`49E6h` 在被清的範圍**下面**，所以初值留著。

ECL 這一側查得到 19 個參照，**全部在野外那幾個區塊**（ECL6/19、ECL6/25、
ECL7/26、ECL8/27）：16 個 `SAVE`（10 個寫 1、6 個寫 0）與 3 個 `COMPARE`。
所以它是野外腳本在開關的旗標，不是死值。**它代表什麼還沒讀**——只知道
為零時火球術會自己收一次人（預算 2），非零時沿用既有的目標清單。

overlay-22 `2687h` 以外的另外六個呼叫點推的是別的東西
（`+6` 的低四位、`+6` 的低三位、常數 `7Fh`／`0FFh`），還沒逐支對用途。
