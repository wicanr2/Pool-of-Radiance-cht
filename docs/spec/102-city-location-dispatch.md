...
- **換區會清它。** 共用 engine 的 `NEWECL` 只換掉 `0x9900` 以上的程式碼段，
  ...
  那一步 `4A01 01→00`，寫入點 `1997:02FE`（overlay-07）。remake 目前沒有這一步，
  修正追蹤在 issue #41。
...
「同一個區塊裡」各支寫了什麼。remake 還沒有載入區塊的清除（issue #41），
`4A01` 會一路帶著，下面最後兩段的順序限制只在 remake 裡成立。
...
**在沒有區塊載入清除的 remake 裡**，`4A01` 不會自己歸零，於是要先說謊
（寫 255）再說實話，而且中間不能回城區：說謊之後 `4AA7` 還小於 254，港務長走
「唯一的船是去索寇要塞」那一支，`9DCEh` 又把 `4A01` 寫回 1。原版回城區時
`4A01` 已經被清掉，沒有這個限制（strong inference：清除段是位元組 exact，
「從要塞回城也經過它」沿用市政廳那一次實跑，尚未單獨量過）。
...
所以**打贏亡魂跟說實話一樣會開航線**，而且不碰 `4A01`。在 remake 目前的狀態下，
「先說謊再說實話」與「先說謊再打贏」兩條路都到得了同一個狀態。說實話多拿到的是 SAMOSUD——
出要塞時通過守衛要用。
