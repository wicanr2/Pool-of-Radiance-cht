...
- **票是誰清的？** 兩個可能已經排掉：
  ...
  **目前最像的答案是 `SAVE 255 @4A01`**（ecl4/21 `ABBDh`）。255 是這一份程式碼
  裡通用的「沒有」哨兵——`LOAD FILES` 的 `FF` 是不載地圖（spec 043）、
  ecl1/24 `99B0h` 那張表的 `FF` 是不換區塊——而 255 同時通得過這兩道閘門
  （不是 0、也不是 1）。它落在要塞裡鬼魂問句的 `LIE?` 那一支，形狀上就是
  「把船票狀態清掉」。**還沒證實**：探索器一直沒走到那一格，所以沒能實測
  「說謊之後港務長會不會再開口」。要嘛讓治具走到那一格，要嘛拿原版當 oracle。
