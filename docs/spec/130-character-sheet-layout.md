...
肖像的 88×88 內容佔 native x 224..311、y 8..95。肖像框與整頁外框共用上緣
y 0..7 與右緣 x 312..319；新增的左緣是 x 216..223、下緣是 y 96..103，
三個交點與繩索同樣使用 `8X8D1.DAX` block 202 的 item 20／21／22。
`cmd/pool-game/portrait_frame_test.go` 逐像素證明內容位置與七個框 tile；先前把
「肖像加框的 96×104 區域」簡寫成「肖像 x 216..311、y 8..103」，會把 88×88
資產誤放到 x=216 並漏畫框，現已勘誤。
...
3. 右上角有肖像與原版繩索框；建角當下用預設那一張
   （`PortraitHead`／`Body` 都是 1）。88×88 內容不得侵入左、下框。
