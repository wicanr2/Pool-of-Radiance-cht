...
| 6、16 | `110Bh` 泛型 | `08h` | 0／0 | 同上 | 群組 11／12；12 要比 `DS:5CF0h` 的陣營，**未接** |
| 7、17 | `110Bh` 泛型 | `09h` | 0／0 | 同上 | 同上，**未接** |
| 8 | `114Ah` 泛型 | `0Ah` | 0／0 | 同上 | 群組 6／12 要看 `DS:6777h`，**未接** |
...
| 24 | `17D4h` 泛型 | `14h` | 0／0 | 同上 | 群組 6／12 要看 `DS:6777h`，**未接** |
| 25 | `180Dh` 泛型 | `15h` | 0／0 | 同上 | 群組 15（沉默，spec 096）|
| 26 | `1846h` | `16h` | `FFh`／1 | 同上 | 只對中毒（`37h`）的；生命值 0 墊成 1 |
| 28 | `19A8h` | `17h` | 0／1 | 同上 | 之後 entry 1 以模式 0 叫 `17h`（生出鎚子），**未接** |
| 29 | `10D1h` 泛型 | `18h` | 0／0 | 同上 | 空常式 |
| 30、50 | `19FBh` 泛型 | `19h` | 0／0 | 同上（兩格都是 0：不計時）| 群組 16 −4、出手就摘（spec 112）|
| 31 | `1A34h` 泛型 | 無 | — | — | `+0Ah` 為 0，什麼都不掛 |
| 32 | `1A6Fh` | `1Ch` | Roll(1, 4)／0 | 同上 | 群組 5／6 擋一擊（`09CDh`），**未接** |
| 33 | `1AB1h` 泛型 | `1Dh` | 0／0 | 同上 | 群組 4 近戰傷害減四分之一，**未接** |
| 38 | `222Eh` 泛型，碰觸 | `21h` | 0／0 | 同上 | 群組 10 −4、11 AC −4、12 豁免 −4 |
| 40 | `231Dh`，碰觸 | `22h` | 0／1 | Roll(1, 6) × 10 | 收尾叫 `2Bh`／`2Ch` 的常式，**未接** |
...
| 52 | `110Bh` 泛型 | `2Dh` | 0／0 | 同上 | 同 6，**未接** |
| 53 | `110Bh` 泛型 | `2Eh` | 0／0 | 同上 | 同 7，**未接** |
| 54 | `110Bh` 泛型 | `29h` | 0／0 | 同上 | 群組 5，**未接** |
