...
節點的完整版面由 overlay-24 entry 2（`0028h`，摘除節點的那一支）定下來：
**一個節點 9 bytes**，`+0` 代碼、`+4`「有收尾常式」旗標、`+5`／`+7` 下一個的
遠指標；串列頭佔記錄的 `+7Fh`／`+81h` 兩個 word，釋放走 RTL 的 FreeMem
（`5BBh:364h`，長度 9）。`+4` 非 0 時，摘除之前會以**模式 1** 叫一次那個代碼
自己的處理常式（`DS:6786h` 那張表，spec 112）——套用時傳的是模式 0。
