...
武器把 `+110h`／`+115h`／`+117h`／`+119h` 算出來的流程與物品型別表的四個欄位）；
DRAFT（`+112h` 的組成、`sub_281`／`sub_39F` 的 AC 修正鏈、`sub_1173`／`sub_12AE`／
`sub_1366` 三個能力值加值、物品型別表其餘欄位）。日期：2026-09-02。
...
overlay-25 `0E36h` 是「重算這個角色的戰鬥數值」：先把 `+111h = +0A9h`、
`+11Ch = +72h`、`+110h = +2Dh` 三個基礎值搬進來，沒有武器時補上兩個能力值加值，
再呼叫上一節那支，然後沿 `+0C8h` 的物品串列（`+2Ah` 是 next）累積五個修正值，
最後 `+111h` 是那五個修正值的和、`+112h` 由其中三個減 2 得到。

五個修正值分別由 `sub_281` 與 `sub_39F` 填，兩支都還沒讀完，所以**穿裝備之後的
AC 尚未閉合**；本規格只把「沒有裝備時基礎 AC internal 是 `32h`」定下來。
...
6. 穿裝備後的 AC 在 `sub_281`／`sub_39F` 閉合前不得宣稱完成。
