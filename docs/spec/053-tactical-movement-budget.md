...
| `+1`／`2759h` | `PathByte1` | overlay-31 `0419h` 取起點格的這一欄當通行門檻；原始 66 筆初始化值全為 0 | exact（用途見 spec 057） |
| `+2`／`275Ah` | `PathByte2` | overlay-31 `0419h` 每一步比較目前格的這一欄 `> 起點格 PathByte1` 就停；原始值集合為 `00h/02h/FFh` | exact（用途見 spec 057） |
