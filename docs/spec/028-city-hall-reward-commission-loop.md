...
- `docs/audit/dos-city-hall-structure.json` 由 `cmd/pool-city-hall-audit` 從上述 trace
  重生，並以 block hash 失敗即關閉。它分別保留墓園戰利品累積量的七個**有序槽位**與四個
...
4. `9C83h ON GOSUB` 依索引進七個結算子程式，再由 `9C9Eh SAVE TABLE` 把
   `6E7Ah` 回寫到 `4A8Fh + [6E79h]`。
5. `9CA8h` 索引加一，`9CB1h..9CB8h` 在索引小於七時回圈。

`docs/audit/dos-ecl-reward-source-refs.json` 再證明 `4A39h..4A3Fh` 的可達直接 producer
集中在墓園 `ECL4.DAX/block10`；真實 block8 執行期逐槽正對照則全部顯示
`ELIMINATED SOME UNDEAD FROM THE GRAVEYARD`／`HERE IS YOUR REWARD`，並依序送出
...
block8 trace 已推翻這個單一 producer 模型：下列較後方的進度通知子程式都以
...
1. 維持機器可讀的七槽墓園戰利品與 16 路 commission 表：條件讀取、寫入、文字、
