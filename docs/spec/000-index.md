...
135 份規格，其中 39 份還沒有任何檔案的註解指回它、60 份沒有測試提到它。
這兩個數字是**盤點用的**：沒有反向引用不代表沒實作，只代表那條線還沒接起來。

## 規格

| # | 標題 | 狀態 | 實作 | 測試 |
|---|---|---|---|---|
| [001](001-dos-title-picture.md) | DOS TITLE DAX 圖像 | CONFORMED | — | — |
...
| [005](005-first-remake-executable.md) | 第一支 remake executable | CONFORMED | — | — |
...
| [013](013-shared-ecl-vm-rolf-path.md) | 共用 ECL VM 與 Rolf 真實 bytecode 路徑 | CONFORMED＋DRAFT | — | — |
...
| [019](019-pool-ecl-dialect-newecl.md) | Pool ECL dialect 與 `20h` 跨 block 交接 | CONFORMED＋DRAFT | — | — |
| [020](020-compare-and.md) | `14h COMPARE AND` 四 operand 比較 | CONFORMED | — | — |
...
| [027](027-save-table-opcode.md) | Pool ECL opcode `35h SAVE TABLE` | CONFORMED | — | — |
...
| [037](037-campaign-save-and-ecl-session.md) | 戰役存檔、地圖位置與 ECL session 續點 | CONFORMED＋DRAFT | — | — |
...
| [062](062-combat-round-loop.md) | 戰鬥回合迴圈與結束條件 | READY＋DRAFT | `cmd/pool-game/tactical.go`、`internal/combat/round.go` | `cmd/pool-game/coverage_test.go`、`cmd/pool-game/playthrough_test.go`、`cmd/pool-game/tactical_test.go` |
...
| [064](064-in-game-journal.md) | 遊戲內《探險者手冊》 | CONFORMED | — | — |
...
| [069](069-character-effect-list.md) | 角色的效果串列（`.spc`） | CONFORMED＋DRAFT | `internal/character/export.go`、`internal/gamepack/checkparty.go`、`internal/gamepack/effect_names.go` 等 6 個 | `internal/character/export_test.go`、`internal/gamepack/effect_names_test.go` |
...
| [100](100-the-6dd5-map-exit-gate.md) | `DS:6DD5h` — 擋住整個世界的那一個變數 | CONFORMED | `cmd/pool-disp-scan/main.go`、`cmd/pool-game/main.go`、`cmd/pool-game/training_gate.go` | `cmd/pool-game/coverage_test.go`、`cmd/pool-game/playthrough_test.go` |
...
| [102](102-city-location-dispatch.md) | 城區的地點是 terrain 索引分派的，而且有些要面對它 | READY | `cmd/pool-game/main.go`、`cmd/pool-map-names/main.go` | `cmd/pool-game/coverage_test.go`、`cmd/pool-game/harbour_walk_test.go`、`cmd/pool-game/inn_test.go` 等 4 個 |
...
| [107](107-newecl-ff-sentinel.md) | `NEWECL FFh` 是「不換區塊」 | READY | — | — |
...
| [122](122-locked-doors.md) | 鎖住的門（`Bash`／`Pick`／`Knock`） | CONFORMED＋DRAFT | `cmd/pool-disp-scan/main.go`、`cmd/pool-game/door.go`、`cmd/pool-game/main.go` 等 4 個 | `internal/gamepack/door_test.go` |
...
| `pool-doc-index` | 產生 docs/spec/000-index.md：每份規格的狀態、實作它的檔案、釘住它的測試，以及 cmd/ 底下每一支工具在做什麼 | — | — |
...
| `pool-game` | remake 的遊戲本體：Ebiten 視窗、玩家輸入、畫面，以及與共用 engine 和 game pack 的接線 | 有 | 003、007、008、015、016、017 等 87 份 |
...
| `pool-journal-corpus` | 把轉錄好的《探險者手冊》上冊切成遊戲內可查的條目 | — | — |
