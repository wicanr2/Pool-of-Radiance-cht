...
休息回血與記完法術的條件都已讀出並實作）；OPEN（休息被打斷）。
日期：2026-09-05。
...
- **休息被打斷**：`Stop Resting?  The Party is rudely interrupted!`
  （overlay-20 `0C15h`／`0C24h`，由 entry 3 `0C45h` 用）。那一段要先有「這一格
  安不安全」的判定——說明書 p.29 說旅店與已清除的地區安全，野外與未清除的
  地區會被怪物打斷。
