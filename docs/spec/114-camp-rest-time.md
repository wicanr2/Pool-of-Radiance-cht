...
注意遮罩是 `7Fh` 不是 `1Fh`——貧民窟的 `80h` 遮完是 0（街上）。貧民窟入口 2
（`9A24h`）只比 `@6E82 == 0`，所以**地形碼不為 0 的格子全都是 0／0**，`81h`／`82h`／`83h`
只是其中幾個。
...
屋內那些格子本身是入口 1 的事件（`ecl2/20 99C9h ON GOTO` 以地形碼分派 21 支），
傷兵走進還沒打過的房間就是一場架。21 支從入口沿控制流走到底（含 GOSUB 與 IF 的跳過）：
4、7、11、17 整支沒有 `COMBAT`；1、2、6、12、14、20 只有一場，開頭是
`COMPARE 旗標, 255 ; IF = ; EXIT`（旗標依序 `4ACA`／`4ACB`／`4A85`／`4AD6`／`4AD0`／`4AD9`）；
5、10、16 的判斷不是單純的 255；0、3、8、18、19 接到共用的隨機遭遇；9、13、15 是大場。
測試駕駛只挑前兩種（後一種要旗標已是 255），並拿步數跟旅店比：2026-09-17 在 house rule
探針 15 個 seed 上量了 139 次，屋內平均 8.7 步、旅店單程 19.7 步，屋內較近的佔 81%
（`TestSlumsQuietRoomLetsThePartyRest`）。
...
**睡覺要睡到早上**：天數不會改變醒來的時刻，而城區過了十四點（`49C9 >= 14`）
市政廳與幾道門會鎖上（spec 102〈晚上鎖門與城衛隊〉），所以駕駛在排時間時另外補
小時把醒來的時刻挪到六點（`hoursUntilMorning`）。排時間前要把天、時、分三欄都
歸零：紮營畫面記得上一次排的時間，而 `D` 只減目前那一欄（`clearRestDuration`）。
