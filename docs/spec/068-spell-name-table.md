...
56 條裡有 55 條帶著說明書下冊第六章的說明（逐條轉錄，含說明書自己的
〔原書如此〕註記）。`Restoration` 沒有——它不在法術章，那是神殿服務。
少一條就失敗即關閉：漏掉一條在畫面上看起來只是「這個法術沒寫」。

⚠ 那是**說明書寫的行為**，不是從反組譯讀出來的規則。接進戰鬥判定之前，
每一條仍要以原版程式碼驗證；目前只作為玩家可讀的參考。

## 玩家路徑

遊戲裡按 `K` 開法術一覽，`TAB` 換級別，游標所在那一條的說明顯示在清單下方。
**這是查閱用的畫面，不是施法**：
