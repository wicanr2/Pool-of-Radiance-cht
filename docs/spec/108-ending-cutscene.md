...
### `DS:4980h` 靜態讀不到（2026-09-04）

**沒有任何一處按名字寫它。** 三十八顆 overlay 加 `START.EXE` 裡，
`4980h`／`4982h` 當成 `mov ds:xxxx, ax`／`bx`／`cx`／`dx`／立即值的寫入
**一次都沒有**；讀有 31 處（overlay-36 六個 `les di, ds:4980h`，
其餘是 `push` 給繪製常式）。正對照：同一組樣式在 `4670h` 找得到
overlay-18 `02CFh` 的那一次寫入，所以是真的沒有，不是樣式漏掉。

**理由是它在 BSS。** `DS` 的位移換算成 `START.EXE` 的檔案位移要加 30640
（由 spec 068 的法術名稱表釘住：`DS:28ACh` ↔ 檔案 41052，讀出來正是
`05 "Bless"`）。`START.EXE` 只有 47,936 bytes，而 `DS:4980h` 換算出來是
49,456——**超出檔案**，也就是未初始化資料段。`4670h`（48,672）與
`52D4h`（51,844）同樣在外面。

所以那個目標緩衝是**執行時配置**的，靜態讀不出它指到哪裡。要往下推只有
兩條路：拿 DOSBox 當 oracle 抓執行時的值，或直接抓原版結局畫面量位置。
**在那之前不接圖**——猜錯的症狀是「圖出現在錯的地方」，而那和「還沒接」
在報表上分不出來。
