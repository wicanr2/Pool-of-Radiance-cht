...
狀態：CONFORMED（部署驅動 `1A99h`、樣板填寫——執行期 `43A2h` 528 bytes 四場逐 byte
對過、寫入端掃完只有 `1A99h`／`14CFh`——兩邊的偏移與象限、逐人放置 `1609h` 的掃描順序、
換陣型的鄰格（陣型 1／2 有執行期收據）、放不下就摘掉、五張 DS 表——dosgolem 三筆
同狀態收據，貧民窟三場固定事件全對）；
...
是哪一支）。
日期：2026-09-02；2026-09-16 補部署驅動與逐人放置、兩筆 dosgolem 收據（#32）、樣板執行期 528 bytes 與寫入端（#35）。
...
## 陣型樣板：`1A99h` 開打前用 `DS:304h` 填，執行期 528 bytes 對過（exact）
...
dosgolem 在四場開打那一幀讀 `DS:43A2h` 整張 528 bytes 與 `DS:304h` 60 bytes
（`docs/audit/dosgolem-deployment-templates.json`，`tools/dosgolem-deployment-templates.py`；
`304h` 執行期與 `START.EXE` 逐 byte 相同，當正對照）。開打那一幀的樣板是**放完之後**的
——`14CFh` 每放一個就把那一格清零——所以它同時記了形狀與放置走過的格。remake 部署完的
`deploymentTemplates` 四場 528 bytes 逐 byte 相同（`TestDeploymentTemplatesMatchTheOriginalAtRuntime`）：

| 場 | 象限（我／敵） | 敵方用到的陣型與格數 |
|---|---|---|
| 獸人的家 (3,3) N | 0／2 | 陣型 0 用 12 格、陣型 2 用 8 格（20 隻） |
| 衛兵攔截 (0,7) N | 0／2 | 陣型 0 用 12 格、**陣型 1**（F=4 整塊）用 20 格（32 隻） |
| 驚動衛兵 (3,11) E | 1／3 | 陣型 0 用 13 格、陣型 2 用 10 格（23 隻） |
| 哥布林 (15,5) E | 1／3 | 陣型 0 用 4 格 |

陣型 1（整塊）與陣型 2 因此都有執行期收據，換陣型的順序與 `1609h` 讀出的一致；
陣型 3 四場都沒踩到（敵方最多 32 隻，兩個陣型就放完）。

寫入端（`cmd/pool-disp-scan`，36 顆 overlay ＋ `START.EXE`，正對照 `5AAh` 掃出
overlay-14 那四條）：`304h`／`305h` 只有 overlay-10 `1BD8h`／`1BF7h` 讀（填樣板的 `lo`／`hi`），
沒有寫入端；`43A2h` 只有 overlay-10 的四處——`150Bh` 讀（`14CFh` 查格）、`15E5h` 清零
（`14CFh` 用掉一格）、`1C2Eh` 寫 0 與 `1C63h` 寫 1（`1A99h` 填）。陣列中段的位移
（`43A3h`、`4408h`、`440Eh`、`44AAh`、`44B0h`、`4516h`、`4550h`、`45A1h`）零筆。
`305h` 另有 24 筆是 `cbw; add ax,3`（`98 05 03 00`）與 RTL 遠呼叫位址的位元組湊巧，不是存取。
