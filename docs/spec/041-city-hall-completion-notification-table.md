...
主線旗標**，讓探索器從開場自己走，走完看有幾個槽被寫成 `FEh`。與世界巡迴
那一支的差別正在這裡——那一支會把 26 槽全部預設成 `FEh` 去解鎖內容，
所以它量不到這件事。

分兩個階段：第一階段從乾淨的開場走；第二階段從「把第一階段打出來的委任交差
完」那個狀態走（槽 `FFh`、十個會加一的槽各讓 `4AC1h` 加一）。**交差是解鎖的
關鍵**——探索器不會自己走進市政廳，`4AC1h` 永遠是 0，而下一批區域正是靠那個
數字開的。交差鏈本身由下一節那 26 條驗過，所以把兩段接起來是有根據的。

現在量得到**五條**：0（諾里斯）、1（索寇要塞）、11（瓦海登墳場）、
18（卡德納的付款）、23（巴恩神殿），順帶走到 20 張地圖、19 個 ECL 區塊，
硬失敗 0 次。

**這一支抓到過兩個「測試綠、玩家卡關」的缺陷**：

1. `36h ADD NPC` 沒把職業從記錄帶出來，NPC 入隊之後隊伍查詢直接報錯
   （一條 → 三條）。
2. 治具在**雙向梯子**上一律答 YES：`ecl8/29` 的井答 YES 下去、到下面再答
   YES 又爬回來，探索器就在兩層之間上上下下，答了四千次出不來。
   **遊戲沒有壞，是治具的答題策略壞了**；答過幾次改回輪流就解開了
   （三條 → 五條）。

第二個是靠**指令級追蹤**找到的，只花六個位址：
`AF71 AF80 AF86 AF87 AE87 AE8D`——答完 `4A10h` 就變 1，梯子往下走了。
在那之前連續四輪「推論 → 挑一個位址量 → 推翻」，三次推錯。
**在還不了解的控制流上挑量測點，命中率不會比擲骰子好**；`eclvm.Machine.Trace`
就是為此加的。

還沒被走到的槽需要密碼、湊物品或挑特定選單，那是「玩家知道要做什麼」，
不是「走得到那一格」。門檻是量到的下限，不是目標。
