...
**「從開場走到各區的觸發格」還沒逐區實跑。** 現在的覆蓋是兩段接起來的：
producer 端驗「條件成立時腳本會不會結案」，交差端驗「`FEh` 之後職員會做什麼」，
中間那一段（玩家怎麼走到那個條件）只有貧民窟那一條是整條連著跑的
（`TestTwentyFiveRealSlumsWinsEarnTheCityHallReward`），其餘靠
`cmd/pool-game` 的世界巡迴與覆蓋測試提供一般性的走圖保證。
