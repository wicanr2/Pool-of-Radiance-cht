...
GEO7/23 (1,1) 的選單卡住**還在**，形狀與下一步記在 `WORKLIST.md`：那一格是
密碼門，答 NO 之後告示牌會從頭再跑一次。修正後的世界巡迴之所以不再報它，
只是那一輪的隨機路徑不再經過 GEO7/23——掃 102 趟才找回一個重現
（seed 106、destination 2）。
...
- 同一份 spec 又解出 `0100h:007Ah`（overlay-24 entry 18）是「把力量往上調」，
  ...
  還缺的四支各自卡在一個還沒有的子系統，不是「再接一支就好」：迷霧術要
