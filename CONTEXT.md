...
## 2026-09-26 祝福、詛咒、急速、緩速有實際作用（#81）

- `08BCh` `0A5Ah` → overlay-24 entry 20 把參數表 `+0Ah` 掛到分邊後的每一個人（祝福 `01h`、詛咒 `02h`、
  急速 `27h`、緩速 `2Ah`；持續 `07C7h` ＝ `+4 + +5 × 等級`）。祝福命中骰 +1、詛咒 −1（群組 10）；
  急速攻擊次數與移動加倍、緩速減半，從下一回合初始化起（群組 18），急速第一次讓人老一歲。
- 群組 10 其餘碼、群組 16、群組 9（免疫）與群組 17 的士氣 ±5 沒接（spec 098 那張差異表）。
- 已推翻：「`2724h` 推的碼是身上已經有就剔掉」與「急速 `2Ah`、緩速 `27h`」。`0100h:006Bh` 是
  overlay-24 entry 15（有就摘掉、印 "is Cured"），推進來的是對面那一支的碼：急速與緩速互相抵銷。
  參數表與群組 18 的處理常式都說急速是 `27h`。spec 112 的「`01h` 士氣 +5」同一支還讓命中骰 +1。

## 2026-09-25（下午）敵方 AI：挑目標照原版、否決查詢補完、怪物施法（#64、#65）

- **#65 關閉**：`0D97h` 搆得到時擲 `骰(1, n)` 挑人（原版骰流 d20 前一擲 d1 共 23 次為證）；
  `37B8h` 沿用前過 `1087h`、劃掉重擲最多二十次、兩輪制，追擊與施法挑目標共用
  `rollFoeTarget`；`1087h` 四個否決代碼（`19h` 看當下行動者的 `18h`、`7Eh` 看目標裝備中
  物品的 `+2Fh..+31h`）都照原版，F1 的暫定行拿掉。
- **#64**：怪物與 QUICK 隊員照 overlay-09 entry 4 施法（法術效果與玩家共用 `castSpell`），
  每回合那對 `d7 d7` 與原版零錯位。剩原版施法怪物的 dosgolem 收據；另開 #71～#73。
- 亂數流因此位移，`route_before_sokal` 三條路線測試改用作弊選單（量路線不量勝負）。
- 發行包 `v.1.1.16-20260925` 對拍：34 張量到、0 欄與基準表不同。整包紅燈仍是 #57 那三條。

## 2026-09-25 台帳補齊；要塞上層、怪物名、隊伍強度、反應攻擊（#56、#58、#61、#62）；原版全滅後回 DOS（#53）

- **台帳**：盤點出十條沒有 open issue 的缺口，開成 #57～#66 並進 `docs/worklist.json`；
  十二條過期斷言改寫（README 的缺口清單改成三層、逐條連 issue）。
- **#56**：要塞上層 `ecl5/7` 是樓梯與暗門的迷宮（入口 0 查「地點索引＋朝向」表，
  spec 137）。產品缺陷：入口 0 停下來問、答完後腳本在同區搬人時，落點的入口 1 不跑，
  覲見廳文字因此不出現；改成與不停下來的路徑一致。主線作弊探針回綠，紅燈 5 → 3。
- **剩下三條紅**是不開作弊的探針（`TestMainlineProbeNaturalPartyFirstBattle` 等），
  改動前的 HEAD 上一樣紅，歸 #57。
- **#62** 戰鬥資訊欄輪到怪物時印怪物名（overlay-25 entry 5，strong inference）。
- **#61** `PARTYSTRENGTH` 由 `livePartyStrength` 從目前隊伍算；一級時對上三份原版 `.CHA`。
- **#58** 脫離鄰接的反應攻擊接上（spec 059）：兩個呼叫端、朝向的四個寫入點、目標轉身
  都讀過；`19h`／`7Eh` 否決與兩個旁路旗標未讀，保守處理。
- **#53** dosgolem 的 `int 16h AH=00h` 佇列空時原本回 0 不等，改成阻塞，並在程式結束時停
  （dosgolem 分支 `fix/pool-wipe-anykey` `2ab6f65`，只配合本專案，不併入 dosgolem main）。
  原版全滅按鍵之後**結束程式回 DOS**；remake **刻意回標題**，可以 L 讀檔（使用者定案）。

## 2026-09-19 探索器改用產品自己的作弊選單；兩個「座標沒寫回 ECL」的卡死修掉（#22、#55 關閉）

- **#22 關閉**：探索器一開場開鎖 HP ＋ 一擊斃命（正常按鍵，spec 141），只給探索器，
  主線探針不碰。對照組：不開 3 個 ECL block、只鎖 HP 8 個、兩個都開 10 個。
- **兩個真缺陷**：`CALL C01Eh` 沒把座標寫回 `C04B`／`C04C`（`DS:6A0Bh`，spec 106），
  腳本裡連走好幾步的那幾支永遠讀到出發點 → 遊牧營地爆堆疊、GEO7/23 卡住。
  配套是 `34h` 的深度上限與「換圖表空列＝牆」的撞牆回復。
- **整包 0 次硬失敗**，紅燈 7 → 3。世界巡迴與委任兩條的覆蓋面門檻拿掉，改成記錄
  走到哪為止（使用者：測試足夠了，#55 關閉）；硬失敗仍是閘門。剩下三條紅是主線探針。

## 2026-09-19 任意鍵修了、探索器會補血了；#21／#39 各有結論（#54、#22、#21、#39）

- **#54 修好**：`anyKeyJustPressed` 掃全部鍵；配套是治具的「回標題就這一趟結束」（三處）。
- **#22 前進一大段**：探索器受傷會休息，世界巡迴每一趟從「79 步就全滅」變成「四萬多步」；
  那四萬多步是市政廳 ECL3/8 的 (4,5)↔(5,5) 空轉，也修掉了。產品是對的（`4A06` 確實有清），
  空轉的是治具：腳本踏上職員格會 `SAVE 1 @4A01`，之後 `9BA8h` 看到它大於 0 就跳過逐槽掃描，
  待交的槽永遠不會被 acknowledge。修完每一趟走 109～161 步，**結束位置集中在市政廳與神殿的守衛**
  ——剩下的不是休息，是「一級隊伍要不要去踩會開打的內容」。紅燈仍七條，沒有 panic。
- **#21 有結論、沒改碼**：原版落在 (5,7)（#5 收據就有），但直接改判準會讓探針在城堡走偏，已退回。
- **#39 拿到第一筆 runtime 收據**：晚上市政廳只是進不去、沒有問句（`6E7D` 11 → 8）。

## 2026-09-18 上一輪的起點：讓「紅就是壞了」（#54 任意鍵、#22 治療策略、#21 上樓落點、#39 晚上鎖門）

主線驗證由使用者判定足夠，#19／#45／#46 已關閉。剩下七條紅燈**都是同一個成因**：一級隊伍在城區
撐不到走完。順序固定 **#54 → #22**——修任意鍵之後全滅會真的回標題、session 被拆掉，
141 處治具呼叫要一起處理，兩者是同一段工作的兩半。#21、#39 是兩筆便宜的原版量測。
goal：`docs/goals/issue-54-22-21-39-red-tests-and-two-measurements.md`。

## 2026-09-18 六條紅燈的成因查出來了：探索器的隊伍早就死了（#22、#23 關閉、#53／#54 新開）

- 十八萬圈的「走完預算」其實是**全滅之後一直按方向鍵**，而方向鍵不算「任意鍵」（#54）。
  讓全滅成為這一趟的終點之後，每一趟的結局一目了然：**每一趟都全滅**，缺的是治療／休息的策略（#22）。
- **紅燈六 → 七**，多的那條（`TestDirectedExplorationReachesMaps`）原本把「隊伍死後走到的地圖」算進覆蓋。
  整包 `pool-game` 從 190 秒降到 96 秒。
- **#23 關閉**：ECL7/17 的迴圈重現不出來，描述已過期。
- **#19 量了一半**：全滅那一刻量到了（空隊伍清單、`4961h=0`、`6DC7h=80h`），再往下 dosgolem 推不動（#53）。

## 2026-09-18 上一輪的起點：讓六條紅燈有結論（#19 戰後收場的原版收據、#23 遊牧營地迴圈、#22 策略層）

`go test ./...` 的六條紅**連續好幾輪原封不動**，代價是每次都要先想一遍「哪幾條本來就紅」——
那正是漂移最好躲的地方（上一輪的 #51 就是這個形狀）。這一輪要每一條都有結論：綠，
或把預期改成「記錄停在哪」並寫明理由。順序是 #19（先量原版，三條探索器是在戰後寫回接上之後
變紅的）→ #23（最小重現追迴圈）→ #22（六條逐條下結論）。
goal：`docs/goals/issue-19-22-23-red-tests-and-original-receipts.md`。

## 2026-09-18 搭船落點與休息打斷都修好，對拍從 33 張走到 34 張（#44、#42；#52 續開）

- **#44**：換圖旗標 `6DD5` 的清除時機是**等玩家**（原版主迴圈頂端每一圈清一次，exact），
  不是換區塊——後者會拆掉城堡外圈那一串。搭船落點與原版一致；順帶更正一條把缺陷釘住的斷言。
- **#42**：原版在導覽終點那一格休息會被城衛隊攔（00:05，GO／STAY），remake 的打斷是對的，
  停住的是畫面——入口 3 停在 `3Ah DELAY`，而那不算「可以接著跑」的呈現邊界。
- **對拍**：`v.1.1.13-20260918` 走完全程，34 張量到、1 張未量（`field-cast-spell`：
  城區街上記不成法術，是原版規則）。`field-cast` 回到表上。
- **#52 續開**：驗掉了「走到那 7 處 TREASURE 的格子」這條線索——逐格跑入口 1 一格都沒有，
  那幾處掛在旗標或戰鬥後面。下一次從固定遭遇或市政廳獎金走。

## 2026-09-18 上一輪的起點：對拍那條路上的三個缺口（#42 導覽格休息重印、#44 索寇回程船落點、#52 戰利品截圖）

量尺上一輪修好了，這一輪補量測面：#42 讓對拍每次停在第 33 張（`field-cast` 兩張自 v.1.1.5 之後沒再量過，
而且它本身是玩家看得到的缺陷）、#44 是兩邊對照唯一還開著的 remake 偏差（原版位元組 exact）、
#52 的路線已縮到貧民窟腳本的 7 處 `27h TREASURE`。**#42 要先量原版再改 remake**——
休息完重跑入口有可能就是原版行為。goal：`docs/goals/issue-42-44-52-parity-path-defects.md`。

## 2026-09-18 現況斷言校準：對拍的量尺修好了，README 的功能缺口清空（#49、#51；#52 續開）

- **#51 不是退步，是文件漂掉**：拿 v.1.1.5（那張表自己的版本）今天重量，爭議的兩項與 v.1.1.12 一模一樣；
  v.1.1.12 的 33 張逐欄對 `dos-parity-sample.json` **0 欄不同**。漂的只有手寫的 md 表。
  兩項「視野」的 100.00% ↔ 91.27% 是**營火兩張動畫**，JSON 的 note 早就寫了。
  修法：md 表改由 `cmd/pool-parity-check -write` 從 JSON 生成；對拍跑完會**拿這一次的結果對那張表**
  （不是對上一次跑的結果）並非零退出；缺席記「未量」；`POOL_PARITY_ALLOW_STALE=1` 是量舊包的明講出口。
- **#49 README**：上一版列的六條功能缺口**全部**早就做完（走路遭遇、探索施法、法術派發表、賊技能升級、
  能量吸取、效果到期收尾、多次攻擊），自評 65～72% → **74～80%**，剩下的限制只有驗證缺口（#22／#19／#6）。
  「多次攻擊」那一條是被註解位置騙的：`tactical.go:827` 的「先用一回合一次」是預設值，`890` 行就覆蓋了。
- **#52 戰利品截圖續開**：腳本已能建六人隊、走到遭遇、打完（`n` 才是結束戰鬥，`y` 會無限重開空回合），
  但隨機遭遇不呼叫 `27h TREASURE`。下一次直接走貧民窟腳本裡那 7 處 TREASURE 的格子。

## 2026-09-18 上一輪的起點：現況斷言校準（#49 README 與自評、#51 對拍基準表漂移、#52 戰利品截圖）

三份現況斷言都沒有人拿去對現在的程式，錯的方向一律是「看起來保守」：README 還寫著主線沒連續跑過
（#5 已經跑完並逐段對照）、對拍的十項差是與 v.1.1.5 產的那張表比出來的、戰利品畫面補好了卻沒有截圖。
真正的產出是**對拍腳本加一道「與表比對」的檢查**——比較的對象要是表，不是上一次跑的結果。
goal：`docs/goals/issue-49-51-52-current-state-calibration.md`。
...
- 使用者的原話：「remake 需要實作 share 喔，另外 remake 密語 or 答案直接放在對話後面」。查下來兩件事都已經有規則或行為，缺的是畫面與開關：
  - **#47 戰利品畫面**：`ShareMoney`／`PoolMoney` 已對過 overlay-21，但畫面看不到金額、`View` 只把名稱串成一行（原版是人物頁）、
    `screenName()` 沒有戰利品的識別字。使用者選的是「戰利品畫面不好用」。
  - **#48 密語提示**：`ecl_input.go` 早就把答案接在問句後面，而且是一律顯示。改成 F6 作弊選單的開關，**預設開、可以關**，
    關掉不算作弊（不寫 `CheatsUsed`）；範圍只有打字的密語。
- goal：`docs/goals/issue-47-48-treasure-screen-and-password-hint.md`。
...
- dosgolem `pool-parity` 分支新增 `57454c4`（`-serve`），已推送（使用者 2026-09-17 同意）；不合併 main。

## 2026-09-17 主線收據的口徑改成作弊通關（使用者決定，#5）

- 使用者判定作弊驗證算成功，決定：
  - **算主線收據**：用產品的作弊選單（spec 141）從標題以正常按鍵跑到結局。
  - **原版那一側**：同樣作弊通關。dosgolem 是我們自己的工具，擴充成在原版裡鎖 HP，兩邊逐段對照必經區塊與旗標。
  - **不開作弊的正常強度通關**：改為可選（#22）。
  - CLAUDE.md §3 已寫入。作弊選單以外的傳送與注入仍然不算。
- #26 關閉（路線 (a) 驗證算成功）。#5 維持 open，等兩邊作弊通關與對照做完。
- 下一輪 goal：`docs/goals/issue-5-cheat-playthrough-both-sides.md`。

## 2026-09-17 作弊選單（鎖 HP、一擊斃命，預設關）；只在諾里斯開，6 個走到的 seed 全部打贏（#43）

- 使用者決定加作弊選單（spec 141）：`F6` 開，`L` 鎖 HP、`O` 一擊斃命，`ESC`／`F6` 關。打開過一次就寫
  `CheatsUsed`，存檔與畫面一直標「開過作弊」；開過作弊的路線不算原版驗收，也不算 #5 的收據。
- 一擊斃命只蓋隊伍造成傷害的兩個入口：`resolveTacticalAttack`（近戰、遠程、快速戰鬥）與 `applySpellDamage`
  （單體、範圍）。能量吸取與 ECL `DAMAGE` 只會打到隊員，不蓋。擲骰照常，只改結果。
- 鎖 HP 在 `Update()` 結束時寫回，與測試治具共用 `restorePartyHitPoints`；治具仍自己掛 `afterTick`，
  不寫 `CheatsUsed`。
- F1 說明頁本來就超出框（第 14 行壓在繩索框上、出處畫到螢幕外），改成兩頁：第 1 頁鍵說明含 F6，
  第 2 頁指令與出處，左右鍵翻頁。
- house rule 探針只在諾里斯那一場開作弊，掃 seed 136..150：
  - 走到諾里斯的 6 個全部打贏；
  - 之後 3 個停在回程的古托井地面、3 個死在索寇要塞中庭的巡邏；
  - 另外 9 個走不到諾里斯。

  表在 playtest 補十四，兩道牆都在 #22。
