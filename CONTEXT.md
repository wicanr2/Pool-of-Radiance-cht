...
- 285-byte 記錄的 `+9Fh` 是**生物種類**、`+6Ch` 是**體型加一個大塊頭旗標**。
  ...
  的從 54 升到 61。被迷住之後的行為仍是近似：只讓它不再行動，沒有倒戈——
...
- 同一份 spec 又解出 `0100h:007Ah`（overlay-24 entry 18）是「把力量往上調」，
  ...
  還缺六支：迷霧術、力量術、死靈術、解除魔法（兩個編號共用）、恢復術。
- `NEWECL FFh` 是「不換區塊」的哨兵，證據在 spec 107：八個封存檔沒有編號
  ...
  剩下那一格兩欄都是 `FF` 且沒有守衛。**修的位置在共用 engine，屬另一個
  repo，尚未動工。** 它是世界巡迴治具目前唯一還在冒的硬失敗
  （22 趟 7 次，走到 14 張地圖、12 個 ECL 區塊）。
