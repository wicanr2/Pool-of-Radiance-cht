...
- overlay-07 entry 3（`01C8h`）在 `[4959h] == 0` 時把 class 0 `4A00..4A1F` 與 class 1 `6E79..6E82`
  清成 0，`[4959h]` 非 0 則跳過並歸零（位元組 exact，spec 106〈載入區塊時清掉的兩段〉）。dosgolem
  對 `4A01` 設寫入監看：職員 `00→01`（`1997:0CBF`）、走出市政廳 `01→00`（`1997:02FE`）
  （`docs/audit/dosgolem-4a01-block-load-clear.json`，`tools/dosgolem-4a01-block-load.py`）。
- remake 的區塊切換不清這兩段，`4A01` 一路帶著：職員走 BACK SO SOON 交不了件、港務長 `9C4Bh`
  不開口。修正要動產品程式（Pool adapter 或 engine 載入 hook），開 #41。
- HP 鎖定診斷（#40，治具 `hp_lock_test.go`，seed 142）：貧民窟 25 場（含攤位那一場）、交件、職員
  列出波多廣場、陸路拍賣喬裝結案（不開戰）、打贏諾里斯都走通；波多與諾里斯的交件、港務長的票
  被 #41 擋住，探針以 `route (a) blocked by #41` 停下。補血 88 次，全在戰鬥中（playtest 補十二）。
- 鎖血時回合帳的勝負推斷不可信（隊員「倒下」後又滿血），勝負看 `6DC7`（0 贏、80h 全滅、81h 被擊退）。
- 已推翻：spec 106「class 0 開機清一次，不隨 `LOAD FILES`／`NEWECL` 重設」與「先進墳場就會把城門的
  馬車關掉——原版也一樣」；spec 102「引擎不動 `4A01`／換區不動它」「城區這一邊沒有人清票」
  「順序：先說謊，而且中間不能回城區」（這個限制只在 remake 成立）。判據是 overlay 的清除迴圈
  用 `add ax, 49FFh` 算位址，兩位元組字面值掃描結構上看不到它。
- spec 137 死路表的 `4A00`／`4A01` 四列改標「remake」，#41 修好後要刪。

## 2026-09-17 波多廣場的委任要先接；貧民窟就近進屋休息

- 古托井西緣 → 波多廣場是陸路（exact，`TestKutoWellWestEdgeLeadsToPodolPlaza`）；斯托揚諾河經野外 26
  小船與金字塔（換圖邊 exact、沒實跑）。但波多廣場的拍賣要 `4AB0 == 1` 才開（入口 4 `ecl1/18 9971h`），
  而職員要貧民窟、索寇、書至少交過一條才會列出它（`ecl3/8 A83Fh`／`AA2Ah`，spec 137）。
- 貧民窟入口 2 只比 `@6E82 == 0`，地形碼不為 0 的都睡得成；測試駕駛只挑不會開打的房間（`99C9h` 21 支
  控制流分類，spec 114），就近於旅店時進屋。house rule 15 個 seed 走到古托井的從 5 變 9，走到索寇仍是 1。
- 已推翻：「137／142／144 是白金用完」（補九）——白金在倒地的人身上，醒著的人付不出來。
- 牆（playtest 補十一，house rule seed 136..475）：諾里斯約 15% 勝率；19 個 seed 走到索寇 (8,5)，19 個全滅
  （A 二級、其餘一級）。索寇裡巡邏在、睡不成時改成帶著現狀往下打（`pressOn`）。收據 seed 143 → 142。
- 斯托揚諾河那條陸路：19 個自然隊伍零個走到金字塔（古托井地面倒地 8、蜥蜴人 3、野外 25 全滅／探索器預算 8）。
  探索器不朝小船走，這不是「路不通」的證據。合成的 60 HP 隊伍量測無效（遭遇隻數跟強度放大）。
...
- overlay-10 `1A99h` 在每場開打前用 `DS:304h` 五組列範圍填 2×4×6×11 的陣型樣板，
  `DS:45BAh` 是「現在在放哪一邊」；我方站原地城格、敵方站「遭遇距離 × 朝向」那一格；
  `1609h` 逐人從樣板中央一腿一列向兩側交替掃，第一腿上限 ⌈n÷2⌉，掃到出界換腿、兩個
  座標都出界換鄰格（spec 061）。`combat.PlaceCombatant` 照它重建，`provisionalRoster`
  拿掉。
- 遭遇距離是 overlay-07 `0489h`「從隊伍那一格朝前走、撞牆就停、最多兩步」，固定事件
  沒有選單，`24h COMBAT` 開打前再走一次、比較小才寫回（spec 078）。dosgolem 收據：
  單人隊貧民窟 (14,4) 面向西距離 0，原版隊員 (28,13)、敵人 (26,12)，remake 同狀態同值。
- 已推翻：「陣型樣板填寫者在 overlay-10 之外」——就在同一顆的 `1A99h`；「`45BAh` 是樣板
  組的選用條件」——它是逐筆的陣營。
...
- dosgolem `shots` 加 `-trace-peek`（每十萬道指令讀一次位置表，變了就記一幀），三筆收據
  多了逐步的 `actions`：一筆一步，分得出誰先動、每一步落在哪。原版一隻怪一輪內來回擺盪
  是 spec 096 卡住規則的第二次（忘掉目標、當場重挑再走），remake 的 `foeTurn` 照改。
  `TestFoeWalkReproducesEveryOriginalAction` 把三場 40 個原版動作逐一擺盤重走，32 個
  remake 走到同一格；剩 8 個是擺盪序列的第二段吃第一段的骰（換到哪個模式、重挑到誰）。
  8d 三場兩 seed：衛兵 136 第一次贏。
- 骰流讀出來了：dosgolem `shots -trace-call 5BB:C94` 記 Turbo Pascal `Random(n)` 每一次呼叫，
  三筆收據的 `dice`。一隻怪一回合的骰是「[沿用 d4] [d8 (d4|d2)] d7 d7 [d(n) 挑目標] 走
  [d(n) 重挑]」，n 是對面還站著的人數。照骰流擺盤重走，三場 40 個原版動作 **40 個**停在
  原版那一格；對到最後抓出三條規則差並改掉——一格能不能進還看步的成本（正 2 斜 3，
  `066Eh` 後半）、每輪開頭剩不到 2 點就收工（`084Dh`）、五個方向全不通只是這一輪不動、
  換了模式再試；候選名單依直線追蹤成本排、第一輪空了放寬地形再填（`37B8h` 兩輪制）。
  spec 096 單步移動與挑目標 CONFORMED。8e 三場兩 seed：獸人家 全滅／贏、衛兵 贏／全滅、
  驚動衛兵 全滅／贏。還沒讀：每回合模式與挑目標之間的 `d7 d7` 是哪一支（沒量到影響走位）。
- 陣型樣板從執行期讀出來對（#35）：dosgolem 四場開打那一幀的 `DS:43A2h` 528 bytes 與
  remake 部署完的樣板逐 byte 相同（`TestDeploymentTemplatesMatchTheOriginalAtRuntime`），
  `304h` 執行期等於 START.EXE；寫入端掃完只有 overlay-10 的 `1A99h`（填）與 `14CFh`
  （用掉一格清零）。衛兵那一場敵方 32 隻走到陣型 1（整塊）、獸人家與驚動衛兵走到陣型 2，
  換陣型的順序也對；陣型 3 沒踩到。spec 061 樣板一節 exact。
- 怪物 THAC0 的來源（#31／#36）：`MONnCHA` 樣板的 `+110h` 是殘值——43 筆亮 bit 7、42 筆恰好
  `+2Dh + 109`（諾里斯 154 ＝ 45 ＋ 109）。開打初始化 overlay-10 `1380h` 對每一隻叫
  overlay-25 entry 7（`0BBEh`）重算，`0E65h` 把 `+2Dh` 抄進 `+110h`、沒武器再加 `12AEh` 的
  力量修正（它自己查 `+0AAh`，exact）；dosgolem 獸人家二十隻執行期 `+110h` 逐隻等於
...
是否存在及其幾何／色盤，不能把使用者觀察直接升格成 exact。issue #17 登記以 DOS
內容與構圖為基準的全套現代奇幻 sprite 重繪；依既有產品契約，它是可切換表現層，
不覆寫原版忠實主題。#17 開工先盤點真實 archive／consumer，再以可丟棄 prototype
逐題確認視覺聖經、忠實度、資產邊界與產製／授權方式。
...
付費過城門只需 0.25 秒；分段後確認 rotate 3 已取得 ECL block 5／7、離開城堡
回到 block 0 後，仍繼續掃整張城區，單輪耗掉 166.69 秒。測試現在只保留會增加
...
`v.1.1.4-20260913`。新增 camp-quit、Sune 神殿、菲蘭武具店、人類牧師的
神術一級法術頁、裝備頁，以及地圖施法的挑人／法術兩層；平面圖與其餘八個
camp 狀態已在上一批完成，因此使用者指定的自動化清單已清空。

神殿與商店都由正常 GEO3/0 走路鍵進入；法術鏈另開新局正常建立人類牧師，
記憶祝福術、紮營兩小時，再從地圖按 C。裝備頁與地圖施法挑人是 remake
表現層，只作 `layout-only` 參考，不宣稱原版一致。每個隔離基準都具
`provenance.json`，對拍前逐組驗 `generator=dosgolem` 且原版
`start.exe` SHA-256 相同。現況與 35 項數字只認
`docs/audit/dos-parity-sample.json`／`.md`。

尚未抽樣的結局依賴 issue #5 的人工主線實跑；`view-pick` 與
`menu-drop-confirm` 沒有原版同一畫面。三者不屬 issue #8 這次的自動化範圍。

## 2026-09-13 對拍抽樣加入紮營 REST

issue #8 的抽樣由 20 增為 28 項。dosgolem 主鍵序在既有 camp 畫面後送 `R`，
原版基準為 `64-r`、SHA-256 `127e40a3…`；remake 由發行 AppImage 的正常
按鍵路徑進入 `camp-rest`。量測結果為整張 79.27%、外框 94.04%、視野
100.00%。同一條正常按鍵路徑再加入 ALTER、SPEED、PICS、ICON、ORDER 兩步與
DROP；各自有不同的原版內容雜湊。新增七項整張為 78.63%～91.11%、外框為
94.04%～96.46%，帶第一人稱視野者維持 100% 或已知營火另一幀的 91.27%。
本輪使用符合正式版號契約的本機包 `v.1.1.3-20260913`；其餘未抽樣畫面仍保留
在 worklist，沒有把滾動清單提早關閉。
...
快照契約處理重疊力量效果，地圖與戰場到期都會同步還原角色及角色庫。
`C` 鍵施放變大術的玩家路徑測試確認 10 回合後力量 18→10；原版 138 格、
目前 production 節點、替代清除路徑與尚未 materialize 的效果碼分類在
`docs/audit/effect-teardown-coverage.json`。

## 2026-09-13 世界探索會嘗試打開鎖門

世界巡迴治具不再一律選 EXIT；每道門會依序用真實按鍵嘗試 BASH、PICK、
KNOCK，失敗才離開。治具具備一名賊與一格 Knock。固定 ECL seed 1 的同測試
前後量測為地圖 21→22、ECL block 19→19、委任 5→5、80.29→55.13 秒；
收據在 `docs/audit/explorer-door-coverage.json`，沒有觸發 #10 的耗時阻塞。

## 2026-09-13 遊戲內攻略加入貧民窟

`F3` 攻略現在除了 `GEO3/0` 文明區，也涵蓋 `GEO2/20` 貧民窟。新增六個點
都由原版資料建立：dosgolem 正常入口實測的 `(14,4)`，以及 ECL2/20
`B6F2h`／`B6F7h` 五組逃跑落點 `(15,4)`、`(14,6)`、`(11,5)`、`(11,2)`、
`(9,2)`；沒有採用第三方攻略座標。繁中／英文目錄的座標與來源一致。
...
`ECL7/block 17` 那一處 `unknown opcode 0x9D at payload offset 1085` 找到了。

`34h ECL CLOCK` 是 **passthrough**（引擎沒有核心處理常式，由前端接），而 VM 的
passthrough 分支用 `ecl.RecordEnd` 算下一條指令的位址——**那一支沒有指令表參數**，
一律走二手表。於是同一條指令有兩套長度：內容用 Pool 的表解（一個運算元）、
位址用二手表算（兩個運算元），PC 多跳三個位元組。

```
payload 1075..1090   01 FF 9C 13 34 00 01 02 01 A7 9D 03 01 82 6E 00
                                 ^1079 34h ECL CLOCK
Pool 的表    → 下一條在 1082（02h GOSUB）
二手表       → 下一條在 1085（9Dh，不是 opcode）
```

**症狀延後發作，這是它難查的地方**：那一條照樣執行完、事件照樣送出去，PC 卻停在
指令中間，要等到走到下一條才冒出 `unknown opcode`。報出來的位置離成因三個位元組，
而且只有走到那一條 GOSUB 的路徑才會踩到——所以它一直表現成「隨機探索偶爾撞一次，
重跑就消失」。

同一族還有兩處漏的：engine 的 `RestoreSnapshot` 重建 machine 時搬了投影器與
隊伍強度解析器、**漏了 `commands`**；Pool 的 `NewDOSECLArchiveSession`（讀檔那條路）
沒有 `SetCommands`。兩者都會讓還原之後整個 session 退回二手表。

**規則：解碼用的表要跟著走完整條路徑。** 只要有一個地方退回預設，出錯的就不是
那一條指令，而是**它之後的每一條**，而報出來的位置離成因很遠。

## 2026-09-11 鎖住的門按不動——一份狀態同時當「記憶」與「正在等輸入」

隨機探索硬失敗一次（GEO5/4 `(13,3)`，遭遇選單 `[COMBAT WAIT FLEE PARLAY]` 停在
游標 0、狀態列 `Locked. BASH EXIT`、**這一格答過 0 次**）。「答過 0 次」是那條
線索：治具連一次都沒能把選擇送出去。

追下去發現的是**兩個**缺陷，同一個根因。`a.door` 是持久狀態——撞不開的門要
留著才接續得了 `Bash`／`Pick` 的額度——而輸入分派把「門的狀態還在」當成
「門選單正在等玩家回答」：

- **撞門的當下走不到。** 那一段本來在 `cellEventPending` 區塊**裡面**，而
  `beginDoorMenu` 不設 `cellEventPending`。選單畫得出來、狀態列印著
  `Locked. BASH EXIT`，方向鍵與 ENTER 卻全都落到轉向與前進——**鎖住的門根本
  開不了**，而這是玩家一定會撞到的。
- **門留著的時候把別的選單吃掉。** 它排在格子選單前面又直接 return，所以腳本
  在同一格排出遭遇時，方向鍵被門吃掉、ENTER 又跑回去撞門，兩層一起卡死。

現在的順序是 `cellEventPending` → 門 → 轉向與前進：有格子事件時門不吃鍵，
沒有的時候門是 modal 的（`EXIT` 一定在選項裡，關得掉）。順帶補上游標顯示——
五處狀態列原本都是 `strings.Join(Options, " ")`，移動游標之後字串一模一樣。

**教訓寫成規則**（spec 122 也記了一份）：一份狀態同時表示「記憶」與「正在等
輸入」的時候，輸入分派會用錯它。門的狀態要跨多次撞門留著，而選單只在最上層
時該吃鍵，兩件事共用 `a.door != nil` 這一個條件，於是放哪裡都錯。

**單元測試抓不到這一族。** 門那四條測試都直接呼叫 `resolveDoorMenu`，繞過輸入
層——規則對，而玩家按不動，報表上兩者長得一樣。新的兩條從 `Update()` 送按鍵
進去。探索治具也跟著要處理門（門變 modal 之後不處理就會困在原地），目前一律
選 EXIT，所以門後的區域仍然沒有覆蓋，記在 worklist。

**門在走成一步之後過期**（`moveInitialDungeonForward`）：`a.door` 持久是為了
接續 `Bash`／`Pick` 額度，而「同一道門」的前提是中間沒有走成任何一步。少了
這一步，門會跟著隊伍跨到別的格，在那裡與腳本事件疊在一起。

**修好門之後還差一道守衛**：`cellTextSticky` 那一段（腳本跑完、字留在框裡時
ENTER 翻手冊，spec 132）沒有讓路給門，而它**只檢查 ENTER／SPACE**——症狀是
門的游標移得動、ENTER 沒反應，看起來完全像門自己的缺陷。下面那個翻手冊的入口
早就有完整守衛（遭遇、神殿、交涉、`WHO`、格子選單），這一段漏了門。

這個缺陷一直沒被踩到，正是因為門的分派本來就走不到那一段——**修好第一個
缺陷，才第一次走到會暴露第二個的路徑上**。

**查它的過程本身是一課**：整合測試要 80～1200 秒、而且只給得出「卡住了」，
靠它猜了四輪全錯；最小重現 0.2 秒就把變因收斂到一個布林（`cellTextSticky`）。
**症狀隨參數改變方向的時候**（guard 給小是走不到區塊、給大是逾時，兩者指向
相反的修法），那就是該停下來寫最小重現的訊號。

**連帶炸出九個死迴圈**：`stojanow_gate_test.go` 七處、`wilderness_explore_test.go`
兩處 `for spawn.Facing != 目標 { press(右) }`——門開著時右鍵移的是門選單的游標，
`spawn.Facing` 一個都不會動。症狀是城堡那條測試從七分半變成「19 分 25 秒還在跑」
然後整包逾時，**看起來像變慢而不是卡住**。九處改走 `faceTowards`（先關門、帶
guard），`answerCellMenus` 的 `busy` 也把門算進去。

## 2026-09-11 賊技能建角端接上；核實抓到四條做完卻還掛著的缺口

**八格賊技能的算式讀完了**（spec 095）。建角在 overlay-16 `0CA7h` 判
「賊等級 > 0」之後呼叫 overlay-23 entry 4（`031Eh`），那是一個 `i = 1..8`
的迴圈：`記錄[+76h+i] = 基礎[等級×8+i] + 種族[種族碼×8+i]`，`i ≤ 5` 再加
`敏捷[DEX×5+i]`；種族修正為負且絕對值大於基礎就夾到 0。

**卡住的那一步是敏捷**。照公式算，三個原版樣本全部差 `+5 +10 +5 0 0 0 0 0`。
差額的出處不是第四張表——**建角那一刻記錄裡的 DEX 還是 0**，所以敏捷套的是
第 0 列（`3CC6h + 1`，正好是 `+5 +10 +5 0 0`）。填記錄的順序是先職業與等級、
後能力值：entry 4 在 `0CAFh` 就呼叫完了，而 overlay-16 寫能力值那一段全部落在
`1206h` 之後。

**兩個樣本自己交叉證了這件事**：矮人賊 DEX 11、人類賊 DEX 13，敏捷表那兩列
差 `-5 0 -5 -10 0`，而實測的八格沒有那個差。帶 0 進去三個逐格吻合，帶真實
敏捷進去三個全錯。**沒有靠「形狀相符」下結論**——`+5 +10 +5` 同時也長得像
AD&D 的 DEX 17 那一列，那一條路是錯的。

`i` 從 1 起這件事要記著：三張表的索引都是「基底 ＋ 列寬 × 序號 ＋ i」，拿
`i` 從 0 去讀會整排左移一格，讀出來的**還是像表**（相鄰列的尾巴接得上），
但每一列都錯。三張表在 `internal/gamepack` 裡是攤平存的，因為**基礎表等級 9
那一列就是種族表種族碼 0 那一列**，拆成三個陣列會把重疊弄丟。

**升級那一端還沒接**：賊在訓練所升到二級，八格不會跟著長。原版那時候記錄裡的
敏捷是有值的，先查清楚再實作。

**順手核實了 README 的缺口清單，抓到四條早就做完的**：休息被打斷（`camp.go`
檔頭還寫著「還沒接」，而打斷、ECL 入口與那兩個參數的來源全都接上了）、遠程
武器的射程（`cast.go` 用 `moverAttackRange` 擋超距）、結局的圖
（`loadEndingScene`）、說明書附錄的規則表（七節都在 journal 裡）。
**四條的形狀相同**：缺口是在別的檔案補完的，補的人改了那一支，沒有人回頭看
總覽段落。所以核實要從總覽往程式走，不是反過來。自評因此從 60～70% 挪到
65～72%——往上挪的主因是原本高估了缺口，不是這一版做了很多。

## 2026-09-11 效果串列接上時間：進出戰場都帶著，時間到了會過期

原版的效果串列**只有一份**，掛在角色記錄的 `+7Fh`（spec 069）——戰場上與
地圖上讀的是同一條，所以「進戰鬥要帶入、出戰鬥要寫回」在原版根本不存在。
remake 把它分成兩個型別存（存檔 `poolsave.EffectNode`、戰鬥
`gamepack.EffectList`），於是兩個方向都得自己搬：`combat_effects.go` 的
`combatEffects`／`storedEffects` 是搬運，`storeCombatEffects` 在收場時寫回，
角色庫那一份跟著同步（原版沒有這個問題，`.CHA` 與 `.SPC` 就是同一份檔案）。

**語意只定義在 `gamepack.EffectNode` 上**，存檔那一份純搬 bytes。存檔的節點
從只有效果碼（schema 7）換成完整的 9 bytes（schema 8），舊檔的碼讀成持續 0
的節點——0 在原版就是永久。

時間這一側原本整個是空的：走一步只加時鐘，紮營連時鐘都沒推。現在兩個呼叫點
都走 `advanceGameTime`，也就是原版把時鐘與效果寫在一起的那支
（overlay-20 entry 2）：換算成分、十分鐘一批、持續 0 跳過、經過量到了就摘掉。
**紮營會把睡掉的時間走到世界時鐘上**（一刻五分），原版量到的是同一個數字——
貧民區被城衛隊打斷的那一刻是 `0, 4 W 00:05`，基準畫面在
`docs/reference/original-dos/camp/`。

還缺的是到期那一刻的收尾（原版 `0100:002A`，worklist 的
`effect-expiry-teardown`）：現在只把節點摘掉，不做別的。

## 2026-09-09 手冊引用直接翻過去；視野框的下緣被對話框蓋了很久

**文字報了手冊編號，按繼續那一下就翻到那一則**（spec 132）。原版把《探險者
手冊》印成紙本，畫面上只寫得下「成為線索報導 46」；手冊既然已經收進遊戲，
就沒有理由再要玩家按 `J` 自己查。

盤點引用時抓到兩件當初漏掉的：

- **不是只有線索報導。** 市政廳外牆引用的是**議會公告**、酒館引用的是
  **酒店傳言**。只認線索報導的話，起始地圖走幾步就到的那一處按下去沒反應。
- **一段話引用多則是常態。** 市政廳外牆一次報四則（`LXIV、LXXVIII、CIX
  與 LIX`），另有兩處卷宗一次報兩則。所以引用要當佇列。

順著實拍看出兩個版面缺陷，都是「被蓋住」而不是「畫錯」，所以不容易發現：

- **第一人稱框的下緣繩索一直被對話框切掉。** 視野是 176×176，但外面還圍著
  一圈繩索，所以那一框真正的下緣是視野下緣再加一個 tile。先前照視野下緣排
  對話框，上框線就從繩索中間穿過去——畫面上看起來像「這一框本來就沒有下緣」。
  修法是把視野往上移一個 tile（`viewTop` 86 → 70），對話框留在 264。
  **`intro` 對拍因此從 78.50% 升到 79.65%**：原版那一幕的文字框上緣在
  native 129，logical 258，264 比 280 更接近。
- **面板蓋上來時對話框整段不能畫**（`showDialogue`）。框會被面板蓋住，
  框外那一列頁尾卻不會，於是兩份頁尾疊在同一行，看起來像字型壞了。

**remake 的視野原點在對拍腳本裡只有 `REMAKE_VIEW_TOP` 一處**，報表裡也只有
dosgolem 一組斷言（AGENTS.md §7）。這兩件事同源：同一個量測寫死兩份、或同一
個畫面留兩組數字，症狀都是「其中一份悄悄不對了，而報表看起來仍然自洽」。

## 2026-09-09 dosgolem 的方向鍵沒壞，是原版的讀鍵層不收擴充鍵

追「dosgolem 送方向鍵到建角選單沒有作用」，逐層排除：

1. **鍵送達了。** 在 `int 16h` 加軌跡：每一次 `Down` 都是
   `AH00 AX=5000 at=0622:031C`——掃描碼 `50h`、ASCII `00h`，正是 BIOS 的
   方向鍵字組，呼叫者是 Turbo Pascal RTL 的 `ReadKey`。
2. **程式讀走了。** `KeysConsumed` 每一步都加一，沒有「一個鍵都沒讀走」。
3. **畫面一個像素都沒變。** 逐幀比對五次 `Down`：`0, 0, 0, 0, 0`；
   接下來那一下 `Return` 改了 990 個像素。

所以問題不在注入，在原版怎麼讀。反組譯 `START.EXE` 的讀鍵層
（`026B:0048`，IDA 認出 Borland 符號 `@READKEY$qv`／`@KEYPRESSED$qv`）：

```
ch := ReadKey
…（Ctrl-S 切音效、STING 這個命令列參數的兩個除錯鍵）…
loop: if ch = #0 then Result := ch          ← 擴充鍵就這樣回去了
      if not KeyPressed then Result := ch
      ch := ReadKey; goto loop              ← 把緩衝區剩下的鍵吃掉，留最後一個
```

**擴充鍵回 `#0`**，呼叫端得自己再叫一次才拿得到掃描碼；而那個「吃掉剩餘鍵」
的迴圈會把第二段丟掉。整個遊戲只有這一個讀鍵入口（`START.EXE` 三處、
overlay-09／21／26／30／35／37 各一兩處），沒有別條路。

**實用的結論：選單裡要移動反白就送 `1`。** 五次 `1` 讓種族清單的反白從
`DWARF` 一路走到 `HUMAN`（逐格，量得出來）；`2`..`9`、`P`（`50h`，Down 的
掃描碼當成字元）、`Space`、`Tab` 都沒有作用。

找到選單元件之後（overlay-26 `00E4h`，靠 `-keytrace` 把三個鍵回溯到同一個
`1E03:02D9` 再對回 `02D4`），這件事解了一半：**主鍵盤的 `1`..`9` 是方位鍵**。
集合 `0C4h` 就是 `123456789`，經 `DS:289Eh` 的表轉成數字鍵盤那一圈的掃描碼
（`4F 50 51 4B 20 4D 47 48 49`，排法與實體位置相同）。擴充鍵那一路讀得到
第二段，但**不比對掃描碼**，只在 `[bp+0Ch]≠0` 時設旗標。全部寫進 spec 133。

再往上追一層就解開了（`-keytrace` 現在回溯兩層，第三層是 overlay-26
`0A41`）：**清單使用端只比對 `47h`／`4Fh`／`49h`／`51h`**——Home、End、
PgUp、PgDn。`48h`（Up）與 `50h`（Down）一次都沒出現。所以

> **原版的清單用 Home／End 上下移動，不是 Up／Down。**

實測收尾：`End` 五次讓反白從 `DWARF` 走到 `HUMAN`、`Return` 進性別選單；
`Home` 一次上移一格；`Down` 仍是 0 個像素。dosgolem 的擴充鍵注入**本來就是
對的**，之前只是送錯了鍵。

**dosgolem 沒有 bug**，實驗性的「阻塞式 `AH=00`」（把返回 IP 退回兩個位元組
讓 `int 16h` 重跑）也不會讓方向鍵生效，已還原。它缺的是**量測能力**：
`KeysConsumed` 這個計數對「程式吃掉了但不理它」與「鍵根本沒送到」給一樣的
答案。那一份已經補上（dosgolem `docs/spec/185`）——`KeyReads` 現在記完整
字組、走哪一條出口、呼叫端的返回位址，`cmd/shots -keytrace` 逐步印出來；
`namedKeys` 也補了 `KP0`..`KP9`、`KPDot`，因為數字鍵盤的四個角與正中央
（`47h`／`49h`／`4Fh`／`51h`／`4Ch`）本來沒有名字可用。

有了 `1` 就建得出人類牧師，也走得到冒險畫面；但一級牧師沒有記憶法術，
按 `C` 仍走前置檢查那條路，所以**原版的施法清單畫面還是沒拍到**——要先
紮營記憶法術，那是另一段鍵序。
...
原文有沒有空白，於是中英夾雜的譯文被接成 `G移動 ENTER結束回合 D延後`。
那個推斷本來只是為了修 `PRESS RETURN OR BUTTON` 被接成一串，「全形字兩側
不補空白」是它的副作用，卻被寫成測試的斷言。現在切詞時就把「前面有空白」
記在 token 上，組行時照著補——**原文有就有，沒有就不加**。

中文可以在任何兩個字之間斷，所以按寬度折會折在「結｜束」中間。資訊欄那種
窄欄位改成**訊息自己帶換行**，`\n` 分段之後再各自按寬度折。

順手清掉三處畫在玩家畫面上、但其實是 remake 對自己的狀態說明：

- 戰鬥畫面資訊欄的「暫定：敵方 AI、部署位置、隊伍傷害骰」→ 移到 `F1`
  說明頁（`adventureProvenanceLines`）。**`combat` 對拍因此 77.58% → 77.76%。**
- 裝備頁的「徒手：原版此時不動傷害骰。」→ 只留「徒手」，那條 RE 結論搬進
  `equipment.go` 的註解。
- 商店狀態列的「原版商店服務：架上 N 件。」→ 「架上 N 件。」

同一類的還有一處**沒有**清掉：`BudgetSource`（`ui.budgetPlaceholder`
「暫定值」）只被設定、沒有任何顯示端，是死欄位不是畫面問題。

## 2026-09-08 戰場的地形圖塊是三個 `*COM.DAX`

戰術地圖是戰鬥開始時生成的（spec 060），**鋪在每一格上的圖不是**。三個
`*COM.DAX` 各是一組 24×24 的圖塊，正好一格，也正好與戰鬥造形同寬：
`DUNGCOM.DAX` 25 個、`WILDCOM.DAX` 34 個、`RANDCOM.DAX` 6 個。畫出來就是
原版那個樣子——地城的灰色碎石與黑色地板、野外的樹與水與仙人掌。

地圖裡存的是**格位類別碼**，中間隔著類別表（`DS:2758h`）第四個欄位
`PresentationCode`，那個值才是圖塊集裡的序號。66 筆類別分兩段，
**0..31 是室內、32..65 是野外**：室內段的序號正好蓋滿 DUNGCOM 的 25 個加
RANDCOM 的 6 個，野外段正好蓋滿 WILDCOM 的 34 個，三個檔一個 item 都不多
不少。切點是這樣推出來的，不是從程式碼讀的（spec 131）。

三個檔都**不遮罩**：地城地板整格就是色號 0，當成遮罩色會讓地板變透明。

最乾淨的一筆對照：室內地板類別 `17h`（`IndoorFloorBuilderClass` `16h` 加一）
的序號是 `16h`，那個 item 整格 576 個像素全是色號 0——原版地城戰鬥的地板
就是純黑。

`F4` 因此變成四頁，最後一頁是這三組圖塊。

## 2026-09-08 起始地圖走不到戰鬥，這是缺口不是腳本

截圖腳本想拍一張「有敵方造形的實戰畫面」，走四百步拍不到。原因不在走法：
起始地圖 GEO3／0 的 1024 個「座標×朝向」樣本各跑一次格子生命週期，
**沒有任何一格走到 `24h COMBAT`**（`docs/audit/pool-initial-cell-sweep.json`，
boundary 全部是 `exit` 或 `event`）。斯倫特貧民區的架在原版是遊蕩遭遇，
而 remake 還沒有遊蕩怪物群（WORKLIST 第一層）。

**順帶記一件容易踩的事**：冒險畫面的方向鍵是「左右轉向、上前進」，
不是四方向移動（`main.go` 的 `ArrowLeft`／`ArrowRight`／`ArrowUp`）。
輪流按四個方向的話四分之三的按鍵都在原地轉。

## 2026-09-08 怪物在戰場上的造形是哪一份

追「怪物總覽」時繞了兩圈，結論值得記下來：
...
## 2026-09-08 戰術視窗開場對到行動者；`F4` 素材總覽

戰術畫面的視窗先前停在 (0,0)，而部署好的隊伍在別的地方——盤面看起來是空的。
原版重畫視窗時中心一定跟著行動者（overlay-32 `07D4h`，餘裕 0 一定捲到正中央），
現在進畫面就捲一次。截圖裡看得到隊員了。

`F4` 是新的素材總覽：肖像、戰鬥造形、牆面圖塊、外框符號各一排。**它是遊戲
畫面，不是素材匯出**——圖形只從玩家自己的原版 ZIP 讀出來，repo 不含也不產生
素材檔，與 `cmd/pool-combat-icon-audit` 那一行「never exports source art」
是同一條界線。

## 2026-09-08 人物資料頁補齊原版的欄位

原版那一頁（spec 130，量自 dosgolem 的第 16 幀）有十三行：性別／種族／年齡、
陣營、職業、六屬性、`LEVEL`／`EXP`、`AC`／`THAC0`／`ENCUMBRANCE`、
`HP`／`DAMAGE`／`MOVEMENT`、`STATUS`，右上角還有肖像。remake 先前只畫得出
前半，那八個欄位在別的畫面算得出來、**是這一頁沒顯示**。

現在每一行與每一欄的 x 都對原版（native 座標乘二，基線加 ascent 14），
欄名保持英文——與隊伍面板的 `AC`／`HP` 同一個處理。值的來源：`AC` 與 `THAC0`
走記錄的內部值再換算回來、負重走 `CarriedWeight`、傷害是徒手 1d2 加力量的
傷害調整、移動力走 `MovementRateFor`。對拍 `sheet` 從 85.16% 升到 86.63%。

**踩到同一個坑兩次**：`memberDefenceStats` 的 `baseArmor` 吃的是**內部值**
（`60 − AC`），傳 10 進去會算出「AC 50」——戰鬥畫面那一輪也是同一個誤讀。
兩處都改好了，常數旁邊寫明它是內部值。

最下面那一列 `KEEP THIS CHARACTER? YES NO` 由這一頁自己畫，全域的 F-key
提示在這個 stage 不畫；先前兩邊都畫，底部疊成一團。

## 2026-09-08 探索畫面按 C 施得了法

指令列的 `C` 先前完全沒有分派——`openCastMenu` 的唯一呼叫端在 `tactical.go`，
所以**施法只在戰鬥中做得到**，打完一場架補不了血、解不了毒。

原版走 overlay-15 entry 2（`0497h`，spec 119 已補完並改成 READY）：挑人與挑
法術是同一個介面，選中之後交給 **overlay-22 entry 5**——**與戰鬥中是同一支
派發**（spec 073 那張 67 格的表）。所以 remake 這一側也共用同一份清單與同一個
`SpellIsImplemented` 閘，看得到的就是施得出來的。施完一條回到挑人那一步，
可以接著施下一條（原版 `054Ch` 跳回迴圈頂）；挑到沒記法術的人會說
`<名字> has no spells memorized`（`047Fh`）——那是正常回應，不是錯誤。

戰鬥外沒有戰術格，所以只結算作用在人身上的那幾種（治療、把生命值墊到下限、
拿掉效果）。其餘照實說一句「要在戰鬥中才有目標」，不假裝施出去了。

**順帶修好一個保護測試的盲點**：`TestTranslatedFormatStringsTakeTheSameArguments`
只認 `%s` 這種順序 verb，不認顯式索引 `%[2]s`。中文與英文的語序常常不同
（「A 對 B 施了 C」對上 `A casts C on B`），顯式索引正是為此而存在；把它當成
沒有 verb 會逼譯文遷就英文語序。改成比**參數集合**：兩邊要用到同樣的第 n 個
參數、同一個參數的動詞相同。改壞一條譯文驗過它仍然抓得到。

## 2026-09-08 戰鬥畫面照原版重做，外框 100%

原版的戰鬥畫面**不是冒險畫面那一套框**（spec 129，量自 dosgolem 走到第一場
遭遇的第 49 幀）：外框下緣在 tile row 22 而不是 23，第 22 欄還有一條直的，
把畫面切成左邊 7×7 格的戰場與右邊的資訊欄，最下面那一列指令仍在框外。
remake 先前畫的是自己的 50×25 全圖加四行狀態。改成原版那一套之後外框
9152／9152 ＝ 100%（整張 53.78% 是預期的——原版那一側是實際遭遇，
remake 這一側是 `F5` 的同一支繪製，盤面內容本來就不同）。

**指令列是算出來的。** overlay-08 的 DS 區有六段 Pascal 字串，`0686h..07D4h`
逐段判斷要不要接：`Use ` 看記錄 `+0C7h`、`Cast ` 掃 `+17h` 起 21 格有沒有記著
法術、`Turn ` 看 `+96h` 的牧師等級。所以第 49 幀那個戰士只看得到
`MOVE VIEW AIM QUICK DONE`。照抄說明書那八項會讓玩家看到按不動的鍵。
掃描基底 `+17h` 與 spec 094 的 `3Bh SPELL` 是同一個，兩條獨立解出來的路
落在同一處。

**視窗是 7×7，不是 6×6。** 迴圈上界是 6，跑的是 0..6；戰場區內部 168 像素
÷ 每格 24 ＝ 7，而中心在原點 +3 對七格正好是正中央。先前 `ViewportTileSpan`
寫 6，已更正。

**順帶修掉一個量測定義的問題**：`frame_cells` 把外框的直行整條算到畫面最底，
於是框下面那一列（指令列）也被當成框——戰鬥那一項第一次差的 72 格全部落在
那七列，一格框線都沒差。改成直行只算到框的下緣之後，另外七張畫面的外框
也回到 100%。同一輪把指令列的起點對齊原版（native x 0，先前是邏輯 20）。

## 2026-09-08 對 code 核實一次「還剩什麼」

[`WORKLIST.md`](WORKLIST.md) 開頭那一節改成三層：玩家會撞到的功能缺口、
驗證缺口、版面與資料的差距。壓住自評的主要是第一層，而先前那份清單一條都沒列到
——它記的是「拿原版當裁判驗過沒有」，漏掉了「原版有、remake 沒有」。

**程式註解裡的「還沒接」跟 worklist 的 `[ ]` 一樣會過期。** 這一輪核實抓到三處：
`spells.go` 說「施展還沒接」（`cast.go` 已經 743 行，戰鬥中的施法、瞄準、豁免
都在跑）與「remake 還沒有紮營畫面」（`camp.go` 早就有），`cast.go` 說「還沒有
瞄準那一層」（瞄準層有了，只是解病術那一支沒吃 `target`）。三處都改寫成現況。
**盤點缺口要 grep 呼叫端，不要讀自承**——`openCastMenu` 的唯一呼叫端在
`tactical.go`，這一件事光讀註解看不出來。

## 2026-09-08 底部指令列移到框外；遊戲內攻略（F3）

**底部那一列移到繩索框外面**，與原版同一條（native 192–198）。原本畫在框內
的 366，現在 `footerBaseline` 是 398——外框下緣佔 368..383，底下 384..399
整條本來就是空的，倚天字型 ascent 14／descent 1 剛好填滿。指令列、功能鍵列、
法術頁、裝備頁、手冊、商店、戰術頁的最後一列全部走同一個常數。
`TestTacticalTextStaysAboveTheFrame` 的契約跟著反過來：前三行在框上面，
最後一行整條要落在框**外**那一帶。

**遊戲內攻略（`F3`）**，做法比照 CoAB 的 `guide_overlay.go`：平面圖上標地點、
右邊列離隊伍最近的幾個，**預設只顯示走過的格子**，按 `V` 攤開（第一次只出
警告）。資料在 `internal/guide`，隨執行檔嵌進去。**開頁時先把腳下那一格記成
走過的**——只在「移動之後」記的話，剛進城還沒走就開攻略會看到一張全空的圖，
那看起來像沒有資料，不像迷霧。

**座標一律出自原始資料**：`cmd/pool-world-graph` 的 `cell_terrain` 配
spec 102 的索引表——城區每一格「這裡有什麼」是 `terrain & 0x7F` 查一張
`ON GOTO` 表，所以哪一格是哪一間店可以直接從原始 GEO 讀出來。名字取自原版
腳本自己印出來的第一句話。**第三方攻略只當檢查清單**（該有幾個地點），
敘述一律自己寫——抄座標的症狀是「測試綠、玩家走不到」，
`TestEveryGuidePointIsOnAWalkableCell` 是那條的閘門。

**同一個坑第二次**：攻略那兩張截圖第一次拍不出來，腳本停在 `guide` 等不到
`guide-full`。看起來像 `V` 沒接上；實測（同一支腳本、同一個容器、按四次 `V`
逐次印狀態）證明 `V` 每次都到，掉的是兩次 `pulse` 之間那一下——腳本在那兩步
**又退回盲按**了。第一次按 `V` 只換底部一列字，`screenName()` 分不出來，於是
沒得等。現在多回報一個 `guide-warned`，兩步都改成 `step`（按到狀態對為止），
`TestGuideStartsFoggedAndNeedsTwoPressesToOpenUp` 一併鎖住這三個識別字。
**新畫面加了狀態就要一起加識別字**，不然下一支腳本又只能賭。

順帶收掉一個先前漏講的：**F1 說明頁整頁是英文寫死的**。之前說「UI 補完」
時沒算到它。現在走譯文表（`ui.helpKeys`，換行分隔），字型稽核也把
`internal/guide` 納進來——上一次漏掉的是 locale 表，而漏掉的時候那支工具
照樣回「沒有缺字」。

## 2026-09-08 建隊到進城逐張對過原版，抓到一個「整個右半邊留白」

dosgolem 的 master 併進十條分支之後畫得出第一人稱視野了，於是把對拍從兩張
擴到十三張，走完建隊到進城那一段，**英文模式跑**（把語言從變數裡拿掉）。

**框線是對的。** 繩索外框在原版是四條 8 像素帶（上 0–7、下 184–191、
左 0–7、右 312–319）；remake 的畫布是兩倍，外框也是兩倍寬的同一圈，降回
320×200 之後逐格落在同一個位置——種族／性別／職業／陣營四張是 8064/8064。
其餘幾張的差都是別的東西畫進那一圈：原版的底部指令列在框**外**、remake 在
框內，而 remake 多一行 `SSI GOLD BOX / POOL REMAKE` 標題。

**抓到的缺陷**：按下 `B` 之後羅夫講話的那一幕，remake 把整個右半邊留白——
`drawAdventure` 畫完半身像就提早返回，跳過隊伍面板。原版那時 `NAME AC HP`、
`HERO 10 8` 與 `15,1 W 00:00` 都還在。**那是玩家開始冒險看到的第一個畫面。**
已修，並加了負對照過的回歸測試（`TestApproachPortraitKeepsThePartyPanel`，
靠 `drawnText` 這個繪製接縫問——`(*ebiten.Image).At` 在遊戲迴圈外會 panic）。

**還沒補的三項**（帶驗收條件在 [`WORKLIST.md`](WORKLIST.md)）：人物資料頁少了
`LEVEL`／`EXP`／`AC`／`THAC0`／`ENCUMBRANCE`／`DAMAGE`／`MOVEMENT`／`STATUS`
與肖像；肖像編輯器原版是在整張資料頁上換；戰鬥造形設計原版有 OLD／NEW 四格。

**這一輪學到的量測教訓**：整張比例會因為做對事情而下降——補上隊伍面板之後
那一張從 50275 掉到 50201，因為它畫在 remake 自己的座標上。
**那一欄是版面差異的量度，不是品質指標**，報告裡要標清楚。
...
**還剩什麼**：自評挪到 60～70%。壓著它的東西在
[`WORKLIST.md`](WORKLIST.md) 開頭，2026-09-08 對 code 核實後改成三層排列。
...
靜態讀不到；填寫者與逐人放置在 2026-09-16 讀出（見該節），remake 照它擺。
...
## 2026-09-14：人物資料頁肖像框已對齊原版

- dosgolem 證明肖像內容為 native 88×88、左上角 `(224,8)`；框的左緣在
  `x=216..223`、下緣在 `y=96..103`，上、右兩邊與整頁外框共用。
- remake 現在由 renderer 使用 `8X8D1.DAX` block 202 原始框 symbol 補框，沒有把框
  烘進人物資產；英文／繁中、selector 1／1 與 14／12 都通過不裁切測試。
- 發行包 `v.1.1.6-20260914` 對拍把建角／`VIEW` 人物頁由 86.02%／86.25% 提升至
  92.12%／92.35%，兩張外框皆為 100%；正式收據是
  `docs/audit/portrait-frame-parity.json`。
