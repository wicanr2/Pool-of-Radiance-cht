...
func TestWorldTourReachesTheAreasBehindTheHarbour(t *testing.T) {
	...
	// **這一條沒有覆蓋面門檻，它記錄走到哪為止。**
	//
	// 使用者 2026-09-19 決定：探索器開作弊（#22）之後每一趟都走得完，量到的覆蓋面
	// 就到此為止，不再往上追（#55 關閉）。所以這裡只印數字，不拿它當閘門。
	//
	// **它現在證明什麼**：34 趟從標題開始、走正常按鍵、開著鎖 HP 與一擊斃命，
	// 全部走得完而且**一次硬失敗都沒有**——移動、換圖、選單、戰鬥與腳本邊界在這
	// 條路上不會卡死、不會炸掉。它**不證明**「世界只有這麼多走得到」：路線是機器人
	// 挑的，索寇那一條（另一組參數）就走得到這裡缺的 21／22／23／16／17／27。
	//
	// 沿革：2026-09-05 量到 17 個 block、門檻寫 15；開作弊之後量到 10 個。
	// 那兩個數字可比（當時還沒有戰後生命值寫回，隊伍等於不會死），差額的成因未明。
	t.Logf("走得到的 ECL block %d 個：%v（記錄值，不是門檻；每一趟的結束理由見上面）",
		len(blocks), blockIDs)
	// 硬失敗仍然是閘門：那是「走著走著卡死或炸掉」，與覆蓋面是兩回事。
	if len(hardFailures) != 0 {
		t.Errorf("出現 %d 次硬失敗", len(hardFailures))
	}
}
...
func TestPlayingTheWorldCompletesCommissionsOnItsOwn(t *testing.T) {
	...
	// **這一條也改成記錄，不設門檻**（使用者 2026-09-19，同上）。
	//
	// **它現在證明什麼**：探索器完全不給主線旗標、自己從開場走，走得完的委任是
	// 上面印的那幾條——它們的條件是玩家真的走到那張圖上滿足掉的，不是注入的。
	// 它**不證明**「只有這幾條走得完」：走到哪由路線與亂數決定。
	//
	// 沿革：曾量到五條（0 諾里斯、1 索寇要塞、11 瓦海登墳場、18 卡德納的付款、
	// 23 巴恩神殿），門檻就寫 5；開作弊之後量到四條。這一支當年抓到過兩個
	// 「測試綠、玩家卡關」的缺陷（NPC 入隊沒帶職業、治具在雙向梯子上一律答 YES）
	// ——那個價值現在由下面的硬失敗閘門與 spec 041 的兩支單測接手。
	t.Logf("玩家自己走完的委任 %d 條：%v（記錄值，不是門檻）", len(slots), slots)
	if len(hardFailures) != 0 {
		t.Errorf("出現 %d 次硬失敗", len(hardFailures))
	}
}
